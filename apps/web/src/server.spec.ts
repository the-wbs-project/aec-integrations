...
describe('createApp: no per-entity vendor route (AECI-978)', () => {
  function appWithSpyRenderer(): { app: ReturnType<typeof createApp>; ssrRenderer: SsrRenderer } {
    const ssrRenderer = vi.fn<SsrRenderer>(
      fixedRenderer(new Response('<html>x</html>', { status: 200 })),
    );
  ...
  it('routes /vendors/bluebeam through the SSR pipeline, not a hardcoded 301', async () => {
    // AECI-685's `/vendors/bluebeam` -> `/vendors/nemetschek-group` route lived
    // here and was deleted by AECI-978. The mapping did not go away — it moved
    // into `slug_redirects`, where the vendor detail RESOLVER emits the same 301
    // after its own read misses. This asserts the Worker no longer short-circuits,
    // which is what makes that resolver the single mechanism.
    //
    // The redirect itself is covered by the resolver harness's retired-slug suite
    // and by `apps/api/src/routes/slug-redirects.spec.ts`, which asserts the
    // migration actually seeds the row.
    const { binding } = recordingApiBinding();
    const { app, ssrRenderer } = appWithSpyRenderer();
    const res = await app.fetch(
