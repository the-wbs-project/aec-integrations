...
describe('HomeCredibilityStrip', () => {
  it('renders every non-zero coverage count with a pluralized noun, in order', () => {
    ...
    expect(labels).toEqual(['43 products', '33 vendors', '90 integrations', '12 reviews']);
  });
  ...
  it('suppresses a metric at 0 rather than showing a bare zero', () => {
    ...
    expect(labels).toEqual(['43 products', '90 integrations', '12 reviews']);
  ...
  it('collapses to a single honest empty-state line when every count is 0', () => {
    const el = setup({ totalProducts: 0, totalVendors: 0, totalIntegrations: 0, totalReviews: 0 });
  ...
  it('uses the singular noun for a count of 1', () => {
    const labels = metricLabels(setup({ totalProducts: 1, totalReviews: 1 }));
