  ...
  template: `
    <section class="mx-auto max-w-2xl px-6 py-12 space-y-6">
      ...
      <brn-dialog #demoDialog="brnDialog" aria-labelledby="demo-dialog-title">
        <ng-template
          brnDialogContent
          class="w-[min(90vw,28rem)] rounded-lg border border-(--border-default) bg-(--surface-raised) p-6 text-(--text-primary) shadow-lg"
        >
          <h2
            brnDialogTitle
          ...
          >
            This dialog uses BrnDialog + Angular CDK overlay and focus trap. Tab moves focus inside,
