...
describe('AutocompleteController — AECI-174 RUM emit', () => {
  beforeEach(() => vi.useFakeTimers());
  afterEach(() => vi.useRealTimers());

  it('emits a federated ok action bucketed by the TRUE total nbHits', async () => {
    const emit = vi.fn();
    const searchFn = vi.fn().mockResolvedValue({ ...result('Revit'), nbHits: 42 });
    const controller = new AutocompleteController(searchFn, emit);
    controller.setQuery('rev');
    vi.advanceTimersByTime(180);
    await flush();
    expect(emit).toHaveBeenCalledExactlyOnceWith({
      ...
      results_bucket: '21+', // 42 matches, NOT the single shown suggestion
    });
  });

  it('falls back to the shown-suggestion count when nbHits is absent', async () => {
    const emit = vi.fn();
    const searchFn = vi.fn().mockResolvedValue(result('A', 'B', 'C')); // no nbHits
    const controller = new AutocompleteController(searchFn, emit);
    controller.setQuery('ab');
    vi.advanceTimersByTime(180);
    await flush();
    expect(emit).toHaveBeenCalledExactlyOnceWith(
      expect.objectContaining({ index: 'federated', status: 'ok', results_bucket: '1-5' }),
    );
  });

  it('emits a federated error action when the search rejects', async () => {
    const emit = vi.fn();
    const searchFn = vi.fn().mockRejectedValue(new Error('Index does not exist'));
    const controller = new AutocompleteController(searchFn, emit);
    controller.setQuery('rev');
  ...
  it('still emits for a superseded (stale) response — every completed query counts', async () => {
    const emit = vi.fn();
    const first = deferred<AutocompleteResult>();
    const second = deferred<AutocompleteResult>();
    const searchFn = vi.fn().mockReturnValueOnce(first.promise).mockReturnValueOnce(second.promise);
    ...
    controller.setQuery('a');
    vi.advanceTimersByTime(180); // seq 1
    controller.setQuery('ab');
    vi.advanceTimersByTime(180); // seq 2
    second.resolve(result('B'));
    await flush();
    first.resolve(result('A')); // stale, ignored for signals — but still counted
    await flush();
