...
export function taxonomyBrowseIndexRequest(
  ...
  return {
    ...
    // The FULL product sort set, shared with `/products` (AECI-657). Before that
    // this page accepted only `created`/`name`/`updated` and rendered no control,
    // so `rating`/`reviews`/`integrations` were unreachable here even by hand-typed
    // URL — and `STAGE_1_SPEC.md` §4.5's "alphabetical, most integrations, most
    // reviewed" was unmet. The toolbar offers all six; this must accept all six.
