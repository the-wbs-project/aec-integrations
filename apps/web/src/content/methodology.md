...
**Retiring is not deleting.** An owner that stops offering an integration can retire it. A retired integration is taken off the public site, out of search, and out of the counts on product pages. Nothing is deleted. The owner can restore it later, and it comes back as it was, with its claims and confirmations. Open contests on it close when it is retired, and restoring it does not reopen them. We never retire or restore an integration ourselves. Deleting is different: we delete an integration only when its record was wrong or a product at one end is gone, and our catalogue tools refuse to delete an integration a vendor holds.

**Integrations a vendor adds.** An owner can also add an integration we have not recorded. It belongs to that vendor from the moment it is added, and it goes live without review by us. We did not research it, and its page carries the same "Offered by" line as any other.
