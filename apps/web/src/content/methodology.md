...
Catalogue data is compiled from publicly available sources: vendor documentation, public API and developer references, public integration directories and marketplaces, and vendor websites. We curate that material into the classification above.

Curation happens in a separate review application, not on the live site. A curator reviews a record there and publishes it deliberately. Nothing reaches the public catalogue on its own. Each publication is written in one step together with an audit record, so the catalogue is never left half-updated and the change is logged.
...
**Where this stands today.** Vendor confirmation requires a claimed and approved vendor account, and the vendor portal that grants those accounts is not yet open. So AEC Integrations is currently the source of every claim on the site, and you will see "Unverified · AECi" throughout. We would rather label that plainly than imply an endorsement nobody has given.
