...
const PRODUCERS: Record<AdminSnapshotMetricKey, Producer> = {
  ...
  //
  // Live rows only, in both tables (AECI-1010, AECI-1091). A retire is a real step
  // down in the catalogue, so recording it is correct.
  'catalog.integrations_total': async (db) =>
    (await countAll(db, integrations, liveIntegrationWhere)) +
    (await countAll(db, connectorEvidencedPairs, liveEvidencedPairWhere)),
