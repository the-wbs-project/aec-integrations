/**
 * Cron entrypoint for the API Worker (AECI-139 / Phase 3.6).
 *
 * The API Worker is request-driven (`index.ts` exports the Hono `fetch`); this
 * adds the first `scheduled` handler. Cron triggers are registered per-env in
 * `wrangler.jsonc` (staging + production only — NOT preview/PR-previews). The
 * handler dispatches on `controller.cron` so future scheduled jobs (e.g. the
 * §10 stats pipeline at 02:00) slot in alongside without a second Worker.
 *
 * 03:00 UTC — daily incremental Algolia sync (`./lib/algolia-sync`): push the
 * rows changed since the stored watermark to the env's three indexes, removing
 * `retracted`/`rejected` records. The full reindex is the separate AECI-138 CLI;
 * this keeps the index fresh between those.
 *
 * Best-effort + observable: the sync work is **awaited** (so a failure is logged
 * and the run isn't torn down mid-batch), while metric/log emission rides
 ...
 * derive the `host` tag, which falls back to the worker slug). Mirrors the
 * promote handler's `aeci.cache.purge` observability (AECI-105).
 */
