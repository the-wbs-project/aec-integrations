...
export const productsRelations = relations(products, ({ many }) => ({
  ...
  // Connector lane (AECI-722). The inverses of the four `one(products)` relations
  // below, so a product can be read from either end of the lane.
  ...
  connectorEvidencedAsConnector: many(connectorEvidencedPairs, {
    relationName: 'ConnectorEvidencedConnector',
  }),
  connectorEvidencedAsA: many(connectorEvidencedPairs, {
    relationName: 'ConnectorEvidencedProductA',
  }),
  connectorEvidencedAsB: many(connectorEvidencedPairs, {
    relationName: 'ConnectorEvidencedProductB',
  }),
}));
...
export const connectorEvidencedPairsRelations = relations(connectorEvidencedPairs, ({ one }) => ({
  ...
  builtByVendor: one(vendors, {
    ...
    references: [vendors.id],
    relationName: 'ConnectorEvidencedBuiltBy',
  }),
}));
...
export const schema = {
  ...
  // not prohibition. **AECI-722 discharged it**: `/admin/connectors` is the first
  // read surface over the lane, so the relations and the five inverse entries on
  // `productsRelations` are below. The public coverage surfaces (AECI-715 / 716)
  // inherit them rather than adding their own.
  // relations
  vendorsRelations,
  productsRelations,
