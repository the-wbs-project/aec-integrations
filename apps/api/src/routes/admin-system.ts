 ...
 * job's first instrumented run — and deleting it would make all ten rows read
...
/**
 * One liveness row per cron. All ten are always present — omitting a job would
...
async function cronRuns(
  ...
  return CRON_JOBS.map((job): AdminCronRun => {
    ...
    // deleting it would make all ten read "unknown" for up to 24h after every
