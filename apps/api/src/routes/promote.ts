...
function planEvidencedPairWrite(args: {
  ...
  /** AECI-730 tri-state, NOT `string | null`: `undefined` means the payload either
   *  omitted `builtByVendor` or named one AECi cannot resolve, and both must leave a
   *  stored FK untouched rather than clearing it. It rides `compact()` below with the
   *  other absent-means-untouched fields; only `connectorProductId` is unconditional,
   *  because `connector_product_id` is NOT NULL. */
  ...
  const links = { connectorProductId, productAId, productBId, direction };
...
export async function runPromoteIngest(
  ...
  for (const intg of payload.integrations) {
    ...
    // `poweredBy.value` is the AECI-730 tri-state; routing needs a CONCRETE id
    // (`connector_product_id` is NOT NULL), so `undefined` — key absent, or a
    // connector that did not resolve — is not a route to the delivered tier, exactly
    // as an explicit `null` is not.
    const poweredByProductId = poweredBy.value ?? null;
    ...
    if (routesToEvidencedPair) {
      ...
      // AECI-730 applies to this branch too. `powered_by` cannot be unresolved here
      // — `routesToEvidencedPair` requires a resolved `poweredByProductId` — but
      // `built_by` can, and `planEvidencedPairWrite` drops it from `compact()` the
      // same way the `integrations` branch does. Without this push a connector-
      // delivered edge whose vendor isn't promoted yet reports nothing, and the
      // `field:built_by` series would under-count by exactly the delivered tier.
