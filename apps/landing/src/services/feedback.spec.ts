...
describe('feedback', () => {
  it('rejects when neither features nor tools is provided', async () => {
    // Guards: empty feedback is meaningless — reject before any persist, even
    // if an email was supplied.
    const { API, fetch } = makeApi();
    const res = await post(buildApp(), makeEnv(API), { email: 'a@b.co' });
  ...
  it('treats whitespace-only fields as empty', async () => {
    ...
    const { API, fetch } = makeApi();
    const res = await post(buildApp(), makeEnv(API), { features: '   ', tools: '\n\t' });
  ...
  it('rejects a malformed email even when a suggestion is present', async () => {
    ...
    const { API, fetch } = makeApi();
    const res = await post(buildApp(), makeEnv(API), { features: 'dark mode', email: 'nope' });
  ...
  it('accepts anonymous feedback (no email) and writes the feedback row via the API binding', async () => {
    // Guards: anonymous path — email null, subscribed false, suggestion stored.
    const { API, fetch } = makeApi(
      new Response(JSON.stringify({ created: true }), { status: 201 }),
    );

    const res = await post(buildApp(), makeEnv(API), { tools: 'Revit, Bluebeam' });

    expect(res.status).toBe(201);
    expect(await res.json()).toEqual({ success: true });
    expect(fetch).toHaveBeenCalledOnce();
    expect(fetch.mock.calls[0][0]).toBe('https://api/api/feedback');
  ...
  it('returns 500 and logs when the feedback persist fails', async () => {
    const { API } = makeApi(new Response('boom', { status: 500 }));
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});

    const res = await post(buildApp(), makeEnv(API), { features: 'sso' });
  ...
  it('also writes to the mailing list when subscribe=true and an email is given', async () => {
    ...
    expect(fetch.mock.calls[0][0]).toBe('https://api/api/feedback');
    expect(fetch.mock.calls[1][0]).toBe('https://api/api/subscribe');
    const subRow = JSON.parse((fetch.mock.calls[1][1] as RequestInit).body as string) as Record<
      string,
      unknown
    >;
  ...
  it('does not write to the mailing list when subscribe=true but no email given', async () => {
    ...
    await post(buildApp(), makeEnv(API), { features: 'sso', subscribe: true });
    expect(fetch).toHaveBeenCalledOnce();
  });
