...
**Do not run `deploy:production` before migration `0048` is applied to production D1.** Since AECI-1091 the agent's queries filter `connector_evidenced_pairs.retired_at` without probing for the column, so on a database without `0048` every catalog query fails.
