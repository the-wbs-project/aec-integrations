...
- **Observability:** **PostHog only** (ADR 0024 / epic AECI-639). Logs, metrics, error tracking, web vitals and product analytics all land there. The AECI-639 dual-run ran PostHog beside Datadog to verify the swap against live traffic; **AECI-651 then deleted the Datadog leg** — both Worker adapters, `@datadog/browser-rum`, `observability/datadog/`, every `DD_*` var, and the CSP grants to the `browser-intake-*` hosts. **`stage-2` carries the whole of `main` as of the AECI-750 reconcile, so this is the only observability plane on this line; `main` (production) still runs the Datadog-only code until `stage-2` merges up.** Three facts worth knowing before touching observability: the Workers hold **no observability secret at all** (the publishable `phc_` `POSTHOG_PROJECT_KEY` is a committed per-env wrangler var, AECI-640); PostHog has **no `notify_no_data` equivalent**, so cron-absence detection is an external scheduled-CI liveness sweep (AECI-647); and **alerts evaluate hourly**, not every 5 minutes — the largest accepted degradation in the swap
