...
| Admin panel / operator console (traffic, audience, catalog, moderation, system health; the consent-independent read surface over `page_views` + a screen for the two cron digests) — **v1.0 build contract**; **Phase 8.3**, `main` line — epic AECI-572 integrated on the now-retired `admin-panel` branch and has landed | `docs/ADMIN_PANEL_SPEC.md` |
...
- Branch from `main` — for everything. **`main` is now the only branch on `origin`**: `stage-2` was retired at the 2026-09-03 Stage 2 merge and `admin-panel` was retired before it (`CICD_PLAN.md` §8). Nothing else is long-lived; short-lived epic branches are still fine when an epic needs one.
