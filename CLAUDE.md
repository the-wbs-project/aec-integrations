...
- **Theme:** light only until Stage 2 (see the "Light only (Stage 1)" constraint below; the semantic tokens in `docs/STAGE_1_SPEC.md` §2a keep dark re-introducible)

## Constraints that aren't negotiable

These appear repeatedly in tasks and Claude Code may be tempted to violate them. Don't.

- **Use Drizzle over the D1 binding (no Prisma in the Worker).** Get a request-scoped client via `getDb(env)` (`apps/api/src/db/client.ts`), which wraps `drizzle(env.DB, { schema })`. Reads use the relational query builder (`db.query.<table>.findMany/findFirst`) or `db.select()`; the schema source of truth is `apps/api/src/db/schema.ts`, with read configs/mappers in `apps/api/src/lib/drizzle-helpers.ts`. **D1 has no interactive transactions** — atomic multi-statement writes go through `db.batch([...])` (ADR 0016 / AECI-249), and every state-changing write emits its `audit_log` (+ `workflow_transitions`) row into the SAME batch via the `apps/api/src/lib/audit.ts` builders (the §26.1 invariant). The §26.5 Datadog forwards run post-commit in `ctx.waitUntil`. Prisma is fully removed (AECI-278) — no `@prisma/client`, no `prisma` CLI, no pg adapter, no `DATABASE_URL`/`DIRECT_URL`/Accelerate anywhere; do not reintroduce any of it. ADR 0016.
- **Promote is async — never commit on the request (ADR 0021 / AECI-563).** `POST /api/promote` validates, starts the `PROMOTE_WORKFLOW` Cloudflare Workflow, and returns `202 { jobId }`; the plan-then-batch ingest (`runPromoteIngest`, `apps/api/src/routes/promote.ts`) runs inside **one non-retried `step.do`** and throws `NonRetryableError` — an auto-retry can replay a half-planned create as a duplicate product. Never reintroduce a synchronous promote handler; the ingest takes a narrow `PromoteRunCtx`, never a Hono `Context`, so it can run off-request. The caller-supplied `jobId` is both the Workflow instance id and the PK of the `promote_jobs` ledger row written as the **first statement of the same `db.batch`** (AECI-571) — the exactly-once guard against at-least-once step replays. Never move that insert out of the batch, never add `ON CONFLICT DO NOTHING` to it, never compute `wrote` after pushing it, and never let an unreadable ledger row fall through to a re-plan (an unreadable ledger means the promote **already committed**). Post-commit hooks are fire-and-forget, dispatched from `run()` *after* the step, never inside it. Upsert-by-`supabaseId` falls back to insert (AECI-568): the update branch stays gated on the existence read, never `Boolean(supabaseId)`, or a dead pointer becomes a no-op `UPDATE` reported as `updated`. Full protocol + mechanism: `docs/REVIEW_APP_PROMOTE_API.md`, ADR 0021.
...
- **Release every `fetch` response body you don't read, and batch fan-out (AECI-666).** A Worker invocation may hold only a bounded number of open connections, and a `fetch` whose body is never consumed keeps holding one. Past the limit the runtime cancels the stalled responses to break the deadlock — and **a cancelled `fetch` returns a promise that never settles**, so the caller's own `catch` never fires, the work is lost with no log line, and the invocation is eventually killed as hung (`"your Worker's code had hung and would never generate a response"`), taking every other in-flight task with it. That is how the promote post-commit hooks silently dropped Algolia upserts and cache purges on ~8% of production promotes. Two rules, both enforced in review: **(a)** call `discardResponseBody(res)` (`@aeci/shared/response-drain`) on every path that doesn't read the body — including error paths that only inspect `res.status`; **(b)** never fan out an unbounded `Promise.all` of `fetch` — if the upstream takes a batch, send one request (the §26.5 audit forwards go through `logBatchToDatadog`, N entries → 1 request); if it genuinely has no batch endpoint (Google Indexing, the GoTrue per-id email lookup), run it through `mapWithConcurrency(items, WORKER_CONNECTION_LIMIT, fn)` from `@aeci/shared/concurrency`. Batching beats bounding; bounding beats nothing. Fire-and-forget `waitUntil` work in the promote path additionally goes through `dispatchHook`, whose 20s watchdog turns a wedged transport into a `console.warn` instead of a dead invocation. See ADR 0021's 2026-08-27 amendment.
...
Every cacheable SSR response sets a `Cache-Tag` header via the AECI-56 helper (`apps/web/src/server/cache-tags.ts`). Writes that affect cached pages purge by Cache-Tag through the shared `@aeci/shared` transport (`callCloudflarePurge`): the API Worker's promote ingest calls Cloudflare's purge-by-tag API **directly** (ADR 0010 — no web↔api binding; since ADR 0021 it fires from the promote Workflow's post-commit hooks, not the request), while the SSR Worker's `POST /admin/purge` remains the manual/incident + CI surface. Tag vocabulary, TTLs, composition rules, and the helper signature live in `docs/CACHE_STRATEGY.md`. The `invalidateForEntity()` / URL-invalidation-map approach in `docs/STAGE_1_SPEC.md` §9.3 is superseded.
