...
- `provideRouter` gets `withInMemoryScrolling({ scrollPositionRestoration: 'enabled', anchorScrolling: 'enabled' })` so new routes open at the top and Back/Forward restores scroll (`apps/web/src/app/app.config.ts:19-40`). The router resets scroll with `window.scrollTo`, which honors the global `html { scroll-behavior: smooth }`, so a browser-only `ScrollBehaviorManager` (`core/scroll-behavior-manager.ts`, started from `App`) forces the reset instant. A browser-only `InitialFragmentScroller` (`core/initial-fragment-scroller.ts`, also started from `App`) covers the one case the router misses — the *initial* load of a `…#section` deep link, where no `Scroll` event fires on the initial hydration navigation and `scrollRestoration: 'manual'` has disabled the browser's native jump. Don't route in-page section jumps through the router — they're native `<a href="{path}#id">` anchors by design. That does **not** make them invisible to the router: a same-document fragment navigation fires `popstate`, Angular reads `popstate` as a browser-driven navigation, and `RouterScroller` re-scrolls to the fragment after the browser already has. Angular's stock `ViewportScroller` ignores `scroll-margin-top` on that re-scroll, so `provideScrollMarginViewportScroller()` (`core/scroll-margin-viewport-scroller.ts`) replaces it with one that honors the target's `scroll-margin-*` and lands on the same pixel the browser chose. Any anchor that must clear a sticky bar declares the clearance in CSS (`scroll-mt-*`), never as a `ViewportScroller.setOffset()` constant.
...
- **Inline text next to an element on the next line needs `&ngsp;`.** Angular's default `preserveWhitespaces: false` collapses the newline between adjacent nodes, so `<ng-container i18n="…">Built by</ng-container>` followed by a `<a>` on the next line renders `Built byAgave`. Flex `gap-*` only helps when the two nodes are direct flex children; inside a plain `<span>` or `<p>` they aren't. Append `&ngsp;` after the closing tag (Prettier may wrap the closing bracket onto the `&ngsp;` line — that's equivalent). Cover it with a spec that normalizes `textContent` whitespace and asserts the joined string, since the two-node markup looks correct on inspection. Lint: 🟡 review-only.
...
- **New interactive/form-control patterns → Angular Aria.** This is the **rule** (ADR 0010, **Accepted**), not a proposal. For _new_ selects, comboboxes, listboxes, radio groups, accordions, trees, grids, menus, toolbars, and tabs, build on Angular Aria (`@angular/aria`, stable in v22) — it's first-party and Signal-Forms-friendly (§13). Spartan stays for the overlay primitives Aria lacks (Popover, Dialog); CDK remains the shared overlay/positioning foundation under both. Reference implementations: the review-submission form (`apps/web/src/app/reviews/review-form.ts`), the header search combobox (`apps/web/src/app/search/search-autocomplete.ts`), and the `/search` sort dropdown (`apps/web/src/app/search/widgets/search-sort-by.ts`).
  ...
  - **Exception: a plain single-value form field may be a native `<select>`** (ADR 0010 deviation (d), 2026-09-17). It applies only to one value from a closed, plain-text list, with no type-to-filter, in an ordinary form. Give it a `<label for>` and map the `""` row to `null`. Reference: the vendor data-flow forms (`vendor/components/vendor-add-claim-form.ts`, `vendor/components/vendor-attestation-control.ts`). Styled, filterable or overlay dropdowns stay on `AecSelect`.
  ...
  - **"Menus" in the Aria rule means APPLICATION menus, not navigation.** The decision table's `Menu / Menubar` row covers commands that act on the page. A row of **router links** is a disclosure-navigation pattern: `role="menu"` is the wrong semantic for it, and the WAI-ARIA practices advise against it. Those extend `layout/nav-disclosure.ts` instead, which is what keeps the four public taxonomy flyouts and the admin console's category row (`admin/admin-nav-dropdown.ts`, AECI-694) on one open/close contract — the contract `DESIGN.md` §Navigation requires. Two shipped precedents predate the admin row and reach the same conclusion from the other direction: `layout/user-menu.ts` ("no `role=\"menu\"`/roving tabindex") and the vendor portal's former Products menu, which was a combobox because `role="menu"` may not own a textbox (retired in `STAGE_2_VENDOR_PORTAL_SPEC.md` §6.11).
  - **The accordion is off-limits for SSR-indexable content (AECI-841).** `AccordionPanel` renders its content through `DeferredContent`, which creates the embedded view inside an `afterRenderEffect` — and `afterRenderEffect` runs **only on the client** ([angular.dev](https://angular.dev/guide/signals/effect#server-side-rendering-caveats)). So on a server-rendered page the panel serialises **empty**: no content for a crawler, and a visible pop-in after hydration. `preserveContent: true` does not rescue it, because it only stops an already-created view being destroyed. If the content inside a collapsible region is indexable, or feeds an `@defer (hydrate on viewport)` block, hand-roll the WAI-ARIA disclosure pattern instead — a `<button aria-expanded aria-controls>` inside the heading over a `role="region"` panel hidden with the `hidden` attribute, content in the DOM in every state. Reference: `apps/web/src/app/products/integration-group-card.ts`. Popups and menus are unaffected: their content is not indexable, so Aria's deferred rendering is correct there.
  - Full rationale + the three deviations: `docs/adr/0010-angular-aria-alongside-spartan.md`.
