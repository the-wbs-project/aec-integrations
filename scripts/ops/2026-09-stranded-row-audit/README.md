...
### Vendor-held rows are counted, never reported (AECI-1005, AECI-1088)
...
| **AECI-916** | The two 2026-09-14 Aquifer HeavyJob pairs. **Not yet ruled** — why upstream stopped claiming them is unknown, and AECI-795 is the precedent for not inferring a cause. **The journal route is now closed to them.** AECI-889 batch 3 deleted both upstream records as unpromoted rows on 2026-09-14, and an unpromoted delete journals nothing, so there is no entry for the consumer to take and no upstream record left to re-delete. Both carried a NULL `supabase_integration_id`, which is the cause of the strand and the reason the delete wrote no entry. Whatever the ruling, executing it needs a tool that reaches `connector_evidenced_pairs` directly — this bucket still has none. |
...
- **Detection was never the weak link.** Five of the seven rows already had issues filed;
  they sat live because nobody executed the ruling. The daily backstop, meanwhile, had
  never run at all for its first 25 scheduled runs — AECI-796 re-pointed it at this lane
  on 2026-09-08, so from now on a filed-but-unexecuted ruling stays visible every morning.
