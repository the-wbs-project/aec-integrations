...
//
// Copied from scripts/ops/2026-09-stranded-row-audit/mcp-client.mjs (the copy with 429
// retry). Each ops lane is self-contained by convention. The AECI-996 repair reads only
// `list_integrations`, `list_products` and `get_product`.
//
...
export async function openMcpSession({ url = MCP_URL, token = requireMcpToken() } = {}) {
  ...
  await post({
    ...
    params: {
      ...
      capabilities: {},
      clientInfo: { name: 'aeci-ops-evidenced-claim-direction-repair', version: '1.0.0' },
    },
  });
