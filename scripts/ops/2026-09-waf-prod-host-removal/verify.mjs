...
//
// STRICTLY READ-ONLY and side-effect-free: three hosts x two user agents = six GETs
// of /products. It writes nothing, submits nothing, and sends no email. Same probe as
// the AECI-659 directory's, over the narrowed host set, so the two tables are directly
// comparable minus the `prod.` row.
//
// Expected BEFORE and AFTER apply.mjs: scraperUA -> 403 (Managed Challenge),
// browserUA -> 200, on all three hosts. This operation REMOVES a host from the rules;
// it must not change the behaviour of the hosts that remain, so an unchanged table is
// the pass condition. A row that flips to 200/200 means the narrowing overshot.
//
// `prod.aecintegrations.com` is deliberately not probed: AECI-807 retires the
// hostname, so a request to it proves nothing about the rules. Check separately that
// it stops resolving to an AECi Worker.
//
...
//   node scripts/ops/2026-09-waf-prod-host-removal/verify.mjs --path /vendors
//
