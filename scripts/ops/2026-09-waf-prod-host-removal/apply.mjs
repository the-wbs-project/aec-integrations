#!/usr/bin/env node
//
// apply.mjs — narrow the three WAF rules by dropping the retired
// `prod.aecintegrations.com` from each host set (AECI-807).
//
...
// the scraper rule are carried through verbatim from the live rule. Descriptions are
// untouched too — none of the three names a host, so narrowing the set makes no label
// false (AECI-659 rewrote Rule A's for a different reason).
//
// This is the SECOND migration of these same rules. Run ../2026-09-waf-host-scope/
// first if any rule is still on the pre-AECI-659 two-host form; this script aborts on
// it rather than skipping a generation.
//
...
//   node scripts/ops/2026-09-waf-prod-host-removal/apply.mjs          # dry run, Zone WAF: Read
//   node scripts/ops/2026-09-waf-prod-host-removal/apply.mjs --apply  # writes, Zone WAF: Edit
//
