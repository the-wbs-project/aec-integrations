...
- **DetailLayout** (`<aec-detail-layout>`) — for product / vendor / integration detail pages.
  ...
  - **Grid:** `xl:` two-column (2fr main / 1fr metadata) with sticky-top metadata spanning both body rows; single column below `xl`. **The split is `xl:`, not `md:`, deliberately** — the body column carries data tables with a 44rem minimum width, and a docked 1fr sidebar at `md`–`lg` squeezed the 2fr column under 40rem, so every table degraded into a horizontally-scrolling well. Below `xl` the page gives the body the full content width instead.
