...
- **Per-tab sort dropdown** (`<aec-search-sort-by>`, `search/widgets/search-sort-by.ts`, AECI-175 / ADR 0014) — the sort control above the `/search` results (Relevance · Name A–Z, per the Products/Vendors tab; AECI-636 PR-B retired Most integrations on 2026-09-22). A **non-editable Angular Aria combobox + listbox-in-overlay**, the same discrete-choice pattern as the review-form role picker (ADR 0010): `ngCombobox` + `[(expanded)]` trigger, with a `cdkConnectedOverlay` (`usePopover:'inline'`) supplying the floating layer since `ComboboxPopup` renders in-flow. A real label names the trigger (`aria-labelledby`), the active sort shows in the trigger with a ✓ on the chosen option, and labels are `$localize`d. Backed by Algolia **replica indexes** (`SEARCH_RANKING.md` §5a); the active sort mirrors to `?sort=`. It mounts browser-side once results load (it reads the controller's sort options), so the degraded/SSR shell shows no sort control.
### Extensions: "Built within" (AECI-710)

Two components in `apps/web/src/app/products/product-extensions.ts` render the `product_extensions` relation on product detail (`STAGE_1_5_SPEC.md` §13.3b). Anchor site: **Faire**, the catalog's anchor. Its "Shop more from {brand}" parent strip is the pattern for the sidebar card, and its brand tile row for the body grid.
...
- **Public request form** (`<aec-request-form-body>`, `requests/request-form-body.html`) — the shared body behind both the routed `/{products,vendors}/:slug/{claim,correction}` page and the in-place `RequestDrawer`. One Signal Forms model; the active `kind` picks the shared Zod schema (`ClaimFormSchema` / `CorrectionFormSchema`). Both free-text bodies carry a **20-character floor**, so both state it in persistent help text under the textarea along with what to actually write — the claim's "Anything we should know?" reads as an optional afterthought otherwise, and its `body` column is NOT NULL (see the §5 → Inputs named rule). In the **drawer** variant the panel is a clipped flex column with **one** scroll port: the eyebrow/title/close/subtitle block is pinned above it and the submit control is pinned below it in a bordered `--surface-raised` bar, so only the fields scroll and "Send claim"/"Send correction" is never scrolled out of reach. The bar full-bleeds past the port's horizontal padding and owns the panel's bottom padding (the port has none), which is what keeps content from showing beneath it. The routed **page** variant keeps normal flow — it has no scroll port of its own and the browser window is the scroller.

- **Request queue** (`<aec-request-queue>`, `admin/requests/request-queue.ts`) — the `/admin/requests` child route. **Corrections only since AECI-922** (`ADMIN_PANEL_SPEC.md` §5.0c): the request pins `kind: 'correction'`, the heading reads "Correction requests", and vendor **claims** live on `/admin/claims`. Lists corrections as cards with a status badge, the linked target product/vendor, submitter, and submitted-age. **One** client-side filter (status: open / resolved / rejected), an `aria-pressed` toggle-button group. Each row surfaces the **possible-duplicate** flag computed at submit (an open sibling request for the same target) — informational, never auto-deciding; the admin resolves every request by hand. The **domain-match** chip left with the kind filter: it compared a *claimant's* email domain to the vendor's, and a correction identifies nobody. The linked **Linear issue** opens out. **Resolve** is one-click; **reject** opens an optional reason field. On success the row leaves, the Operations nav badge ticks down, and a `role=status` live region announces it; a capped-note shows when the server holds more rows than the page loaded.
...
- **A sortable header selects a sort on the first click and REVERSES it on the second.** `<th aec-sort-header>` (`shared/sort-header/sort-header.ts`) renders a button plus a ↑ or ↓ and sets `aria-sort` to the direction currently in effect for that key. Clicking an inactive header adopts that column's **natural** direction — one copy, in `ADMIN_VENDOR_SORT_DEFAULT_ORDER` / `ADMIN_USER_SORT_DEFAULT_ORDER` (`packages/shared`), which the API's `apps/api/src/lib/sort.ts` also resolves its ORDER BY from, so the arrow cannot disagree with the list. Clicking the active header sends `?order=asc|desc`. *(It originally did not flip, because no list endpoint took an `order` parameter; that held over a two-key API and became a defect at seven — an arrow on a clickable header reads as a toggle, and a second click that silently no-ops is the worse lie. `ADMIN_PANEL_SPEC.md` §5.7 carries the full reasoning.)* The corollary is unchanged and matters more: **a column the API cannot order by gets no control at all**, and stays plain `<th>` text with no hover state. A header that looks clickable and reorders the 25 rows on the current page presents a page as a ranking, which is worse than no control. (`/admin/users` Last sign-in is the standing example: it comes from GoTrue per id, *after* the ORDER BY has already chosen the page.)
...
- **Explanatory copy that does not deserve a paragraph is an `<aec-info-hint>`** (`shared/info-hint/`, AECI-915) — a 16px "i" button next to the thing it explains, with the text in a `cdkConnectedOverlay` on hover, focus and click. It generalises the control `<aec-relative-time>` introduced above and keeps that component's binding rule: **the accessible name of the button IS the text**, and the panel is `aria-hidden`. Not a `title` attribute (not reliably keyboard-reachable, inconsistent in screen readers, unstylable) and not an `aria-describedby` pointing at the panel (which makes assistive tech depend on a transient overlay and double-announces). Reach for it when the copy is a sentence or two of *optional* orientation; if a reader needs it to complete the task, it is body text, not a hint. First consumers: the vendor portal's taxonomy cards, at two levels — the facet hint beside the heading and each term's description beside its row. **Never nest one inside a heading**: an accessible name is computed from descendants, so a hint inside an `<h3>` makes the whole paragraph part of that heading's name and wrecks the screen-reader heading list. Put it in a wrapper row *beside* the heading. Nothing lints this and axe cannot see it.
...
**One header, and it follows the context** (`vendor/vendor-dashboard-tabbed.ts`,
§6.11). A breadcrumb sits where the eyebrow was: `Vendor › Company` on vendor pages,
`Vendor › Company › Products › Product` once a product is open. The `h1`, the "View
public page" link and the tab row all switch to the open product, and a quiet "← Back
to Company" link sits at the end of the breadcrumb line. Never stack a second heading
or a second nav row under the first: two headers of different sizes left it unclear
which one owned the page, and a segmented product row beneath an underlined vendor row
read as a button group rather than navigation. Breadcrumb links are `text-secondary`,
hover Forest, no underline, with the current crumb in `text-primary`, matching the
public product page.

**The tab row is a horizontal row under the title**
...
nav on a long scroll, and a router nav has no such coupling. The same component draws
the product row, so the two can never drift apart visually.
...
- **Products is a plain link to a product list** (`sections/vendor-product-list-page.ts`, §6.11). The list is one bordered `--surface-raised` card of whole-row links: a small logo, the name, and a quiet "Primary" tag. Primary first, then by name. It is deliberately basic until the product list gets its own design pass. It replaced a search-box dropdown in the nav, which had to go once "Products" became a breadcrumb destination.
- **A product page shows one product** (`sections/vendor-products-page.ts`). The form opens with its read-only name block, and a URL naming a product the vendor does not own says so, with a link back to the list, rather than quietly rendering a different one. **No card wraps the form** (AECI-1116): it sits flat under the tab row, the same depth as the vendor Profile form. A raised card there put the sunken name block and the logo drop zone inside a second card. Explanatory sentences inside a full-width sunken note carry `max-w-prose`, so the box spans the form and the text keeps a reading measure.

- **Each taxonomy facet is its own tab, edited inline** (AECI-994, `STAGE_2_VENDOR_PORTAL_SPEC.md` §6.12). Categories, Trades, Audiences and Phases each render the full vocabulary as a checklist, a `<label>` around a real checkbox with the term description beside it, and one page Save. Under a ticked Audience or Phase sits a bullet list (`vendor/components/vendor-bullet-list-editor.ts`): an input per point, a plus button that adds at the bottom, and bordered 32px icon buttons to move up, move down and remove. Reordering is buttons rather than drag so it needs no second interaction model. Unticking a term with points confirms in a `BrnDialog` that lists what will be lost. The history below is superseded.
- **(Superseded) Taxonomy reads as a summary and writes in a modal** (AECI-915). The four facets shipped as fieldsets of `aria-pressed` toggle chips — 107 chips on one screen — which made "what is this tagged as" a visual diff and left the per-term descriptions nowhere to go. Now each facet is a card in a **two-column grid**: name, a right-justified pencil, and one row per *assigned* term. This is the exception the "Don't use modals as the first thought" rule contemplates — the alternatives were tried and shipped, and the read case and the write case genuinely want different densities. The picker (`vendor/components/vendor-taxonomy-facet-dialog.ts`) is a `BrnDialog` opened imperatively from the click handler (never an `effect()`, NG0602), and every row is a `<label>` wrapping a real `<input type="checkbox">` so the whole row is the click target and the keyboard and ARIA behaviour is native rather than re-implemented over `role="checkbox"`. **Its Save persists**, because there is no `CanDeactivate` guard on this app and a Save that only staged would lose work silently; a failure keeps the modal open with the draft and puts the error inside it.

- **The Overview is a landing page, not an inventory** (AECI-983, `STAGE_2_VENDOR_PORTAL_SPEC.md` §6.10). Three blocks: the access strip, a glance band, and a "What needs you" list of whole-row links (`rounded-(--radius-md)` bordered rows, a Lucide severity icon, a status pill, an `aria-hidden` chevron with `rtl:-scale-x-100`). Row hover moves the **surface** (`hover:bg-(--surface-sunken)`), never the border, for the cascade reason recorded above. Pill border tones are the unlayered `.aec-pill-conflict` / `.aec-pill-attention` classes in `styles.css`, the same escape hatch as `.aec-nav-tab`. Below `sm` the pill drops under the row's text instead of taking a column beside it (AECI-1116): at 375px a long pill left the title about two words a line.
- **The glance band reuses the home stats cards, with two departures** (`vendor/components/vendor-glance-band.ts`). Vocabulary from `home/home-stats-cards.ts`: three `rounded-(--radius-lg)` `--surface-raised` cards, a `font-display text-5xl` figure, a quiet `--text-secondary` line, and every label row `min-h-9 md:min-h-12` so the figures align even when a label wraps at `md` with the Views toggle. **A zero is never a bare 0**: it renders a sentence ("No conflicts", "None open"). **Departure one: a non-zero conflict figure is `--status-error`**, not Forest. `conflict` is the one sanctioned red, and a conflict count that reads like good news defeats the tile. At zero there is no figure, so red never appears for good news. **Departure two: tile hover moves the surface**, not the border, because a border-colour utility does not apply here.
- **The Views period toggle is an `aria-pressed` button set, not Angular Aria** (`vendor/components/vendor-views-tile.ts`). Aria@22 ships no radio group (ADR 0010), and the admin date-range control (`admin/traffic/traffic.html`) already made this call, so the vendor surface follows the shipped precedent: a `fieldset` with an sr-only legend, a sunken segmented track, and the pressed segment filled Forest by the unlayered `.aec-period[aria-pressed='true']` rule, which also sets the pressed segment's focus ring to Forest so it does not vanish. Visible "1d", accessible name "1d, last day" (WCAG 2.5.3). The tile is a placeholder until AECI-941 and shows no number.
- **The connector seat's Catalogue tab is a row card, not a table** (AECI-1083, `STAGE_2_VENDOR_PORTAL_SPEC.md` §6.16). It shows only on a `connector`-role product, last in the product row. The listings sit in the product list's bordered `--surface-raised` card, one row each: the name in the body face at 16px (on a span, because the `h3` element rule is unlayered serif), a "View listing" link, then each match as a phrase with a quiet `--surface-sunken` "Counts toward reach" pill. The admin triage table needs 64rem, so the portal does not borrow it. Edit opens the form inline under its row, capped at `max-w-2xl`; one form at a time, focus moves in on open and back to Edit on close. Disabled buttons swap to the sunken surface rather than dimming. Text blocks are capped at 52ch, because `npx impeccable detect` measured ~96 characters a line at 65ch in Atkinson.
- **The plan panel has a compact strip for the quiet state** (`vendor-plan-panel.ts`, `compact` input). Only `active` and not expiring collapses, to one row: the account badge, the term, and a `<details>` "What an active account covers" holding the framing sentence. Every other state is a conversation and keeps the full panel. The overview hides its "Account access" `h2` visually (`sr-only`) exactly when the strip shows, so the heading outline does not change. `impeccable detect` reports the closed disclosure's sentence as `text-occlusion`, and the Views toggle's sunken track as `nested-cards`. Both are false positives, recorded as EX-004 in `docs/CODE_REVIEW_EXEMPTIONS.md`.
...
**No new Mobbin anchor was picked, deliberately** — the same call the operator console made above (`ADMIN_PANEL_SPEC.md` §9.10), and recorded here because the Anchor-Site Rule's "record the anchor site with the surface" had never been satisfied for `/vendor`. The portal inherits the Phase 5/6 admin-queue and Phase 8.3 console vocabulary: bordered `--surface-raised` cards, border not shadow, the eyebrow-then-heading header, Forest figures, `tabular-nums`. The §6.11 breadcrumb reuses the public product page's breadcrumb, so it does not introduce a second anchor. It is an internal, signed-in surface reading the same catalog the public directory renders, so a second reference site would make AECi read as two products. One publication, one voice (Anchor-Site Rule). Token-only, i18n throughout, light-only.
...
- **Every claim lane says what happens next.** One plain-text sentence under the direction line, in all nine states, naming the actual consequence: who gets emailed, after how many days, and what the public listing shows meanwhile. It quotes the real detector thresholds (14 days, 7 days, 12 months) rather than rounding them into words, because the numbers come from the same shared constants the detector reads. A vendor who denies a false claim and sees no acknowledgement assumes nothing happened, and that action is the most valuable thing a seated vendor does for us. It is standing state, so it is plain text and never its own live region; the shell's one channel announces the same sentence when a write lands. Contract: `STAGE_2_ATTESTATIONS_SPEC.md` §6.2.
...
- **A child route level replaces its parent's row; it does not stack a second one.** The vendor portal tried a segmented secondary row under the underlined primary row (AECI-959) and retired it (§6.11): two rows of different styles read as a button group and a nav competing for one page. Show one underlined row for whichever level the page is at, and let a breadcrumb carry the way back up.
...
- **DetailLayout** (`<aec-detail-layout>`) — for product / vendor / integration detail pages.
  - **Slots:** `breadcrumbs`, `hero` (required), `nav`, `body-lead`, `metadata`, `body` (required).
  - **Grid:** `lg:` two-column (2fr main / 1fr metadata) with sticky-top metadata spanning both body rows; single column below `lg`. **The split is `lg:`, not `md:`, and the number is arithmetic, not taste** — the body column is `(content − 48px gap) × 2/3`, i.e. **608px at a 1024px viewport** and 778px at 1280px. A body-column table whose `min-width` exceeds that figure does not wrap; it overflows its own `overflow-x-auto` wrapper and scrolls inside a narrow well. **Check any new body-column table against 608px, not against the viewport.** *(This was `xl:` until AECI-853, 2026-09-10: the product-integrations table asked for 44rem / 704px, so only 1280px cleared it. Folding that table's Direction column into the partner cell's meta line took its worst case to 545px, and the vendor-detail products table came down to the same 34rem floor, so 1024px now clears both. Do not raise either back above 38rem.)*
  - **The sticky in-page nav gets its own slot.** `nav` is projected **bare** (no wrapper element) as a direct child of the page container, above the grid. `position: sticky` is bounded by the parent's box, so a nav inside a body column unpins as soon as that column scrolls past — long before the sections it links to. Its bottom margin therefore lives on the nav element itself. Never wrap that slot.
  - **Where the sidebar goes when it undocks:** between `body-lead` and `body` — *not* at the foot of the page. `body-lead` holds About; the sidebar's vendor / taxonomy / action facts are what a visitor wants first, and collapsing them under the last body section buried them. Placement is explicit `col-start` / `row-start`, so DOM order is the single-column reading order and the two-column arrangement is pure grid placement. One instance, never two: the projected content labels its groups with `aria-labelledby`, so a render-twice-hide-one approach would duplicate DOM ids.
  - **Two formats for one sidebar.** Docked (`lg`) it is a bare stacked column on the page background with full-width action buttons. Undocked (below `lg`) it becomes a contained fact panel — `surface-raised` fill, `border-default` hairline, `radius-lg`, groups pairing into two columns from `sm`, the vendor row and action buttons rendered as `surface-base` cards on the panel, and actions inline at their natural width. Same markup, breakpoint-scoped utilities.
  - **Internal rhythm:** hero bordered below; body sections stack with `space-y-12`; metadata sidebar uses `space-y-6`. Vertical rhythm between the three grid children is margins, not `gap-y`, so an absent `body-lead` collapses to nothing rather than leaving a gapped phantom row.
