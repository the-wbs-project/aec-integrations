...
- **Handover breakpoint:** the inline primary nav appears at `lg` and up; below that the hamburger carries it (`aec-nav-menu` is `lg:hidden`). Moved up from `md` when Trades became the fourth taxonomy flyout (AECI-544): seven items plus the wordmark and sign-in CTA no longer fit a 768px header, and clipping nav items out of the viewport is worse than deferring to a labeled overlay that already lists every facet. Adding a further top-level nav item needs a re-measure, not just an insert.

- **Search changes shape across three bands; it never disappears.** Search is the one header affordance that is *resized* rather than dropped, because the row's slack swings ~250px across the desktop range: below `lg` it lives inside the hamburger overlay, from `lg` to `xl` it is a 36px icon that opens a popover (`aec-search-trigger`), and at `xl`+ it is the inline `w-52` input. The three mounts are the same component at three widths, and their visibility classes are complementary by construction, so exactly one is ever live. This is a rule because it was once broken: the hamburger stopped at `lg` and the inline box started at `xl`, leaving **1024-1279px with no search at all** for as long as those two breakpoints were set independently. Widening the box down to `lg` is not the fix and was measured, not guessed - see the next rule.

- **The header row has no slack left at 1024px.** Measured, signed out (the worst case, since "Sign in" is wider than the account icon): wordmark 235px, nav content 561px, right cluster 124px. The nav's flex box at that width is 561px, i.e. the row fits with about **1px** to spare, and it fit with about 1px to spare *before* the search icon too. The icon only fits because the `lg`-band gaps were tightened to pay for it (nav `gap-3.5`, row `lg:gap-6`; both go back up at `xl`). Consequences: a new control in this row must displace something, not join it; a longer wordmark or a locale with longer nav copy will clip; and any change here is verified by measuring `scrollWidth - clientWidth` at 1024px, not by eye at 1440px. The cheapest ~51px available is demoting `Home` into the wordmark, which already links to `/`.
