...
**Drift layers, recap.** Layer 1 (this workflow) guards the **D1 app schema**. Layers 2–3 are a *separate* gate for the retained Supabase **Auth** Postgres (`scripts/prisma-drift-check.sh` against `schema.prisma`), documented above and below — they no longer cover the app tables:
...
| 3. Post-promote | `promote-to-prod.yml` | `apply-prod-migrations` job | Supabase Auth Postgres: `schema.prisma` ↔ prod |
...
| `scripts/prisma-drift-check.sh` | (AECI-77) `prisma db pull` + `prisma migrate diff --exit-code` against the retained Auth Postgres — reused by refresh-staging and promote-to-prod (no longer by drift-check, which is D1/Drizzle since AECI-264). |
