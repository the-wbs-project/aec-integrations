...
- **Deploy marker + smoke** — **two markers, both best-effort** (dual-run, ADR 0024): the Datadog `/api/v1/events` marker (docs/CICD_PLAN.md §9.1) tagged `env:production`, `service:aeci-ssr`, `commit:<sha>`, **and** the PostHog marker (`scripts/ci/posthog-deploy-marker.sh`, AECI-640) — a project annotation plus a queryable `deployment` event carrying `env`/`service`/`version`/`deploy_kind`. The prod rollback path emits a PostHog marker too, with `deploy_kind: auto_rollback`. Then via `scripts/verify-version.sh` + `scripts/verify-health.sh` polls until **both** `https://prod.aecintegrations.com/api/version` (API Worker) and `/_version` (SSR Worker, AECI-92) report `sha: "<input>"` **and** `/api/health` is `db:ok`. Fails after a 60-second budget; a smoke failure auto-rolls-back both Workers.
...
| `DATADOG_API_KEY` | ❌ | ❌ | ✅ | Used by `promote-to-prod.yml` (AECI-78) to POST deploy markers to Datadog `/api/v1/events` per CICD_PLAN §9.1. **Dual-run (ADR 0024):** the PostHog deploy marker (`scripts/ci/posthog-deploy-marker.sh`, AECI-640) now runs on every deploy path alongside it; the Datadog marker steps and this secret are deleted at **AECI-651**, not before. |
...
| Datadog `DD_*` (per `apps/web/wrangler.jsonc` header) | ✅ per env | ✅ per env | — | RUM + Logs intake. **Still live and still the operating safety net** — production alerting runs on Datadog today (ADR 0024 dual-run). `DD_API_KEY` (both Workers, manually provisioned) + `DD_APPLICATION_ID`/`DD_CLIENT_TOKEN` (web Worker, CI-pushed) + the `DD_SITE` vars all stay until **AECI-651** deletes the Datadog leg after a 2–4 week prod soak. Do not remove them as part of PostHog work. |
...
> Datadog's `DD_*` provisioning above is **not** superseded by this section. Production
> alerting still runs on Datadog and both vendors are provisioned in parallel until
