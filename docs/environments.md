...
| PostHog Worker secrets | ❌ **none** | ❌ **none** | — | **Deliberate: the Workers hold no PostHog credential at all.** All three PostHog intakes — OTLP logs (`/i/v1/logs`), OTLP metrics (`/i/v1/metrics`) and event capture (`/capture/`) — authenticate with the publishable `phc_` project token above. This is the "Worker telemetry secrets 4 → 0" line in ADR 0024, and it is the reason the row above exists as a var rather than a secret. If a future change wants a Worker-side PostHog *personal* key, that is a design change, not a provisioning step — see `POSTHOG_CLI_API_KEY` below. |
