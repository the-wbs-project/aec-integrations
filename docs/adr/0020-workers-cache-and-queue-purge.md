...
> **Update — WC-7 done (AECI-321, 2026-07-16).** The **datatool** (§4) now enqueues instead of HTTP-purging. `apps/datatool/src/cache-purge.ts`'s `purgeEnvCache` no longer calls `callCloudflarePurge` with `BROAD_CACHE_TAGS`; after a copy/seed/reindex it sends a single `{ purgeEverything: true, source: 'datatool' }` `CachePurgeMessage` onto the **target tier's** `aeci-cache-purge-{env}` queue, consumed by that tier's own SSR Worker (`ctx.cache.purge({ purgeEverything: true })`). `purgeEverything` — not a broad-tag set — because a full clone/seed invalidates the tier's entire cache and it sidesteps any `route:*` tag-coverage gap. Because the datatool is a single Worker with **no wrangler `env.*` blocks**, it binds all three cacheable tiers at once — new producer bindings `CACHE_PURGE_QUEUE_{STAGING,DEMO,PRODUCTION}` in `apps/datatool/wrangler.jsonc`, resolved per request in `targets.ts` (mirroring its four `DB_*` bindings). There is **no preview producer**: preview is `*.workers.dev` with no cache and no `aeci-cache-purge-preview` queue, so a preview/unbound target is a graceful no-op (best-effort preserved; a `queue.send` failure is caught, never fails the D1 write). **Cross-tier bleed is gone** — under per-Worker caches each tier evicts only its own cache (the old shared-`aecintegrations.com`-zone caveat in the datatool doc-comment is retired). `source: 'datatool'` and `CachePurgeMessage.purgeEverything` were already reserved in `@aeci/shared` by WC-5, so no shared-package change. The datatool stops reading `CF_PURGE_API_TOKEN` / `CF_ZONE_ID` (annotated in `env.ts` / `wrangler.jsonc`; their global retirement remains WC-10). The three call sites (§4) are now all on the Queue except `retract-product`; verified via `apps/datatool` unit tests, with the real staging enqueue→consume validated post-deploy via `aeci.cache.purge{source:datatool}`.
