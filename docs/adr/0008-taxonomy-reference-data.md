...
- ~~Descriptions are seeded `NULL` for now; SEO copy can be added to the file later.~~ **Closed 2026-09-14.** All 73 category / audience / phase terms now ship a populated `description`, and `description` was added to both the `INSERT` column list and the `ON CONFLICT DO UPDATE SET` clause so later edits propagate on re-seed. The copy is written to do two jobs at once: it is the reader-facing paragraph and meta description on `/{categories,audiences,phases}/:slug`, **and** it tells whoever is tagging a product which of two adjacent terms to pick (for example BIM Authoring vs BIM Coordination, Estimating & Takeoff vs Bid Management, MEP Engineering vs Mechanical Engineering). That second job matters because these three facets resolve **find-or-create** in promote, unlike trades and data objects: a near-miss label mints a real term with a permanent public URL rather than failing loudly.
- **Exception (AECI-540):** `taxonomy_trades.description` is `not null` and ships populated, because `/trades/:slug` is an SEO landing page from day one. The three sibling columns stay nullable — the values are now always present, but the constraint was not tightened.
