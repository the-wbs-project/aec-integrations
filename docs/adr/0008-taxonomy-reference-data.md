...
- **Application** is uniform across environments:
  ...
  - **Dev / Staging / Prod** — a `psql -v ON_ERROR_STOP=1 "$DIRECT_URL" -f supabase/reference-data/taxonomy.sql` step after `supabase db push` in `deploy.yml`, `refresh-staging.yml`, and `promote-to-prod.yml` (mirrors the existing `seed-staging-users.sql` pattern). The dev seed in `deploy.yml` runs on every PR; because that job otherwise authenticates via `SUPABASE_ACCESS_TOKEN` (not a direct URL), its seed step is gated on the optional `DIRECT_URL_STAGING` secret and skips with a warning if it isn't configured, so a missing secret never fails the PR-gating job. Staging/prod keep their `DIRECT_URL_*` as hard requirements (those workflows already need them for the drift/RLS/restore steps).
