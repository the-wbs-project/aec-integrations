...
- `0008-taxonomy-reference-data.md` — **Accepted**: the taxonomy vocabulary (categories/disciplines/phases) is code-managed reference data in `supabase/reference-data/taxonomy.sql`, applied to every environment via idempotent upserts. Supersedes the §13 "Airtable owns taxonomy" stance for the taxonomy facets only (vendors/products/integrations stay Airtable-owned).
