...
- All canonical-building sites consume `canonicalUrl()`: the browse + detail resolvers (`taxonomy-browse.resolver.ts`, `create-detail-resolver.ts`), the index pages (`products-index.ts`, `vendors-index.ts`, `integrations-index.ts`), and the `/categories` resolver (`categories-index.resolver.ts`). The previously-divergent `DEFAULT_ORIGIN` constant / inline apex strings are removed.
