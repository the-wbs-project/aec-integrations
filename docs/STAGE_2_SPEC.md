...
- **Vendor dashboard UI.** Edit product/vendor content (name, description, links, taxonomy within guard-rails), see claim/correction status, manage the verified badge. **Multi-seat** (§8.1(2)) — several admins per vendor. The FIRST seat is granted through the review above; since AECI-664 an **owner** seat can invite colleagues on the vendor's own email domain self-serve (`STAGE_2_VENDOR_PORTAL_SPEC.md` §11a), and every other address still falls through to that review. Flat in data capability, not in seat management: `profiles.seat_owner` gates invite/remove alone.
