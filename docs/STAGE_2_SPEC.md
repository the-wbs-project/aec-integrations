...
- **Maintenance marker: real `last_reviewed_at` + vendor-maintained branch** (AECI-616) — Stage 1 shipped the marker on `main` as attribution only (`Maintained by AEC Integrations.` on product detail, vendor detail, and the pair page). The date clause and the `Vendor-maintained.` branch exist in the component but are dormant, because **no column stores a real review timestamp**. Stage 2 adds `last_reviewed_at` + `maintained_by`, gates the write on an explicit "re-checked" signal in the promote payload, and lights up the vendor branch off real attestations. **Hard constraint: never source the date from `updated_at` / `created_at` / `promoted_at`, and never backfill it** — `updated_at` is `$onUpdate` and the promote ingest restamps it on every re-promote, so a bulk sweep would silently refresh the whole catalog's "freshness" without anyone re-checking a record.
