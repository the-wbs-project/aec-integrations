...
> **WC-5 update (AECI-319 / ADR 0020 §3).** With native Workers Cache enabled on the SSR Worker (WC-3), the zone-level HTTP `purge_cache` is **inert against the SSR cache** — `ctx.cache.purge()` is entrypoint-scoped, and no zone configuration touches Workers Cache. So the two API-Worker purge sites (**`POST /api/promote`** and **review moderation**) no longer call Cloudflare directly; they **enqueue** a typed `CachePurgeMessage` onto the **`aeci-cache-purge-{env}` Cloudflare Queue**, and a `queue()` consumer on the **SSR Worker** issues `ctx.cache.purge({ tags })` against its own cache. The SSR `POST /admin/purge` surface **(a)** stays on the HTTP transport until WC-6; datatool moves in WC-7; `callCloudflarePurge`/`CF_PURGE_API_TOKEN` are retired in WC-10. `CF_ZONE_ID` is kept (the AECI-262 WAF poll uses it).

The historical HTTP transport is a stateless `POST https://api.cloudflare.com/.../zones/{zone}/purge_cache` with a `{ tags }` body (≤ `CF_PURGE_MAX_TAGS` = 30 tags/call, Pro plan). It lives **once** in `@aeci/shared` (`callCloudflarePurge`, `packages/shared/src/cache-purge.ts`); the shared module also holds the queue message contract (`CachePurgeMessage`, `CACHE_PURGE_QUEUE_MAX_TAGS` = 1000). Call sites:
...
- **§5 Invalidation** → **Queue-based.** `ctx.cache.purge({ tags })` replaces the HTTP `callCloudflarePurge`. **WC-5 (AECI-319) landed** the promote + review-moderation producers and the SSR `queue()` consumer onto `aeci-cache-purge-{env}` (see §5); datatool (WC-7) and `/admin/purge` (WC-6) still follow. The ordered **refresh-stats-then-purge** home flow is preserved (recompute `home.*` stats, then enqueue the `index:home` purge). Today the consumer purges the SSR Worker's single default entrypoint; **WC-4 must relocate the purge into the `Renderer` entrypoint** once the gateway split lands (`ctx.cache.purge()` is entrypoint-scoped — ADR 0020 §2). `CF_PURGE_API_TOKEN` is retired (WC-10); **`CF_ZONE_ID` is kept** (AECI-262 WAF poll).
