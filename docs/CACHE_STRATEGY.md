...
- **New: Observability & local-dev** → **landed in WC-8/WC-9 (AECI-322/AECI-323):** `Cf-Cache-Status` (`HIT`/`MISS`/`EXPIRED`/`BYPASS`/…) is the primary HIT/MISS signal and edge HIT-rate lives on the **Cloudflare Workers observability dashboard**. The SSR render metrics only emit `cache_status:MISS`/`miss`/`non_cacheable` (a HIT skips the Worker), so the Datadog `cache hit rate < 70%` monitor + dashboard widget were **retired**. Wrangler 4.111.0 / Miniflare 4.20260710.0 is a confirmed local no-op for the native front cache (no `Cf-Cache-Status`/`Age`); exact `MISS → HIT` runs against each deployed PR preview in `e2e/edge-cache.spec.ts` (ADR 0020 Q3). See `docs/OBSERVABILITY.md`.
