...
>
> **WC-3 (AECI-317) has landed:** native `cache.enabled` is on the SSR Worker's **preview + staging** envs, and the hand-rolled `caches.default` match/put + `cacheKeyUrl` normalization are **removed** — so **§4a below is now historical** (the code it describes is gone; native cache keys on the full, order-sensitive query string until **WC-4** restores normalization via a gateway entrypoint). WC-3 also added `stale-while-revalidate` / `stale-if-error` to the detail + index/browse routes (§4). The HTTP `callCloudflarePurge` transport (§5) is **still in force** until WC-5/6/10. `demo`/`production` stay uncached until WC-4/5/6/8 land (ADR 0020 §1). The **full rewrite of this doc is tracked in WC-11 (AECI-325)**; §9 sketches the target shape.
...
> **⚠️ Historical as of WC-3 (AECI-317).** The `cacheKeyUrl()` function and the per-route `cacheKeyParams` allowlist described in this section were **removed** with the manual `caches.default` pipeline. Native Workers Cache keys on the **full, order-sensitive query string** — so today `utm_*`/`fbclid` fragment the cache and `?a=1&b=2` ≠ `?b=2&a=1`. **WC-4 (AECI-318)** restores this exact behavior — utm-strip, the per-route allowlist below, canonical param order, and the AECI-223 multi-select CSV sort — via a `cache.enabled:false` **gateway entrypoint** that computes the normalized URL and forwards it as `cf.cacheKey`. The allowlist and rationale below are retained as the spec WC-4 rebuilds against; until WC-4 ships, this fragmentation is a chief reason `demo`/`production` stay uncached.

The edge cache is keyed by URL. Historically the SSR Worker's `caches.default` lookup/write key was built by `cacheKeyUrl(url)` in `server-runtime.ts`, **not** from the raw request URL — otherwise marketing/tracking params (`utm_*`, `fbclid`, `gclid`, `ref`, …) on shared links would fragment the cache for pages that render identically, turning every variant into a MISS → SSR render → store.
...
**Value-level normalization for multi-select facets (AECI-223).** `cacheKeyUrl` canonicalizes param *order* (`searchParams.sort()`), but **not** the bytes of a value. The taxonomy facets are now **multi-select** — each dimension accepts a comma-separated UUID list in a single `{kind}_id` param (`category_id=a,b`), matched as `OR within the dimension, AND across dimensions`. Two selections in different click orders (`a,b` vs `b,a`) are the same filter but **distinct cache keys** and would also break SSR↔client HTTP-transfer-cache parity. The producer is therefore responsible for canonical value order: `aec-facet-sidebar` emits the ids **sorted** before writing the param (`facet-sidebar.ts` `onRefine`). The allowlist is unchanged (the param names already covered single-select); the sorted CSV is the load-bearing invariant. Any future writer of a list-valued cache-key param must sort it the same way.
...
The historical HTTP transport `callCloudflarePurge` — a stateless `POST https://api.cloudflare.com/.../zones/{zone}/purge_cache` with a `{ tags }` body (≤ `CF_PURGE_MAX_TAGS` = 30 tags/call, Pro plan) — still lives **once** in `@aeci/shared` (`packages/shared/src/cache-purge.ts`), but since WC-7 backs only the `retract-product` CLI (datatool moved to the queue — see (c)); the shared module also holds the queue message contract (`CachePurgeMessage`, `CACHE_PURGE_QUEUE_MAX_TAGS` = 1000). `CF_PURGE_API_TOKEN` is retired in WC-10; `CF_ZONE_ID` is kept (the AECI-262 WAF poll uses it). Call sites:
...
- **§5 Invalidation** → **Native, split by locality.** `ctx.cache.purge({ tags })` replaces the HTTP `callCloudflarePurge`. **Cross-Worker** producers on the API Worker (`POST /api/promote`, review moderation) enqueue onto `aeci-cache-purge-{env}` and the SSR `queue()` consumer issues the purge — **WC-5 (AECI-319) landed** this. **`/admin/purge` is the exception:** it already runs on the SSR Worker, so **WC-6 (AECI-320) migrated it to call `ctx.cache.purge()` in-process** — no queue hop. Datatool (WC-7 / AECI-321) also **enqueues** — a `{ purgeEverything: true, source: 'datatool' }` message onto the target tier's queue per copy/seed/reindex — **WC-7 landed**. The ordered **refresh-stats-then-purge** home flow is preserved (recompute `home.*` stats, then enqueue the `index:home` purge). Both purge paths are entrypoint-scoped; today they run on the SSR Worker's single default entrypoint, and **WC-4 must relocate them into the `Renderer` entrypoint** once the gateway split lands (ADR 0020 §2). `CF_PURGE_API_TOKEN` is retired (WC-10); **`CF_ZONE_ID` is kept** (AECI-262 WAF poll).
