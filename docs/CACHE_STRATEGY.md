...
| `trade:{slug}` | Trade browse page (`/trades/:slug`, AECI-538). Emitted for **every** trade, published or not — the publication gate (`STAGE_1_SPEC.md` §5.5a) controls indexability, not cacheability, and an unpublished page still renders and still needs purging when its product set changes. **Purge caveat:** because the `/trades` index and the primary-nav flyout list *published* terms only (the facet sidebar is exempt — its counts are scoped, see `TRADES_VOCABULARY.md` §6), a promote that pushes a term across (or back under) the `TRADE_PUBLISH_MIN_PRODUCTS` floor changes those surfaces too — so **any write that changes a product's trade set** must purge `index:trades`, `taxonomy`, and `sitemap` alongside `trade:{slug}` (AECI-542/546). "Touching" includes a **removal**, which can push a term back under the floor. **There are two such writers, and they must stay in lockstep:** (1) `POST /api/promote` — the response echoes only the trades that were *set*, so `promote.ts` reads the product's prior trades before the batch and passes them to `cacheTagsForPromote` as `removedTradeSlugs`; and (2) `PATCH /api/vendor/products/:id` (AECI-665), where a vendor assigns their own trades — `productEditTags` (`routes/vendor.ts`) already holds both sides and keys the purge on the **symmetric difference** of the before/after sets, so re-saving an unchanged set purges nothing (tighter than promote's trigger, which cannot diff; an unchanged trade's browse page is already covered by the `product:{slug}` tag it embeds). (The three sibling facets have no equivalent rule: without a publication gate, a change only staleness-affects that one browse page, which the 5-minute TTL covers.) **Both writers announce a newly-published trade URL since AECI-944.** This row used to read "Only promote submits newly-published trade URLs to IndexNow/Google; a vendor edit purges the edge but does not ping a crawler." That was true while no vendor held a seat. `PATCH /api/vendor/products/:id` now buffers the same published trade URLs into `indexnow_queue`, and queues them for Google at tier 3 in `gsc_recrawl_queue` (`DATABASE_SCHEMA.md` §9.8). It resolves the publication floor post-commit exactly as promote does, so a sub-floor `noindex` trade page is still never announced. |
...
4. **A connector's product page tags its powered edges — and a promote purges it** (Stage 1.5 Addendum B, `STAGE_1_5_SPEC.md` §12.4). A `product_role: 'connector'` detail page renders the integrations it **powers** (`integrations.powered_by_product_id`), where it is the mechanism rather than an endpoint. Two halves, both required: **(a)** the resolver pushes `integration:{id}` plus **both** endpoint `product:{slug}` tags per powered edge — this product is neither endpoint, so both are genuinely embedded; **(b)** the promote ingest emits **`product:{poweredBySlug}`** from each integration result that names a powered-by product (`promote-cache-tags.ts`, fed by `PromoteIntegrationResult.poweredBySlug`). Rule (b) exists because no other rule reaches the connector: it is not the promoted product, not an endpoint, and not the pair. Without it a promote touching an Agave-powered edge leaves `/products/agave-erp-sync` stale until TTL. **Bounded gap:** re-pointing an edge to a different connector purges only the **new** one — the response carries the post-update slug — the same shape as the endpoint-move gap. Since AECI-730 the response also carries the **preserved** connector's slug: when a payload names a connector that doesn't resolve, the ingest leaves `powered_by_product_id` as it was rather than nulling it, and reads the stored value off the pre-update row so rule (b) still purges the connector whose hub actually renders that edge.

4a. **A promote that re-points an ENDPOINT purges the pair page the edge left** (AECI-953, `STAGE_1_5_SPEC.md` §7.2a). This is the endpoint-move half of rule 4's bounded gap, and it is now **closed**.

   Everything else in this file derives its pair and product tags from the **post-update** endpoints, because that is all the response used to carry. So an edge moving from `procore-project-management` to `procore` purged `pair:okta__procore` and left `pair:okta__procore-project-management` serving a cached copy of an edge it no longer holds, until TTL. AECI-726 did that to 37 live URLs at once.

   `PromoteIntegrationResult.movedFromSlugs` now carries the two endpoint slugs the edge left, and `cacheTagsForPromote` emits **`pair:{min}__{max}` for the old pair plus `product:{slug}` for both old endpoints**. No new vocabulary — only the emission.

   Three notes:

   - **`movedFromSlugs` is a purge target, not state.** Nothing stores it. The durable half of the same event is the `integration_endpoint_moves` row the ingest writes in the same batch, which is what makes the old URL **301** rather than merely repaint. Without this purge the 301 would be invisible behind a cached 200.
   - **Both old endpoints get a `product:` tag**, not just the one that changed. The product the edge moved away from lost a row from its own integrations table and is otherwise unreachable here: it is no longer an endpoint, and on a promote of the *other* endpoint it is not `response.product` either.
   - **Nothing fires on a no-op or a swap.** The ingest compares the two endpoints as an **unordered** pair, so a re-promote that restates them — or one that merely swaps source and target, which AECI-920's direction corrections do in bulk — writes no move row and echoes no slugs.

5. **A connector-catalogue sync purges the endpoint pages whose reach line moved** (AECI-892, `STAGE_1_5_SPEC.md` §13.7). Until 2026-09-14 the connector-lane tables had **no** tag vocabulary at all and that was deliberate (AECI-714): `connector_catalogs` / `_catalog_surfaces` / `_stubs` / `_stub_mappings` / `_pairs` / `_evidenced_pairs` are written only by `POST /api/promote/connector-catalog`, and no cacheable route's output depended on them **yet**. That was a positive statement rather than an omission, with the obligation stated as transferring to whichever issue first rendered them. §13.7's *"N more pairs reachable via connectors"* line is that first public reader, so the transfer has happened and this is it.

   The tag set is exactly what the parked rule named: **`product:{connectorSlug}` plus `product:{slug}` for each endpoint whose reachable count moved**. No new vocabulary — both tags already exist — only the emission, in `cacheTagsForConnectorPage` (`apps/api/src/routes/promote-cache-tags.ts`) and `dispatchConnectorHooks` (`apps/api/src/routes/promote-connector.ts`), which is now three hooks rather than two.

   Four rules come with it, and each is a way the emission could be wrong while still enqueueing something:

   - **Never `pair:{a}__{b}`.** §13.7 forbids enumerating reachable pairs at all, so no pair page renders one and the tag would purge a page whose content did not move. The *delivered* tier keeps that tag (rule 4 above); the reachable tier has no URL to name.
   - **Never `sitemap`.** Reachable pairs create no URLs. A 30-page sync of ~3,573 stubs would otherwise repaint `sitemap.xml` thirty times for nothing.
   - **A page that writes nothing purges nothing.** The slug set is collected where statements are emitted, so an unchanged re-sync enqueues no message at all. Without that, the nightly six-catalogue sync repaints the whole catalog.
   ...
   **A second producer since AECI-724: the mapping edit.** `PATCH /api/admin/connector-stub-mappings/:id` and `PATCH /api/vendor/connector-stub-mappings/:id` write one mapping on a vendor-managed catalogue, and an edited `mapped` row clears the provenance gate. When the row was or becomes publishable, the handler enqueues the same tag shape post-commit: `product:{slug}` for the product losing the row, the product gaining it, and the connector (`apps/api/src/lib/connector-mapping-edit.ts`). An edit publishable on neither side purges nothing. The bounded gap below applies to it unchanged.
...
6. **A retired-slug 301 is tagged on the SURVIVOR** (AECI-978, `STAGE_3_SPEC.md` §2.6 option B). `slug_redirects` maps a retired product/vendor slug to the one that replaced it, and the detail resolver emits a 301 on its not-found branch rather than a 404.
   ...
   The redirect carries **both** slugs — `{entity}:{from_slug},{entity}:{to_slug}` — because each answers a different way it goes wrong:

   - **`{to_slug}`, the survivor.** A rename or a further retirement of the destination makes this 301 point somewhere wrong, and that write already purges `{entity}:{to_slug}` through the ordinary rules.
   - **`{from_slug}`, the retired slug — and it is NOT redundant.** The obvious reasoning is that a deleted row has no writer, so nothing would ever emit its tag. That is wrong in the one case that matters: the row **comes back**. A re-promote of that slug purges `product:{from_slug}` and nothing else, and without this tag the edge keeps redirecting readers away from a page that is live again for the full 24h `s-maxage` — while the resolver itself has already started serving the page, because a live row beats a mapping. AECI-953 tags the **old** pair for exactly this reason.
   - **Editing the map itself is an operator action with no automatic purge.** Changing a `slug_redirects` row purges nothing — there is no writer to hook. Follow it with `POST /admin/purge` on either tag, or wait out the 24h `s-maxage`.
   ...
   **Since AECI-991 the map also rewrites the PAIR route's path prefix.** `/products/{from_slug}/integrations/{x}` 301s to `/products/{to_slug}/integrations/{x}` on the pair route's own not-found branch (`STAGE_1_5_SPEC.md` §7.2b). A merge-then-retire had been leaving every pair page under the retired slug on a 404 while the product page itself redirected correctly — AECI-809 did that to 44 indexed Autodesk Construction Cloud pair URLs.

   That redirect carries **four** tags rather than two, because the URL it names is a pair page and not a detail page:

   - **`pair:{old_min}__{old_max}`** — the retired slug COMES BACK. Same argument as `{from_slug}` above: a re-promote of that slug purges its own pair tags and nothing else, and without this the edge keeps redirecting readers away from a page that is live again.
   - **`pair:{new_min}__{new_max}`** — the destination pair changes or empties, so the 301 stops being the right answer.
   - **`product:{from_slug}` and `product:{to_slug}`**, for each URL slug that actually changed. An unmapped endpoint's product page has nothing to do with this redirect and gets no tag.

   The `Cache-Control` is the same `public, max-age=3600, s-maxage=86400`, set the same way and for the same reason.

   **No discovery channel advertises a `from_slug`.** Three surfaces, each filtering at its own choke point: `sitemap.xml` excludes them from the product and vendor loops; the **IndexNow drain** drops a buffered URL whose path now only redirects, which is the last gate before the wire and the only one that catches a URL buffered by a promote *before* the mapping was seeded; and **`gsc_recrawl_queue`** filters at ENQUEUE rather than at the operator's read, because its rows persist until cleared by hand and hiding one at read time would leave it disagreeing with the `/admin` count badge. That last one has a bounded residual — a URL queued before its mapping was seeded stays on the list — which the operator can clear, that being the whole interaction model of the queue.
...
| Home (`/`), `/about`, `/methodology`, `/updates`, `/roadmap`, `/legal/*` | none — strip all |
...
**`page` now has a second consumer, and it runs the rule in reverse (AECI-803).** Since the paginated listings self-canonicalise, `page` is an input to `<link rel="canonical">` as well as to the cache key — the canonical is *baked into* the cached HTML, so removing `page` from `LISTING_CACHE_KEY_PARAMS` would not merely serve page 2's rows for page 3, it would serve page 2's **canonical** to every page of the series. The coupling is directional and worth stating both ways: a param in `CANONICAL_QUERY_ALLOWLIST` (`apps/web/src/app/core/meta.helpers.ts`) **must** be in this allowlist, but not the reverse — `sort`, `view` and the facet ids are cache-key params that stay out of the canonical on purpose (`STAGE_1_PHASE_2_SPEC.md` §9.1a). `utm_*` is in neither, which is exactly why `?utm_source=x&page=2` may share an entry with `?page=2`: they emit the same canonical.
...
### 4a.1 The gateway MERGES into `request.cf`; it must never replace it (AECI-868)

**A `cf` object supplied to `Fetcher.fetch` replaces the incoming `request.cf` wholesale on the loopback. The runtime does not merge it.** So the gateway's GET/HEAD branch is:

```ts
const cf = { ...(request as { cf?: CfLike }).cf, cacheKey: cacheKeyFor(new URL(request.url)) };
return Renderer.fetch(request, { cf });
```

**Why this is a cache-doc rule and not a page-views one.** `request.cf` is the *only* source of the eyeball's network context inside `Renderer`. `applyCfContextHeaders()` (`server-runtime.ts`) reads `cf.asn` / `cf.country` / `cf.colo` / `cf.asOrganization` / `cf.tlsVersion` / `cf.httpProtocol` off it and copies them onto the trusted `x-aeci-cf-*` headers, because `request.cf` does not survive the `env.API` service binding either. The API's `POST /api/page-views` then stores them as `cf_asn` / `cf_country` / `cf_colo` / `cf_as_organization` / `tls_version` / `http_protocol`. **A gateway that replaces `cf` therefore nulls six `page_views` columns on every full-document arrival, and the only code that says so lives here.** Anyone adding a second `cf` field to the loopback — `cacheTtl`, `cacheEverything`, a future key input — will be editing this line, which is why the rule is stated next to the mechanism rather than in the analytics docs.

**The degraded window: 2026-09-07 → the AECI-868 fix (production).** WC-4 shipped the replacing form on 2026-07-19 (`1b0db1aa`, `stage-2`), so **staging and preview lost arrival metadata from that date**; production only ran the gateway from the Stage 2 promote on `2026-09-07T00:44Z`, and the last arrival carrying an ASN is timestamped `00:44:50Z`. Production coverage went **1,458 of 1,458 arrivals with a `cf_asn` on Sep 6 → 0 of 2,633 on Sep 8**. `POST` took the no-override branch throughout, so the browser tracker's `navigation = 'spa'` rows kept their metadata — which is exactly why the outage read as "fewer bots detected" rather than as a failure. **Those days' columns are not reconstructable from D1.** `ADMIN_PANEL_SPEC.md` §9.8 and `ANALYTICS_BASELINE.md` carry the consequences for anyone reading the numbers.
...
- Emits a structured log plus `aeci.cache.purge{source,outcome,mode}` (outcomes: `ok` / `failed` / `skipped`).
...
- **Integration retire and restore** (`POST /api/vendor/integrations/:id/retire` /
  ...
  integration counts carry the `taxonomy` tag), and the sitemap (the pair URL leaves the
...
- `Content-Security-Policy` — **defined and first emitted in AECI-89.** (Earlier drafts of this section and `STAGE_1_PHASE_2_SPEC.md` §8.6 called the CSP "unchanged / existing from Phase 1," but no Phase 1 CSP was ever implemented — AECI-89 closes that gap.) The policy is a static, cache-safe string assembled in `apps/web/src/server/seo-headers.ts` and applied via `withCacheHeaders`. Nonces/hashes are deliberately **not** used: the HTML is edge-cached and served byte-identically to every visitor, so a cached nonce would be reused by all (defeating it), and Angular's `withEventReplay()` injects a version-generated inline script a hash allowlist would have to chase across upgrades. The directives:
  - `default-src 'self'`
  - `script-src 'self' 'unsafe-inline' https://static.cloudflareinsights.com` — the `index.html` theme bootstrap, the injected Algolia / Supabase / PostHog bootstraps, and Angular's event-replay inline script. The host entry allowlists the Cloudflare Web Analytics beacon (`beacon.min.js`) that Cloudflare auto-injects at the edge for the zone
  ...
  - `img-src 'self' data: https:` — vendor `logo_url`s promoted from the review app come from arbitrary https origins
  - `connect-src 'self' https://*.algolia.net https://*.algolianet.com https://cloudflareinsights.com https://us.i.posthog.com https://us-assets.i.posthog.com https://<supabase-ref>.supabase.co` — the `/api/*` proxy, the Algolia search origins, the Cloudflare Web Analytics report host, the two PostHog US hosts (ingestion + assets), and the shared Supabase auth project. The Algolia origins were added in **AECI-136** (Phase 3.4) for InstantSearch: the browser client resolves its query host as `{appId}-dsn.algolia.net` with `{appId}-{1,2,3}.algolianet.com` retry fallbacks, so the two wildcards cover every search XHR. `https://cloudflareinsights.com` is where the Cloudflare Web Analytics beacon POSTs its RUM payload (`/cdn-cgi/rum`); it pairs with the `static.cloudflareinsights.com` entry on `script-src`. PostHog is initialised with `disable_external_dependency_loading: true`, so it never injects a remote `<script>` and `script-src` stays untouched. (Two `browser-intake-*` Datadog RUM hosts lived here until **AECI-651** removed the Datadog leg; **AECI-162** had caught a missing US5 host that CSP-blocked RUM in every deployed env — worth remembering if a vendor host is ever added back.) The authoritative list is `CSP_DIRECTIVES` in `apps/web/src/server/seo-headers.ts`.
...
**HIT/MISS signal.** Under native Workers Cache a HIT **skips the SSR Worker**, so there is no per-HIT Worker log to count. The primary HIT/MISS signal is the **`Cf-Cache-Status`** response header (`HIT` / `MISS` / `EXPIRED` / `BYPASS` / `DYNAMIC` / `REVALIDATED` / `UPDATING` / `STALE`), and edge HIT-rate lives on the **Cloudflare Workers observability dashboard**. The SSR render metrics only ever emit `cache_status:MISS` / `miss` / `non_cacheable` (a HIT never reaches them), so the **`cache hit rate < 70%` monitor + dashboard widget were retired in WC-8 (AECI-322)** — an in-Worker hit-rate is structurally unmeasurable when HITs never run the Worker. See `docs/OBSERVABILITY.md` and `docs/RUNBOOKS.md` ("Low cache hit rate").

**Purge observability.** Every purge — in-process (`/admin/purge`), queue-consumer (promote / moderation / datatool), or a no-op on an uncached tier — emits `aeci.cache.purge{source,outcome,mode}` (§5). That metric is the signal that invalidation is firing and which outcome it took (`ok` / `purge_failed` / `no_cache` / `noop` / `skipped`).
...
## Logo assets (AECI-955)

`GET /api/logos/:key` returns original validated raster bytes with `Cache-Control: public, max-age=31536000, immutable`, SHA-256 ETag, detected image Content-Type, `X-Content-Type-Options: nosniff` and `Content-Security-Policy: default-src 'none'; sandbox`. The SSR `/api/*` passthrough preserves the body and headers. Invalid keys and missing/corrupt objects return non-cacheable errors. Immutable objects need no Cache-Tag invalidation.

**What `immutable` actually buys today: browser caching, not edge caching.** Cloudflare's default cache-everything rules key off file extension, and `/api/logos/<sha256>` has none, so the zone treats it as dynamic and passes every request to the API Worker. That means the Worker re-reads the object from R2 and re-runs `validateLogo` on every logo view. This is deliberate for correctness — the response `Content-Type` is derived from the bytes, never from R2 object metadata (ADR 0032) — but it is a per-view cost that a Cache Rule would remove. **Open operator item:** add a Cache Rule matching `starts_with(http.request.uri.path, "/api/logos/")` with Eligible for cache + Respect origin TTL. It is safe at any time because the key is the content hash, so a cached entry can never go stale. Catalog logo saves purge vendor:{slug}, or product:{slug} plus index:products, post-commit. Product pages already carry their vendor tag. Upload alone changes no page. New logo URLs remain compatible with the page img-src self/HTTPS policy.
