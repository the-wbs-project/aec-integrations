...
>
> **WC-3 (AECI-317) + WC-4 (AECI-318) have landed:** native `cache.enabled` is on the SSR Worker's **preview + staging** envs. The hand-rolled `caches.default` match/put is **removed**; key normalization (utm-strip / per-route allowlist / canonical order / multi-select CSV sort) is **restored** — WC-4 rebuilt it as `cacheKeyFor()` behind a two-entrypoint **gateway** (`default`, cache off) → cached **`Renderer`** pair (§4a, ADR 0020 §2). WC-3 also added `stale-while-revalidate` / `stale-if-error` to the detail + index/browse routes (§4). The HTTP `callCloudflarePurge` transport (§5) was **retired in WC-10 (AECI-324)** — all purge paths are now native `ctx.cache.purge()` / queue (WC-5/6/7). `demo`/`production` stay uncached until WC-8 lands (ADR 0020 §1). The **full rewrite of this doc is tracked in WC-11 (AECI-325)**; §9 sketches the target shape.
...
- **§8 Cross-references** → add ADR 0020 and the WC-* issues; the references to the HTTP purge transport were retired in WC-10 (AECI-324).
