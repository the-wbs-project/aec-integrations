...
- **The timeline is the ONE endpoint in the system that omits `liveAttestationsWhere`.** It reads
  two configs: `integrationTimelineConfig`, and its evidenced-pair twin
  `connectorEvidencedPairTimelineConfig`, added by AECI-1035 so claims on a connector-evidenced pair
  have a history too. Retracted rows are the point. Their doc comment says so loudly and
...
| `POST /api/vendor/contests/:id/decision` with `accept` (AECI-1008) | `integrations`, or `connector_evidenced_pairs` since AECI-1092 | folded into the field write; one `integration.updated` row (on a pair, `connector_evidenced_pair.updated` with entity type `connector_evidenced_pair`) with before/after, `metadata.reason = 'contest-accepted'`. On a connector-powered row it needs an active entitlement (`STAGE_2_VENDOR_PORTAL_SPEC.md` §11b.13) |
