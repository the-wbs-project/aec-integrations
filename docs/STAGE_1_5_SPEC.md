...
> **Amended 2026-09-14 by AECI-921 — the mechanism ROW uses this vocabulary too.**
>
> `integrations.direction` was `one-way | bidirectional`, and §7.1 below described a second,
> parallel translation for it. There is now only one. `one-way` became `a_to_b` and `bidirectional`
> became `both` (migration `0034`, a pure re-spelling that changed no row's meaning), so
> `integrationDirectionForContext` is a null-tolerant delegate to `claimDirectionForContext` rather
> than a second implementation of the same table above.
>
...
> change is what gives that correction somewhere to land.
>
> **One spelling survives, and only outside this section.** `one-way | bidirectional` is now a
> **context-free presentation** vocabulary, used where a list has no context product to frame an
> arrow against — the home page's recent-integrations tile, the Algolia `direction` facet, and the
> `?direction=` filter on `GET /api/integrations`. `presentedDirection()` in `integration-context.ts`
> is the only bridge to it and is deliberately lossy. So there are now **three** representations,
> not two: **stored** (`a_to_b`…), **context-relative** (`outbound`…), and **context-free**
> (`one-way`…). Keep all three distinct.
...
- **The arrow vocabulary is closed (AECI-919, 2026-09-14).** On the pair page an arrow glyph means
  **data-flow direction and nothing else**, and every arrow is framed to the context product. Three
  consequences, all shipped:
  - **The rail separator is a hairline rule, not a glyph.** It rendered a hardcoded `⇄` at
    `text-3xl` between the two logos — the same character `directionGlyph('both')` emits into a
    bidirectional mechanism's lane, and louder than it. Two meanings, one character, one page. `+`
  ...
  - **The mechanism card `h2` never carries a directional pair title.** `mechanism_name` on
    ...
    an arrow, 7 have no `mechanism_name` at all (those keep their name — "Autodesk Revit export"
    survives, "AviontéBOLD ↔ Adobe Acrobat Sign" does not), and 5 carry an arrow inside
    `mechanism_name` itself, which is the curator's own label and renders verbatim.
  - **A navigation affordance is a chevron.** See §13.3a for the product-detail table row.

  The binding design rule is `DESIGN.md` §"Named Rules" → **The Arrow Rule**.
...
#### 7.2a The moved-pair 301 (AECI-953, 2026-09-15)

**A promote that re-points an endpoint moves the pair page's URL, and the old one now 301s to the new one.** Before this it served 200 + `noindex` with no redirect — the indexed page decayed over weeks while the new one started from nothing.

The cause is structural rather than a defect in the promote: an edge's identity is its id, which a re-point preserves and updates in place, but a pair page is keyed by the **two product slugs** in its URL. [AECI-726](https://linear.app/aec-integrations/issue/AECI-726) moved 37 live edges off `procore-project-management` onto the new `procore` platform record on 2026-09-14 and [AECI-950](https://linear.app/aec-integrations/issue/AECI-950) moved 15 more, so 52 URLs on the catalog's most-linked product went quiet at once.

**This is not the slug-alias problem.** A renamed product is a different mechanism and stays with `STAGE_3_SPEC.md` §2.6, whose option survey is still open. Here both slugs are unchanged; the edge is what moved between them.
...
**Orientation is preserved, not canonicalised.** The endpoint that did not move stays in the reader's frame, so `/products/procore-project-management/integrations/okta` lands on `/products/procore/integrations/okta` rather than flipping to the alphabetical context. §7.1's alphabetical rule is a `<link rel=canonical>` concern and the destination page applies it itself; a redirect that also changed the frame would read as two changes.

**The 301 is emitted from the resolver via `RESPONSE_INIT`, not via Angular's `RedirectCommand`.** `@angular/ssr`'s `createRedirectResponse` unconditionally sets `Vary: X-Forwarded-Prefix`; `CLAUDE.md` permits no `Vary` value but `Accept-Language`, and the SSR Worker's `applySeoHeaders` — the thing that strips a forbidden one — runs on 2xx and 404 only. The Worker drops the rendered body on any 3xx, so the redirect ships bodyless. On a client-side navigation the equivalent is a `replaceUrl` navigation, so Back does not land on the page that just redirected.

**Cache.** The 301 carries its own `public, max-age=3600, s-maxage=86400` — the same directive the SSR Worker's other permanent redirects use — plus the **old** pair's `pair:{min}__{max}` tag. `withCacheHeaders` applies neither a route TTL nor a path-derived tag to a 3xx, so without them the mapping would be `private, no-store` with no purge handle. The promote purges that same tag, which is how the old page stops serving its cached 200. See `CACHE_STRATEGY.md` §2 and §3 rule 4.

**The 52 pre-existing moves** predate the ingest change and are seeded once by `scripts/ops/2026-09-pair-endpoint-move-backfill/`, keyed on pair-URL slug triples rather than edge ids (the Linear comments name upstream `rec…` ids, which are not app-DB ids).
...
**Three changes, and each is load-bearing on its own:**
...
**What still 404s, deliberately.** A pair whose endpoint was retracted with **no** `slug_redirects` mapping — an ordinary retraction rather than a merge — has no successor page to name, and inventing one would be worse than the 404. Seeding the mapping is the operator action that turns it into a 301, and it can be seeded before or after the retraction: a live row always wins.
...
- **The empty band has two copies, and the Layer-A arrow picks between them (AECI-919).** The band
  ...
  renders, which is the original defect inverted. Basic is excluded deliberately: it hides the
  standalone arrow, so "below" would name nothing there and the original copy is the honest one.
...
- **Promote (`promote-cache-tags.ts`):** `PromoteIntegrationResult` gains optional
  ...
  the new connector (the response carries the post-update slug), same shape as the endpoint-move gap.
  ✅ **The endpoint-move half of that gap closed 2026-09-15 (AECI-953, §7.2a).** The response now
  also carries `movedFromSlugs` — the two endpoint slugs the edge left — and the deriver emits the
  OLD `pair:{min}__{max}` plus a `product:` tag for each. The connector half above is unchanged and
  still bounded.
...
objects" chip, at the kind badge's weight, in Basic and Detailed views. This section's rows show the
...
> **Amended 2026-09-14 by AECI-919 — the row's trailing affordance is a chevron, not `→`.** Putting
> direction on the meta line above gave the row **two arrow glyphs**: a context-relative direction
> `→` in the first cell, and a decorative navigation `→` in the third. Same character, one row, two
> unrelated meanings — an artefact of folding the column, since before the fold the two arrows sat in
> different cells with a labelled `<th>` between them. The trailing cell is now a hand-inlined Lucide
> `chevron-right` (`aria-hidden`, `rtl:-scale-x-100`), which is the site's existing "there is more
