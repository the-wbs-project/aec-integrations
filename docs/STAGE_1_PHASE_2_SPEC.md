...
- `DetailLayout` — full-width hero (name, vendor, key facts) above a two-column body + right-column metadata. The two-column split is **`lg:` and up** (`xl:` until AECI-853, 2026-09-10 — see `STAGE_1_5_SPEC.md` §13.3a for the arithmetic); below that the page is single-column, so the body's data tables keep the full content width. Single-column pages do **not** push the metadata to the foot: the body is split across a `body-lead` slot (About) and `body`, and the undocked sidebar renders between them, reformatted as a contained fact panel. The sticky in-page nav sits in its own `nav` slot, projected unwrapped above the grid so it stays pinned for the whole page (see `DESIGN.md` §"Layout shells")
