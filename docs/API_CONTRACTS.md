...
On a pair, `direction` is stored in the canonical A/B frame, with A in the source seat, and an omitted frame means the caller's own endpoint or A. The batch is the same list below, run against `connector_evidenced_pairs`, with `metadata.table: 'connector_evidenced_pairs'` on the audit row. The purge adds the connector's `product:` tag.
