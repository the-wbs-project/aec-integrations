...
scope:
  paths:
    ...
    # Added 2026-09-14 (AECI-945/AECI-944): the two search-engine re-crawl buffers.
    # Both tables' INSERTs are log-class under ADR 0022: derived from a write that
    # already audited, invisible on every public surface, regenerated by the next
    # write touching that page. `indexnow-queue.ts` is here retroactively; it has
    # been exempt since AECI-826 and was never listed, exactly like the three above.
    # `vendor-shared.ts` is the vendor-side appender (`bufferVendorRecrawl`); it
    # holds no domain write of its own, so listing it exempts only the buffering.
    - apps/api/src/lib/indexnow-queue.ts
    - apps/api/src/lib/gsc-recrawl-queue.ts
    - apps/api/src/routes/vendor-shared.ts
  categories:
    - Audit logging
    - Data integrity
  finding_matches:
...
**Justification.** `STAGE_1_SPEC.md` §26.1 formerly read as an absolute — "every write path … no state change without a corresponding audit entry" — and reviewers were correctly flagging every unaudited write against it. **ADR 0022 scopes that invariant to *domain state*.** Derived and log-class writes are exempt when they are all three of: computed entirely from data already in the database (or an append-only event / lead-capture log), invisible on every public surface, and reproducible by re-running the job. That covers `stats_cache` (the 07:00 home-stats job and the Algolia sync watermark), the denormalized product counters, `page_views`, `mailing_list` / `feedback` (already documented in `API_CONTRACTS.md` §6.9/§6.13), and `metrics_daily` / `job_runs` (`ADMIN_PANEL_SPEC.md` §7.1/§7.2, **shipped 2026-08-13** with AECI-581 and AECI-583). It also covers the two search-engine re-crawl buffers, `indexnow_queue` and `gsc_recrawl_queue` (`DATABASE_SCHEMA.md` §9.6/§9.8), whose appends are derived from a write that already audited. Observability for these is `job_runs` plus the emitted metrics (PostHog — ADR 0024).

**Two files a reader will expect here and will not find, both deliberate.**
`apps/api/src/routes/promote.ts` holds the promote-side appender (`bufferIndexNowAfterPromote`)
and is **not** listed, because it also holds the ingest's domain writes, which must audit. An
entry covering the file would suppress the finding that matters most in it.
`apps/api/src/routes/admin-reindex.ts` is not listed either: its `DELETE` **does** audit, per
row, and a missing row there is a real finding (`STAGE_1_SPEC.md` §26.1).

**This exemption is narrow, and two things fall outside it — keep flagging them.** (1) **Domain-state writes always audit, regardless of actor.** A cron or `actorType: 'system'` write that touches the catalog, users, reviews, claims, requests, or workflows is *not* exempt — the test is entity class, not actor class. The `*/15` reconciliation sweep mutating `vendor_requests` / `workflow_instances` in `apps/api/src/lib/linear.ts` is a real violation and has its own issue; do not treat this entry as covering it. (2) **Scheduled `DELETE`s always audit** — exactly one summary row per run (`action='retention.pruned'`, `metadata={table, cutoff, rowsDeleted}`) in the same batch as the delete. A retention cron that deletes without one is a finding. **And an *unscheduled* delete on one of these tables audits per row, not per run.** `DELETE /api/admin/reindex/:id` (AECI-946) clears one `gsc_recrawl_queue` row on an operator's click, which is the ordinary per-write invariant rather than the summary-row exception. Do not read "queue delete" as "summary row"; the discriminator is *scheduled*.
