...
| 6 | Monorepo lint clean; zero hardcoded color literals on new home components | ✅ | `pnpm lint` exit 0 (ESLint all packages + `check-logical-properties` + Prettier). Home components use only tokens (`text-(--text-primary)`, `bg-(--surface-raised)`, `text-(--accent-primary)`, `bg-(--accent-warm)`, …) — no hex/`rgb()`/raw tailwind palette classes. |
