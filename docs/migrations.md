...
**The alternative to a CHECK recreate: a hand-written COLUMN constraint on an added column (AECI-1005).** `0044_slippery_edwin_jarvis.sql` adds `integrations.claimed_at`, `origin` and `retired_at` as three plain `ADD COLUMN`s, and `origin` needed a closed vocabulary (`'aeci' | 'vendor'`). SQLite accepts a `CHECK` on `ALTER TABLE … ADD COLUMN` and verifies it against existing rows, so the constraint went into the migration by hand as `origin text DEFAULT 'aeci' NOT NULL CONSTRAINT "integrations_origin_check" CHECK (…)`. It is **deliberately absent from `schema.ts`**: a table-level `check()` there is what makes drizzle-kit render a recreate, and a recreate of `integrations` is the dangerous class above. Two costs come with it. drizzle-kit cannot see the constraint, so its snapshot does not record it; and the next recreate of `integrations` that drizzle-kit generates from `schema.ts` would silently drop it. `src/test/migration-0044.spec.ts` asserts the constraint is still in the live DDL, so that drop fails CI instead of shipping. Use this pattern for a new closed-vocabulary column on a table with cascade children; it does not help with narrowing a CHECK that already exists. **AECI-1046 used it a second time**: `0046_absurd_ozymandias.sql` adds `integrations.retired_by text CONSTRAINT "integrations_retired_by_check" CHECK ("retired_by" IN ('owner', 'aeci'))`. A NULL passes the CHECK, so existing rows need no value, and no backfill was written. `src/test/migration-0046.spec.ts` asserts both hand-written constraints are in the live DDL. **AECI-1088 used it on a second table**: `0049_majestic_mentallo.sql` adds the same four ownership columns to `connector_evidenced_pairs`, a cascade parent of `claims` and, through them, of `attestations`. It is four plain `ADD COLUMN`s, with `origin` carrying `CONSTRAINT "connector_evidenced_pairs_origin_check"` and `retired_by` carrying `CONSTRAINT "connector_evidenced_pairs_retired_by_check"`, both by hand and neither in `schema.ts`. drizzle-kit emitted no recreate for it. `src/test/migration-0049.spec.ts` is the tripwire: it asserts the file is four `ADD COLUMN`s, that a seeded pair keeps its claim and attestation across the migration, and that both constraints are in the live DDL.
