...
**The rule is not "always read `capabilities`" — it is: the client gates on whatever field the API gates on.** The point of §6.1 is that the enabled state of a control and the answer a write would get can never disagree, and the only way to guarantee that is to read the same bit the server asserts on. Since AECI-623 that is one field, because the API asserts on one:
