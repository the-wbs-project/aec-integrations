...
| **The CI liveness sweep** (`posthog-liveness-sweep.yml`, every 3 h) | Cron **absence**, across all fifteen crons — **already running** and worth reading during the dual-run | Anything about *why* a cron failed |
...
> including the data-quality run's full result set, which used to exist only in the 04:00
...
attestation-notify, entitlement-expiry, asn-registry, indexnow-drain, claim-stale-check, waf-poll
and the per-key half of home-stats. Several shipped after the Datadog monitors were written and
nobody went back. `indexnow-drain` was the last to join: AECI-826 wired only its liveness
heartbeat, and AECI-864 added `aeci.indexnow.drain` to the combined failure alert. That is the migration's largest single
...
**Tuning any threshold in this table obsoletes the stored `traffic.page_views_human_after_automation`
series, and nothing detects it (AECI-827 / ADR 0027).** The AECI-827 re-check converges
`metrics_daily` when a raw count moves, and recomputes the filtered key alongside it. A threshold
change moves `flagged(X)` for **every** stored day with no change to any raw count, so the re-check's
trigger set is empty, the pass does nothing, and the stored series silently mixes two definitions.
That key is snapshot-only and in `NOT_BACKFILLABLE`, so there is no repair tool at all —
`ops:backfill-metrics-daily` cannot reach it. So: **before changing a value here, decide what the
stored series means afterwards, and record the date in `POST_LAUNCH_HEALTH_REPORT.md` so a later
reader can see the seam.** The three raw `traffic.*` keys are unaffected; they do not read these
constants.
