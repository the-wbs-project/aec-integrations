...
| `GSC_RECRAWL_PAGE_SIZE` | `apps/api/src/lib/gsc-recrawl-queue.ts` | `200` | One worklist read. Sized for a screen a human works down, and far above any day's Request Indexing quota, so the operator cannot exhaust a page. Anything below the cut is by definition tier-4 work that was not going to be reached today. Raise it only if the screen starts truncating tier-1 and tier-2 rows, which would mean something far more wrong upstream |
