...
the one UX gap it found into AECI-1109 (step 14b).
...
| 9 | **Claimant.** `/vendor`, which goes to `/vendor/:slug/overview`. Calls `GET /api/vendor/me`. | Tour the tabs: overview, profile, products, messages, seats, and one product's integrations tab. | `/vendor/me` returns `200` with `entitlement.tier: "verified"` and all eight capabilities. The plan panel shows the active state and the "Vendor account active" badge. No tab 404s. A wrong `:vendorSlug` renders the 404 page, not a stack trace. | `/vendor/me` body. Screenshot of overview. | [your-seat] |
...
| 14b | **Colleague**, signed out. `/vendor/invite/:token`. Calls `GET /api/seat-invites/:token`, then `POST …/accept`. | Sign in by magic link as the invited address. Accept. | Signed out, the page bounces to login and back to the same path. The page reads "Join your team on AEC Integrations" and "Accept invite". Accepting makes the Colleague `vendor_admin` with `seat_owner = false`, audit `vendor_seat.invite_accepted`, and lands on `/vendor/:slug/overview`. Signed in as another address, the page says "This invite was sent to {email}. Sign out and sign back in with that address…" A redeemer who is an AECi admin, or already holds another vendor's seat, gets `409 GRANT_CONFLICT`, and the page today shows the generic "We couldn't load this invite". That is known gap AECI-1109, not a new finding. | Accept response. The overview screenshot as the Colleague. | [your-seat] |
...
|---|---|---|
| 1. Claiming your vendor listing | [claiming-your-profile] | 1, 2, 4, 6 |
| 2. Your seat | [your-seat] † | 5, 7–10, 11e, 14–16 |
| 3. Attesting an integration | [attesting-to-integrations] | 11a–11d |
| 4. Owning your integrations | [owning-an-integration] | 11a (the maintenance marker) |
...
| 6. Plans and the Verified badge | [plans-and-entitlements] | 12a–13b |

[claiming-your-profile]: https://www.aecintegrations.com/docs/vendors/claiming-your-profile
[your-seat]: https://www.aecintegrations.com/docs/vendors/your-seat
[attesting-to-integrations]: https://www.aecintegrations.com/docs/vendors/attesting-to-integrations
...
[plans-and-entitlements]: https://www.aecintegrations.com/docs/vendors/plans-and-entitlements
