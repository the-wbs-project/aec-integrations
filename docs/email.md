...
lands on that screen's Feedback inbox.
...
|---|---|---|
| **`renderEmailHtml` / `renderEmailText`** (house layout) | `claim-approved`, `claim-rejected`, `claim-submitted-alert`, `vendor-seat-invite`, `stuck-request-alert`, `stale-claim-ticket-alert`, `attestation-claim-denied`, `attestation-ops-alert`, `landing-signup`, `landing-feedback` | 10 |
...
blocks and **one** CTA, and it drops that template's sign-off. **The decision pair moves
together.** `claim-rejected` was migrated with no other reason than that
`claim-approved` had been: one claimant getting a branded approval and an unbranded
rejection reads as carelessness exactly where they are being told no. It is also the
worked example of a migration with **no** CTA, since the layout's button is optional and
a rejection has no action the §9 AC permits.
...
The one template left on `opsText` is the entitlement sweep's `entitlement-expiring-admin`.
Both lead-capture alerts have migrated.
