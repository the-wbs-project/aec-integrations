...
**Search wiring (AECI-145 / Phase 3.12).** The same jobs extend to the search/listing surfaces without any workflow change (both already run every `e2e/*.spec.ts` and `lhci autorun`): `/search` joins the Lighthouse collection under a dedicated **SEO-exempt, MISS-only-TTFB** class (it's `noindex` + `private, no-store`); the AECI-143 facet sidebar gets interaction E2E (`apps/web/e2e/facets.spec.ts`) plus a `cacheKeyUrl()` unit test that proves distinct facets → distinct cache entries. Live `/search` results aren't asserted in CI (no Algolia on `dev:bound`) — that flow self-skips to local/preview; see `TESTING_STRATEGY.md` §7.2/§8/§10.5.
