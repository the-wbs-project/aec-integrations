...
|---|---|---|
| `CLOUDFLARE_API_TOKEN` | Wrangler auth + queue provisioning. Scope: the **Workers Scripts edit** `wrangler deploy` requires, **`Account → Queues → Edit`** (ADR 0013 + ADR 0020 §3 — the deploy provisions + binds the scheduled-job queues **and the WC-5 `aeci-cache-purge-{env}` cross-Worker purge queue**; without it `wrangler queues create` and the consumer-binding deploy fail), **and — since AECI-955 — `Account → Workers R2 Storage → Edit`** (the API Worker binds `UPLOADS`, and `wrangler deploy` resolves every `r2_buckets` entry against the API *before* it uploads, so without this grant every tier's deploy fails outright). **Diagnosing that one: `Authentication error [code: 10000]` on `/r2/buckets/<name>` is the TOKEN, not a missing bucket.** The cheap disambiguator is `wrangler r2 bucket list`, which names no bucket at all — if that also returns 10000, the token cannot see R2 and creating buckets will not help. The former **`Zone.Cache Purge`** grant is no longer needed — the HTTP purge transport was retired in WC-10 (AECI-324) and native `ctx.cache.purge()` needs no token — so it can be dropped on the next rotation. Keep it as narrow as these need; issue a new token at the same scope and rotate rather than broadening reactively. | All |
