...
> | 4 WAF before the claim endpoint | ✅ AECI-659 (#619), applied to the production host set 2026-09-03. **Extended by AECI-773** (ADR 0026): the claim POST was always covered — `POST /api/requests/claim` matches Rule A's `/api/requests/` prefix — but the rest of the vendor-write surface was covered by nothing, because Pro's two rate-limit slots are both spent and there is no third. AECI-773 added an **in-Worker** per-actor layer beneath the WAF and **changed the zone not at all**. `docs/waf-rate-limits.md` §6. |
...
| **AECI-1106** (filed 2026-09-23, High) | Account erasure of a vendor's last seat handed nothing back either. **Fixed 2026-09-24 (PR #823):** `DELETE /api/account` now calls AECI-989's `planVendorHandback` in the erasure batch, and `planOwnerSeatLapse` when only banned seats remain (`STAGE_2_ATTESTATIONS_SPEC.md` §13.9). The AECI-531 `auth.users` skip is unchanged. | The follow-up AECI-989 named. It is the same seat-granting gate. |
