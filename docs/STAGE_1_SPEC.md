...
| `/methodology` | **Editorial methodology** — how the catalogue is built, what verification means, no pay-for-placement, corrections, who maintains it. Body is build-time-inlined Markdown (the `/legal/*` pattern). Indexable, and the **one non-legal static page in `sitemap.xml`** _(AECI-804; governed by `STAGE_2_5_SPEC.md` §7.1)_ | 24 hr edge |
| `/docs/vendors/:slug` | **Vendor guide** — six task pages for vendors (claim, seat, attest, own, contest, plans). Build-time-inlined Markdown through the docs manifest (`apps/web/src/app/docs/`), one explicit route per page. **Noindex in every env** and absent from `sitemap.xml` until the portal opens (AECI-1105) _(AECI-1104; governed by `STAGE_2_PRODUCT_DOCS_SPEC.md` §3–§5)_ | 24 hr edge |
...
**Every term ships a `description`, and it does two jobs.** It is the paragraph rendered under the heading on each browse page and, through `MetaService.setEntityMeta`, that page's meta description and `og:description`. It is also the tagging guidance: where two terms are adjacent, the description names the neighbour outright ("Clash detection and federation belong under BIM Coordination"). Categories, audiences and phases resolve **find-or-create** in promote, so an operator who picks the wrong one of a similar pair does not get an error, they get a second near-duplicate term holding a permanent public URL. Trades and data objects resolve find-only and cannot fail that way. Category / audience / phase descriptions were backfilled on 2026-09-14, closing the ADR 0008 "Follow-ups" deferral that had seeded them `NULL`; the three columns stay nullable, but a new term is expected to carry copy (`apps/api/seed/taxonomy.sql` header). Since AECI-915 the vendor portal's product taxonomy picker renders each term's description beside it as its own column, so the string reaches whoever is tagging as well as whoever is reading (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.3).
...
1. **The names must match the review app exactly.** Audiences resolve **find-or-create** in the promote flow (unlike trades, §5.5a). Since AECI-970 a label matches a term by its slug or by `slugify` of its name, so a label that matches neither — "Mechanical Engineers" → `mechanical-engineers` — mints a *second*, near-duplicate term rather than matching the seeded one. The review app's `Audiences` vocabulary must gain these six options under exactly these names.
...
**Deferred (not Stage 1):** integration-page JSON-LD (Phase 2 §9.2 → Stage 2 — ✅ **shipped there 2026-08-20 as AECI-518**, on the pair page that superseded `/integrations/:id`: `WebPage` + `about` + `BreadcrumbList`, contract in `STAGE_2_SPEC.md` §8.7); sitemap index/sub-sitemap split (AECI-63 — only needed beyond 50k URLs; now tracked as AECI-560 under the AECI-518 epic, still **not warranted** — see §20.1).
...
> **Static pages are mostly absent, and `/methodology` is the one exception (AECI-804).** The four `/legal/*` documents have always been listed. `/`, `/about`, `/updates` and `/roadmap` are **not** in the sitemap: the first three by omission, `/roadmap` deliberately, since it is `noindex` and advertising a noindexed URL is the same contradiction the trade publication gate exists to prevent. `/methodology` **is** listed (`changefreq: monthly`, `priority: 0.5`), because it is the page a quality rater or an answer engine is meant to find and cite, so discovery is its entire purpose. The noindexed `/docs/vendors/*` guide (AECI-1104) is absent too, until AECI-1105 opens the portal. `sitemap.spec.ts` pins both halves — the entry, and the continued absence of the others — so "add every static page" has to be a deliberate change rather than a drift.
...
>
> - **Nothing ages a Google row out, and nothing must.** `indexnow_queue` drops rows past `INDEXNOW_QUEUE_MAX_AGE_DAYS` (7) because a missed ping is recoverable from the sitemap. A row silently dropped from the Google worklist is work no human ever saw. There is no staleness sweep here and there must not be one.
> - **Both queues gate on `INDEXNOW_KEY` AND `PUBLIC_SITE_URL`.** That reuse looks wrong for a table with nothing to do with IndexNow and is deliberate. The API Worker has no `ALLOW_INDEXING` var, that one lives on the SSR Worker, and `INDEXNOW_KEY` is provisioned only where `ALLOW_INDEXING="true"`. It is therefore the only available "this environment is public and indexable" signal inside this Worker. A second var expressing the same fact would mean five more wrangler blocks to keep in step.
>
> **The operator surface is `/admin/reindex`** (AECI-946, `ADMIN_PANEL_SPEC.md` §5.11), an Operations-group screen with a nav badge, over `GET /api/admin/reindex` and `DELETE /api/admin/reindex/:id`. Done **deletes** the row rather than flagging it, so an empty screen means genuinely nothing is pending. That delete emits one `audit_log` row, `action='reindex.cleared'`, in the same `db.batch`, attributed to the admin. See §26.1 for why the scheduled-deletion summary-row allowance does not reach it.
>
> **New metric `aeci.gsc_recrawl.queued`**, tagged `source:promote` or `source:vendor` — both writers emit it, so split the series by `source` before reading it. `docs/OBSERVABILITY.md` carries the catalogue entry and the one behaviour change this causes to the existing IndexNow refusal alert.

> **Trade URLs are publication-gated (AECI-546).** `/trades/:slug` joins the submit set only when the term clears `TRADE_PUBLISH_MIN_PRODUCTS` (§5.5a) — pinging an indexing service for a page that serves `noindex` is the same correctness bug the "provision `INDEXNOW_KEY` only at launch" rule prevents. Because `affectedUrlsForPromote` is pure over the promote response (which carries no `product_count`), the handler resolves the floor with one grouped count **after** the batch commits (`apps/api/src/routes/promote-trade-publication.ts`) and hands the single result to both re-crawl arms (the IndexNow buffer, and since AECI-945 the Google worklist). The `/trades` **index** is submitted whenever any trade is touched at all, published or not: it renders live per-term counts and gains or loses a tile on a floor crossing, and trades are find-only so the "a term was created" trigger that covers the sibling index pages can never fire for them. This supersedes AECI-542's blanket exclusion, which deferred the decision here.
...
> that adds or materially changes public pages (`docs/environments.md` → "Request indexing by hand (Google)").
...
>
> **Half of that correction expired on 2026-09-14 (AECI-945 / AECI-946).** The Google push is still a human step and always
> will be. It is no longer **unrecorded**. The writes that need it now queue themselves into `gsc_recrawl_queue`, and the depth
> of that queue is a badge in the admin nav. So "working from memory after a promote" is replaced by "work the list until
> Google's quota stops you". The word that stays true is *undelegated*. Nothing does the Search Console step but a person, and
> nothing alerts when they stop. See `docs/RUNBOOKS.md` for why an unworked-queue alert was considered and declined.
>
...
> **Step 5 is the floor under step 4, and that is what lets step 4 fail open.** Every consumer in step 4 is best-effort and swallows its own error. A missed buffer costs discovery latency, not correctness, precisely because the sitemap's `<lastmod>` still moves. A vendor write moves it too, since every vendor write stamps `products.updated_at`.

> **The pipeline has two writers, not one (AECI-944).** It was written for `POST /api/promote`. Since vendors hold seats, `afterVendorWrite` (`apps/api/src/routes/vendor-shared.ts`) runs steps 3 and 4 for eight vendor-portal endpoints, and step 5 falls out of the write itself. Step 2 is the one consumer a vendor write does **not** run inline. Vendor edits reach the index through the nightly sync instead, which selects rows by `updated_at` window, so step 5 is what carries them. The hook lives on the shared tail rather than on each handler, which is the "one function call" property above holding up under a second caller.
...
- `indexnow_queue` (§20.2 / AECI-826) — the IndexNow submission buffer. Derived from the write that already audited, invisible on every public surface, and regenerated by the next write touching that page. Its **INSERTs** are exempt; its scheduled **DELETE** is not — see the exception below. *(Two appenders since AECI-944: the promote hook and `afterVendorWrite`. A second appender changes nothing here, because the test is the entity class of the row, not who wrote it.)*
- `gsc_recrawl_queue` (§20.2 / AECI-945) — the Google re-crawl worklist, on exactly the same grounds. **Its DELETE is the one row in this list that audits per row rather than per run**, and not through the scheduled-deletion exception below. It is an operator clicking Done on an admin screen, so the ordinary per-write rule reaches it directly: `action='reindex.cleared'`, `entity_type='gsc_recrawl_queue'`, in the same `db.batch` as the delete, attributed to the admin rather than to `'system'`.
...
Use `logBatchToPosthog` (`apps/api/src/posthog.ts`) for any caller with N related entries: the logs intake accepts an array, so N entries cost **one** request. Never loop `logToPosthog` over a collection, and never put two single-row forwards in a `Promise.all`. A request handler forwards through `forwardAuditBatch` (`apps/api/src/lib/moderation-forward.ts`): every audit row and transition the write committed, in one request, tagged with the handler's `source` (AECI-1112). That holds for writes that commit one or two rows today, so a handler that later gains rows cannot run past the connection limit. The promote paths (`routes/promote.ts`, `routes/promote-connector.ts`), `lib/attestation-notify.ts` and `routes/vendor-shared.ts` build their own batch with `logBatchToPosthog`. The entitlement-expiry cron collects its warned rows and forwards them once per sweep. Every transport must also release its response body — see the `CLAUDE.md` constraint, `packages/shared/src/response-drain.ts`, and ADR 0021's 2026-08-27 amendment.

**Implementation.** `forwardAuditLog()` (`packages/shared/src/audit-log.ts`) and `forwardWorkflowTransition()` (`packages/shared/src/workflow-transition.ts`) take an **injected forwarder**, so they were already transport-agnostic before this migration — re-targeting them was a wiring change at the injection site, not a rewrite of the audit path. The single-row forwarders survive only on paths that forward exactly one entry per invocation: `admin-connector-catalogs.ts`, `admin-reindex.ts`, the profile self-heal, the Linear sync transition and the retention-prune summary row. Each builds its own forwarder, which returns `undefined` when `POSTHOG_PROJECT_KEY` is unset. `forwardAuditBatch` needs no forwarder: `logBatchToPosthog` self-gates on the key.
