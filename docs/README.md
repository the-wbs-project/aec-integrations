...
| [`local-dev.md`](./local-dev.md) | Active | Local development: build/test commands, the SSR to API service binding, parallel Conductor workspaces (`dev:conductor` vs `dev:agent` ports, isolated wrangler registry, stale-`dist` rebuild, orphaned `workerd` recovery), version reporting, and the checked-in agent skills and MCP servers. Moved out of `CLAUDE.md` 2026-09-23. |
| [`local-tracing.md`](./local-tracing.md) | Active | Local dev tracing (AECI-548): the `wrangler dev` Local Explorer SQL endpoint, `spans`/`logs` schema, and debugging recipes. Dev-only and ephemeral — distinct from the PostHog pipes. |
...
| [`linear-issue-conventions.md`](./linear-issue-conventions.md) | Active | How an AECi Linear issue is written so an agent can resolve what governs it: the `**Spec section:**` anchor grammar and its `n/a` form, the three-repo title-prefix routing, the checked-in mirror of the team's Linear issue templates (AECI-601), and §6's close rules: every `Closes AECI-N` belongs in the PR description, and an epic closes only when every sub-issue is closed. |
