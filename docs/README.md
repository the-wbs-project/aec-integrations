...
| [`ADMIN_PANEL_SPEC.md`](./ADMIN_PANEL_SPEC.md) | Draft | Operator console over the data AECi already collects (traffic, audience, catalog, moderation, system health) — the consent-independent read surface for `page_views` plus a screen for the two cron digests. Scope + build order settled; timing undecided. |
