...
- [ ] **Deploy gates green on the target commit** — CI `deploy.yml` / `promote-to-prod.yml` passed; the dual SSR+API version gate (`scripts/verify-version.sh`: `/api/version` **and** `/_version` == target SHA) is satisfied on `www.aecintegrations.com`. (Pre-AECI-807 this gate ran against the internal `prod.aecintegrations.com`; that hostname is retired.)
...
3. [ ] **Merge the cutover PR, then run `promote-to-prod`** with the standard `COMMIT_SHA`/`DEPLOYED_AT` vars (CLAUDE.md version-reporting rule). This deploy applies steps 1–2 **and** reconciles the apex + `www` custom domains onto `aeci-web-production` — **it is the DNS flip** (see step 4). Before it, **verify on the tier's existing served host** — at the time of the cutover this was the internal `prod.aecintegrations.com`, which was unaffected by the apex move; that host has since been retired (AECI-807), so a future re-run of this step has no pre-flip hostname to use and must verify on demo instead: home, search, a detail page, `/legal/*`, `/about`, `/contact` all render; dual version gate green; `robots`/canonical now indexable.
...
- [ ] **IndexNow fired** — a promote (or the first crawl-worthy write) records `aeci.indexnow.submit{source:promote,outcome:ok}`; the `<key>.txt` file resolves at the root. **Absence of that metric does not mean the key is missing** (AECI-801): the hook is skipped with no emission both when `INDEXNOW_KEY`/`PUBLIC_SITE_URL` are absent and when the promote touched no public URL, so a zero series has three causes and only one is a defect (`docs/OBSERVABILITY.md`, metric catalogue). Check the secret at the store per §2; use this metric only to confirm a submission that did happen was accepted.
