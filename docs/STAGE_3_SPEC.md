...
The non-feature backlog that fits Stage 3's operating posture, batched loosely: **AECI-533** (drop the 19 review-app-only D1 columns), **AECI-602** (plan check → CI sticky comment, once the FP rate is known), **AECI-555** (re-evaluate Cloudflare CI on Workflows at Artifacts GA — spike, trigger-gated), **AECI-620** (slim CLAUDE.md; if not taken as the 2.5 rider), **AECI-621** (spec-grill skill — *useful for authoring the Stage 3 companion specs themselves; consider building it first*). The **AECI-637 temp-env decision** that used to sit on this list is **resolved and off it**: `stage-2` merged into `main` on 2026-09-03, so the `stage2` mirror lost the branch it mirrored, and it was torn down under **AECI-808** rather than being carried through Stage 2.1 and 2.5. **AECI-597** (hardcoded-color-literal lint rule) is likewise **off this list**: it was built on 2026-09-11 against `main` rather than waiting for Stage 3, because the scoping pass it was deferred for turned out to be the whole job — both false-positive classes AECI-549 measured are defeated by pattern, and the enforcement machinery already existed. See `ANGULAR_STYLE_GUIDE.md` §24. **AECI-590** (reverse-proxy PostHog) is **off this list too, closed rather than done**: its own AC said to measure the blocker-lost delta first and to close on the number if it did not justify the work, and on 2026-09-13 the number came back at about **16 real browser page loads a day**, making the recovery 2–5 a day. The measurement, the two corrections it forced, and the **named re-open trigger** (200+ `app_started` a day for a week, or the first outbound campaign) are in `ANALYTICS_BASELINE.md`.
