...
| 4 | `workflow_transitions` recorded for all three workflow types (`vendor_claim`, `correction_request`, `review_moderation`) | ✅ | `vendor_claim`/`correction_request`: genesis `requests.ts:277`, admin action `admin-requests.ts:382`, webhook `webhooks.ts:253`, site→Linear sync `linear.ts:518`. `review_moderation`: submit `reviews.ts:218`, admin moderation `admin-reviews.ts:296`. Bonus reversible `reviewer_ban`: `admin-reviewers.ts:261`. All via `workflowTransitionInsert` in a `db.batch`. |
...
| Linear issue creation on request submit (CF Worker, idempotent, `waitUntil`) + failure handling | AECI-211 | ✅ | `apps/api/src/lib/linear.ts:387` (`createLinearIssueForRequest` — "Vendor Requests" project, template, `Source URL` attachment, labels incl. `domain-check-pending`, idempotency guard `linear_issue_id IS NULL`, never throws); wired `requests.ts:295` via `ctx.waitUntil`. Metrics `aeci.linear.issue[.duration_ms]`. |
...
Required secrets for the live run: `LINEAR_API_KEY`, `LINEAR_WEBHOOK_SIGNING_SECRET`, `RESEND_API_KEY` + `ADMIN_ALERT_EMAIL`, and a Linear webhook registered at `POST /api/webhooks/linear`.

> **AECI-851 changed where this run can happen.** `LINEAR_API_KEY` is now CI-pushed to
> **production only**, and no non-prod tier may hold it — the board constants in
> `apps/api/src/lib/linear.ts` are hardcoded to the one live "Vendor Requests" project, so a
> staging key files fixture claims as real AECi issues (`AECI-638` "Claim: Fixture Procore" and
> `AECI-662` are exactly that). So the §F1 walkthrough above **cannot be run on staging as
> written.** Two options: run it against production with a throwaway claim you then resolve, or
> give non-prod its own Linear project and make those constants env-configurable first.
>
> The failure-path case is already discharged by production itself, unintentionally. Production
> ran with **no** `LINEAR_API_KEY` from the 2026-07 apex cutover to 2026-09-10; the Procore claim
> (`943e4502-…`) stayed `open`/`linear_issue_id=null`, the 15-minute sweep retried it, and past
> the 60-minute threshold `sendAdminAlert` fired the `stuck-request-alert` email as designed. The
> backstop works. What did not work was anything noticing the key was absent, which is what
> AECI-851 fixes.
