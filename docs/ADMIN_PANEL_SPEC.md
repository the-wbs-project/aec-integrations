...
- **Edit the role or the trust tier.** Neither is writable *from this screen*. Since **AECI-740** one role write does exist — `POST /api/admin/vendors/:id/seats` provisions a `vendor_admin` seat — but it lives on `/admin/vendors/:id` (§5.7) for the same reason revoke does: only the vendor roster shows the blast radius that makes the decision safe, and a seat is a fact about a *vendor* as much as about a person. `trust_tier` still has no writer at all, and granting `admin` remains the per-environment SQL runbook in `environments.md` §10.7.
