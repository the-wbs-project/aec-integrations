...
| Everything else | Live: version, the eleven checks on demand, drift on demand, the watermark, per-table row counts, `stats_cache` freshness. | The eleven checks now **default to the last stored 04:00 result** (`source` + `computed_at` say so); drift stays recompute-only. The rest unchanged. |
...
`lib/data-quality.ts` (04:00 UTC), eleven checks: `products_without_vendor` · `ready_products_unpromoted` · `broken_integration_refs` · `vendors_without_products` · `reviews_missing_anonymized_at` · `stale_stats_cache` · `duplicate_vendors` · `duplicate_products` · `logo_404` · `algolia_index_drift` · `entitlement_mirror_drift`.
