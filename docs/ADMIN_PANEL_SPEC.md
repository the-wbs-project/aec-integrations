...
| *(no PostHog equivalent)* | **Writer** — which of our two writers produced the row (`ssr-arrival` / `browser-spa`). The trusted counterpart to Claimed source above: we stamped it, nobody claimed it (§13 **D18**). **Served by the API, not yet rendered** — the feed carries no request-shape column today, `client_verdict` included | `writer_provenance` (AECI-871) |
...
Filters: date range · traffic type (humans / bots / all, default humans) · source · country · path contains · **writer** (§13 **D18**, `?writer=`, API-side today) · **"filter out internal traffic"**, mirroring PostHog's toggle, backed by an `ANALYTICS_INTERNAL_ASNS` var. That last one is not cosmetic: on 2026-08-10, **67 of 92 "human" views came from the operator's own ISP** (AS23700, Jakarta). It is bound by the three constraints in §13 **D10** — query-time only (it never touches `is_bot` and never runs at ingest), both numbers always shown, and the var ships unset. **D13 shrinks what that toggle is for**: session-identified operator views are now removed unconditionally, so the ASN filter is left holding only what it was always meant to hold — the operator's *other* devices on a known network, and nothing else.
...
- **D18 — Writer provenance is a header we stamp, not a field the client sends** (settled by AECI-871 on 2026-09-11; §5.2, §9.6, `API_CONTRACTS.md` §6.9/§6.10, `DATABASE_SCHEMA.md` §9.1). D14 built `client_verdict` to catch a client that does not look like a browser. D16 then made it decide on its own, with no view floor. Neither noticed that the column had a **free upgrade built into it**: `classifyClientSignals` branched on the request body's `navigation` field, and `'spa'` returned `browser` outright, on the reasoning that "a same-origin `fetch` only happens because our JavaScript ran, which is itself proof of a browser".
  ...
  **Report-only holds, for the fourth time.** Nothing here writes `is_bot`, no ASN joins `DATACENTER_ASNS`, and the headline stays an upper bound. The new column is an annotation and an admin filter axis (`GET /api/admin/page-views?writer=`), nothing more. The Activity feed's UI is unchanged — it renders no request-shape column today, `client_verdict` included, so adding one is a follow-up rather than a one-line addition.
