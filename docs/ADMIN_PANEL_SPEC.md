...
- General catalog editing remains out of scope. **AECI-955 admits a logo-only exception** on vendor detail and the product roster. Operators may save or clear logo_url, which also sets logo_source and updated_at. Other content still flows through review-app promote. Account writes, connector governance, annotations and queue consumption keep their existing boundaries. Stub mapping authoring remains excluded.
...
/admin                     → redirect to /admin/overview
  Insights
    ...
    /admin/activity        §5.2
    /admin/traffic         §5.3
    /admin/audience        §5.4  (aggregates)
    /admin/subscribers     §5.4  (the roster — AECI-859)
  Catalog
    /admin/catalog         §5.5
  ...
  Operations
    ...
    /admin/contests        §5.12 (integration field contests — AECI-1008)
...
**Shell restructure — SHIPPED (AECI-576, P1.2); the nav was COMPLETE for this epic's own routes at AECI-586 (P5.1).** The `h1` reads "Admin", `/admin` redirects to `/admin/overview`, and the nav is grouped. Every screen listed above exists and is linked, so the "omitted until built" mechanic below has no *pending* subjects — it is recorded because it governed how the epic shipped, not because anything is held back. It does still govern later additions: **AECI-652 added Vendors, AECI-692 added Users and AECI-859 added Subscribers through exactly that mechanic**, one `ADMIN_NAV_GROUPS` entry each. ("Complete" therefore means every §5 route is linked, not that the array is closed — `admin-shell.component.spec.ts` pins the ordered result so a later entry cannot land silently.) AECI-692 also demonstrated the inverse — a nav entry can be *replaced*: "Reviewer bans" was swapped for "Users" in the same slot, because `/admin/users?banned=true` is a strict superset of what that screen could show (§5.8). Two mechanics were settled while building it:
...
The three groups were a **14rem left sidebar** from AECI-576 until AECI-694. They are now a **horizontal row under the `h1`**, and the content column is full width.

The reason is §5.7 and §5.8. Both screens render cards until AECI-694 and tables after it, and a table of vendors with seven columns or of people with seven columns wants every pixel of a `max-w-7xl` shell. A rail that permanently spent a fifth of that width on eleven links the operator visits a few times a session was the thing standing between them and the data.

Four mechanics, all pinned by `admin-shell.component.spec.ts`:

- **A category is a disclosure, not a `role="menu"`.** `AdminNavDropdown` (`app/admin/admin-nav-dropdown.ts`) extends the shared `layout/nav-disclosure.ts` base, so it opens on hover, toggles from the button for keyboard, closes on Escape returning focus to the trigger, and closes when focus leaves. That extends the shared open/close base contract of `DESIGN.md` §Navigation (`layout/nav-disclosure.ts`), while the public-nav flyouts use clean text link triggers opened via ArrowDown (Yahoo Finance pattern). **The trigger renders no arrow icon** — the category label (plus its pending badge) is the whole control, matching the public row; `aria-haspopup`/`aria-expanded` carry the disclosure to assistive tech. It is deliberately **not** `@angular/aria/menu`, despite ADR 0010 routing new menu patterns there: this is a navigation row of router links, and the ADR's "Menu / Menubar" row means application menus. The ADR and `ANGULAR_STYLE_GUIDE.md` §19 both carry the clarification.
...
integration field contests routed to AECi. It is a different table
(`integration_field_challenges`), so it is disjoint from the other four by construction,
the same argument as the re-index queue below. It badges `/admin/contests` (§5.12) and
is in the Operations SUM and the header badge. Owner-routed rows are not counted: the
screen shows them read-only, and an operator cannot act on them. The wire field is
optional on the TypeScript type for deploy skew only, and `seed()` leaves an absent key
alone, so an older API shape cannot zero the badge.
...
The analytics digest as a live page. Stat tiles with sparklines — human page views, unique visitors, new sign-ins, active subscribers, catalog totals — each with a day-over-day and 7-day delta matching the email's `deltaText` semantics. Below: a 30-day traffic chart (human vs bot), top traffic sources, top viewed products, and a status strip (prod SHA · stats freshness · failing DQ checks · Algolia drift · moderation depth). A **"recompute today's digest"** action so the operator is not waiting for 05:00 UTC — implemented as `GET /api/admin/overview?recompute=1`, which re-runs the digest's metric collection (already a pure read) and returns it. It does **not** send the email and writes nothing; §13 **D8** draws that line. The lead tile is labelled **"Requests of unresolved origin"** (§13 **D20**, AECI-869) — the same figure the 05:00 email leads with, under the same name. It carries its own **measurement envelope** in the caption (§13 **D15**): the raw server-side count it was subtracted from and how many views the automation filter removed (AECI-745), that the raw figure is an upper bound, the AECI-683 corroborated floor (`corroborated_views` / `corroborated_visitors`), and `operator_leak_excluded` when it is non-zero — on the tile rather than in a tile of its own, because the email prints the same three beside the same number and a caveat one card away from its figure is a caveat nobody reads. **§13 D19 (PROPOSED, AECI-872) would replace this caption's single headline with four reporting classes plus the classifier version that produced them; nothing on this screen changes until that decision is agreed.**

**Since AECI-869 the envelope carries a fifth element, and it is the loudest: the telemetry-health line.** When `traffic.arrival_telemetry.degraded` is true — fewer than `ARRIVAL_CF_COVERAGE_MIN` of the day's full-document arrivals carried a `cf_asn` — the caption states *"Arrival network telemetry was unavailable for this day; network-based exclusions did not run"* with the two counts behind it, and the honesty envelope above carries `arrival_telemetry_unavailable` as a `warn`. It is absent when healthy, and absent on a day with no arrivals at all. Unlike every other item in the envelope it does not qualify the figure, it **invalidates** it against any other day, which is why it sits first among the conditional parts.

**Since AECI-870 the envelope carries a sixth element, and it is the only one sourced from outside D1: the browser-start line.** When `traffic.browser_starts` is present the caption reads *"Browser starts: N, of which search-referred: M. Operator and PostHog-detected bots excluded."* with the two things it is not — a people count, and a figure any browser with a tracker blocker reports — said in the same breath. It is a **separate observation** and the caption says outright that it is never added to or subtracted from the figure above it: `app_started` counts bundle executions and the headline counts requests, and their populations overlap. Present only after `?recompute=1`, because the read costs one PostHog query and §13 **D8** keeps external calls off the default load; on a default load both fields are null and the existing `requires_recompute` note already covers it. When the read ran and failed, `traffic.browser_starts_unavailable` carries the reason and the caption prints it rather than a zero — a zero is a real and alarming value here, so it must not be the shape a failure takes. §13 **D21**.

**`deltaText` semantics, and the degraded-day rule (AECI-869).** `delta_day` is post-automation on both sides (AECI-741) and `delta_7d` is raw on both (AECI-745); both are now additionally subject to a comparability rule. On the email, when either the reported day or the prior day is degraded, `deltaText` prints *"not comparable with the prior day"* and **no arithmetic at all** — suppressed rather than hedged, because a delta is a single claim and a claim qualified underneath itself is still read as the claim. On the panel the structured deltas are unchanged (the UI localizes its own prose) and the caveat is carried by the note. `series_30d` points carry a per-day `degraded` flag, and `series_spans_degraded_days` counts them. Since AECI-877 the 30-day chart draws a degraded day **dimmed and hatched** across the whole column, adds a *Missing network information* legend entry and a matching column in its hidden table, and shows none of the three when no day is degraded — the note keeps the prose; a day with no stored `quality.arrival_cf_coverage` row is `false` — *not assessed*, never *blind*. The sign-in delta is exempt throughout: `profiles.created_at` reads no network column.
...
| *(no PostHog equivalent)* | **Request** — which of our two writers produced the row, rendered *Page load* (`ssr-arrival`) / *In-app navigation* (`browser-spa`) / *Not recorded* (null), with the request-shape verdict under it (*Browser-shaped headers* / *Headers contradict the browser it claims* / *No browser headers* / *Not enough signal to judge*; nothing when null). The trusted counterpart to Claimed source above: we stamped it, nobody claimed it (§13 **D18**). Only the two verdicts that flag a row on their own (`inconsistent`, `non-browser`) are tinted, to the same strength as the non-eyeball network note, and the words carry it alone. **Rendered since AECI-877**, which also put `client_verdict` on the wire | `writer_provenance` (AECI-871), `client_verdict` (AECI-658) |
...
Filters: date range · traffic type (humans / bots / all, default humans) · source · country · path contains · **writer** (§13 **D18**, `?writer=`; a static select since AECI-877 — *Any* / *Page load* / *In-app navigation* / *Not recorded*, because the vocabulary is closed and ours, so there is nothing to discover from the window) · **"filter out internal traffic"**, mirroring PostHog's toggle, backed by an `ANALYTICS_INTERNAL_ASNS` var. That last one is not cosmetic: on 2026-08-10, **67 of 92 "human" views came from the operator's own ISP** (AS23700, Jakarta). It is bound by the three constraints in §13 **D10** — query-time only (it never touches `is_bot` and never runs at ingest), both numbers always shown, and the var ships unset. **D13 shrinks what that toggle is for**: session-identified operator views are now removed unconditionally, so the ASN filter is left holding only what it was always meant to hold — the operator's *other* devices on a known network, and nothing else.
...
#### 5.4a The subscriber roster — SHIPPED (AECI-859, 2026-09-11)

The section above reports the mailing list as a **shape**: a count, a growth line, a churn rate, a UTM breakdown, a geography breakdown. None of those can be read down to a person. `/admin/subscribers` is the other half — **every subscriber, and the day they signed up** — behind `GET /api/admin/subscribers`, which is the first endpoint anywhere in the product that returns a `mailing_list` row.

It is a nav-able Insights screen beside Audience rather than a section on it. Audience is five charts sharing one window control and one load; a roster needs its own search, membership filter, sort and pagination, and three independent fetches on one screen is where a dashboard stops being legible. Placing it under **Insights** rather than Operations follows §5.7's rule in the mirror: what that screen does is account administration, what this one does is read.

**The two screens link to each other, and both directions are required.** The roster's intro points at Audience for growth, churn and attribution over time. Audience carries the reverse link directly beneath its lifetime KPI row, because those three tiles count *people* and that row is the only place on the screen where a reader can get from a count to the people it counts. The link sits under the tile row rather than inside a tile: `aec-stat-tile` has a closed contract (label · value · delta · sparkline, and no more, §5.3), and one link serves all three subscriber tiles. Asserted in `audience.component.spec.ts` — a one-way pair is the failure this is written down to prevent, since the nav would still make the roster reachable and nothing would look broken.

**Four things the build settled.**

**(1) It shows people who left, by default, and marks them.** The membership filter opens on *Everyone*. `unsubscribed_at` is a soft delete, so an unsubscribed person is still a fact about the list, and defaulting to *Subscribed* would make the table silently disagree with the lifetime `total_ever` printed directly above it. The per-row `status` badge and its opt-out date are what make showing them honest rather than confusing — and the date is the useful half: *left* is a fact, *left the day after the launch email* is a finding.

**(2) The stocks beside the table do not narrow with the filter, and come from the same function Audience uses.** `total` on the envelope is post-filter, because the paginator needs it. The quoted "N people have joined, N are still subscribed, N have opted out" comes from `subscribers`, produced by the identical `subscriberTotals()` call `/api/admin/audience` makes — the one-implementation rule P1.1 note 1 established for `collectAnalyticsMetrics` and §5.6 re-applied to the status items. Two screens reporting the same stock must not be *able* to disagree.

**(3) There is no opt-out control, and §5.2's read-only rule is not what stops one being added.** The only writer of `unsubscribed_at` is the subscriber, through the tokenized `POST /api/unsubscribe` (AECI-537). An operator button here would let the console suppress a consent record with no evidence the person asked — a different act from honouring an unsubscribe even though it writes the same column. Asserted in `subscriber-list.component.spec.ts` so a later PR cannot quietly add one.

**(4) `unsubscribe_token` is excluded from the projection, and that is a security property rather than tidiness.** The token is a bearer capability: `POST /api/unsubscribe` takes it alone. A response body carrying one per row would put a working opt-out link for every subscriber into a browser tab, a screenshot, and any log that captures a response. The query names its columns rather than selecting the row, so a column added to `mailing_list` later cannot leak by default, and the handler spec asserts over the **serialized body** — parsing first would let Zod strip the very key the test is looking for.

Two smaller notes. **Email sorts under `COLLATE NOCASE`** (AECI-825, `textDir`), with the `id` tiebreaker behind it: `NOCASE` makes `A@x` and `a@x` equal where `BINARY` never did, so without a unique trailing term a page boundary can drop or repeat a row. And the two sort keys have **different natural directions** — signup date opens newest-first, email opens A-to-Z — held once in `ADMIN_SUBSCRIBER_SORT_DEFAULT_ORDER` and read by both the server's `ORDER BY` and the header arrow, so the control cannot promise an order the query will not deliver.
...
- **Each column reads the same population as its card (AECI-1074, 2026-09-22).** The integrations card counts live `integrations` rows plus live `connector_evidenced_pairs` rows, which is the AECI-721 lockstep rule (live on the pair arm since AECI-1091). The net integrations series now reads both tables too. Each row is bucketed by its own table's `created_at`, the retired filter applies to both tables (to `connector_evidenced_pairs` since AECI-1091, when pairs became retirable), and the per-day counts are summed. Until this fix the series read `integrations` alone. Production on 2026-09-22 had 941 live `integrations` and 51 evidenced pairs, so the card read 992 and the column summed to 941. It also made every cross-table promote move (AECI-888) look like a removal. `count-lockstep.spec.ts` now executes this series against the card as site X4. **One residue remains:** a cross-table move re-inserts the row without carrying `created_at`, so a moved row lands in the bucket of the day it moved rather than the day it arrived. The count is right and the date is not. That is AECI-1076.
...
- **Cron liveness** — last run, duration, outcome per job, for all fifteen crons (§7.2).
...
|---|---|---|
| Cron **last run** | Derived for two of fourteen — `home-stats` from `MAX(stats_cache.computed_at)`, `algolia-sync` from the `algolia_sync_watermark` row's stamp — and `unknown` for the other twelve. | Read from `job_runs` for every job that has run. `derived` survives only as the pre-first-run fallback (deleting it would blank all fourteen for 24h after each deploy), and still reports no outcome — a stamp proves the job *ran*, never that it *succeeded*. |
...
| Everything else | Live: version, the checks on demand, drift on demand, the watermark, per-table row counts, `stats_cache` freshness. | The checks now **default to the last stored 04:00 result** (`source` + `computed_at` say so); drift stays recompute-only. The rest unchanged. |
...
- **`/admin/vendors`** — every vendor in the catalog, searchable by name or slug, filterable by verified state, sortable by **every** rendered column, and paged by **scroll** (see the list-rendering note below). This is the screen whose absence was the problem: until it existed the only route to a vendor's entitlement ran through an `/admin/claims` card, so a vendor that never filed a claim was unreachable and concierge onboarding could not be done at all.
- **`/admin/vendors/:id`** — six sections behind a four-tab horizontal menu (see the tabs note below; the fourth tab, **Integrations**, is AECI-1046's and described after the tabs note): **Vendor** (basics + counts, the entitlement control — **moved here** from the claim card, so the §5.2/§5.3/§5.4 copy invariants live in one file — and the seat roster with an admin revoke **and, since AECI-740, an admin provision**), **Products** (the vendor's catalogue, read-only), and **Audit Trail** (the first `audit_log` viewer the platform has ever had).
...
> **List rendering revision — SHIPPED (2026-09-03).** Four changes to `/admin/vendors`; the contract lives in `STAGE_2_PAID_TIERS_SPEC.md` §5.6, which carries the reasoning in full. In this doc's terms:
>
> - **The Integrations column is gone**, and with it `integration_count` on `AdminVendorRowSchema`. An operator on the list is triaging entitlements and seats; the integration count is a catalog fact acted on at `/admin/vendors/:id`, and carrying it here cost a correlated subquery per row for a number nobody triaged on.
> - **The slug is its own column**, not a second line under the company name — scannable, sortable, and one line per row.
> - **Every column is sortable.** AECI-694 shipped two headers on the rule that a control the API cannot honour would reorder one page and present it as a ranking (the same rule §5.8 applies to Last sign-in, which stays plain). The rule is unchanged; the API changed — a new admin-only `AdminVendorSortSchema` orders by all seven columns, including the two on the joined entitlement row. **Direction is now a toggle** (see the sort-direction note below).
> - **Paging is scroll-append**, via the shared `<aec-pagination-footer>` the public listings use, replacing `<aec-admin-paginator>`. **This is the first `/admin` screen to page by scroll** — the Activity feed, `/admin/users` and the claim queue keep prev/next, and nothing here proposes changing them: a chronological feed is walked, a lookup surface is scanned. `GET /api/admin/vendors` is unchanged and still page-based; `total` still describes the filter, which is what the footer's "Showing X of N" reports.

> **Sort direction — clicking an active header REVERSES it.** The list originally had no `order` parameter at all: direction was fixed per key server-side, the arrow was a statement of how a column sorts, and clicking the active header was a deliberate no-op. That rule was written when the endpoint could order by two keys and became a defect once it could order by seven — an arrow on a clickable header reads as a toggle to any operator who has used one, and silently doing nothing on the second click is a worse lie than the one the rule was avoiding. `?order=asc|desc` is now on `GET /api/admin/vendors` and `GET /api/admin/users`, **optional**, and absent still means the key's natural direction, so every pre-existing link orders as it did. Four notes:
>
> - **One copy of the natural directions.** `ADMIN_VENDOR_SORT_DEFAULT_ORDER` / `ADMIN_USER_SORT_DEFAULT_ORDER` live in `packages/shared`; the API resolves its ORDER BY from them and the table draws its arrows and first-click direction from them. Two copies would drift silently — a ↑ over a descending list, with nothing failing.
> - **Switching columns resets to the new column's natural direction.** Moving from "Updated ↓" to "Vendor" gives A–Z, not Z–A: direction is a property of the key, not a mode the table stays in.
> - **Only the primary term flips.** The `id ASC` tiebreaker and the intermediate name term never move (AECI-99 — reversing a tiebreaker is how a paginated list duplicates and skips rows), and `term` pins its NULLs-last guard so `desc` reads "who lapses last" rather than floating every perpetual vendor to the top.
> - **`aria-sort` reports the live direction**, and the button carries an sr-only hint saying what the next press will do — the arrow is `aria-hidden`, so without it a screen-reader user hears an identical button name before and after a flip.

> **Tabs — the detail screen is a three-tab menu.** A horizontal row — **Vendor** / **Products** / **Audit Trail** — over one panel, with the tab held in `?tab=` (absent = Vendor, which keeps the bare URL canonical). Four notes:
>
> - **Basics, entitlement and seats stay on ONE tab.** They are read together: the entitlement state explains the roster, and a decision about either needs both on screen. The two blocks that moved out are the two that are long, separately paginated and scanned rather than read alongside the rest.
> - **A query param, not child routes.** A route per tab would cost two more lazy components and two more resolvers to carry nothing but a signal. A query param keeps the tab linkable, bookmarkable and Back-navigable, which an in-page toggle silently gives up — the same lesson the vendor portal learned when its `@switch` sections became child routes (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4).
> - **Links, and a `nav`, not an ARIA `tablist`.** These change the URL rather than swap panels in place; a `tablist` would promise arrow-key semantics the router does not implement. The open tab carries `aria-current="page"`.
> - **Each tab fetches once, lazily.** The vendor and the audit trail load on arrival (the trail is refetched after every write on this screen, so the page holds it regardless of which tab is open); Products waits until someone opens it, which is why it is a separate endpoint rather than a field on the detail payload.

> **Integrations tab — AECI-1046.** `?tab=integrations`, between Products and Audit Trail, over `GET /api/admin/vendors/:id/integrations`: the **vendor-held** integrations this vendor owns (claimed, or added by the vendor), live and retired, in one fetch of up to 100 rows by name. Each row shows the pair, a status (**Live**, **Retired by AEC Integrations**, **Retired by the owner**) and one action. **Retire** on a live row and **Restore** on an AECi retire each open an inline form under the row with a required reason, and only the form's own button sends the request (no browser dialog). An owner retire offers nothing and says "Only the owner can restore it", because the owner controls its own retire (ruled 2026-09-22). Pessimistic, announced through the page's one live region; a lost race reloads the list. The reason goes to the audit row and is not shown to the vendor. The write's audit row files under the integration, so this page's Audit Trail tab does not show it. An AECi-held row is not listed, because the write refuses it. Contract: `STAGE_2_VENDOR_PORTAL_SPEC.md` §4.6.4.
>
> **Evidenced pairs — AECI-1091.** The tab lists the vendor-held `connector_evidenced_pairs` rows this vendor owns beside its `integrations` rows, in one name order. A pair row adds "Delivered through {connector}" under the pair, and its retire form says the row also stops counting on that connector. The actions, the reason and the cross-refusal are the same, through the same two routes. A retired pair's audit row files under entity type `connector_evidenced_pair`. The connector admin screens (§5.9) count and list only live pairs, so a retired pair is reached here, on its owner's page.

> **Products tab — `GET /api/admin/vendors/:id/products`.** Paginated, ordered by name, and it joins **every** `product_vendors` row rather than only the primary — a co-owned product is owned, the rule `product_roles` already counts by (`STAGE_2_SPEC.md` §8.8(1)) — with `is_primary` on the row so the two are distinguishable. Columns: product (name + slug), role, promotion status, integrations, reviews, record updated, and a **View page** link that opens the public product in a new tab (there is no admin product detail page to link to instead). Role and promotion status are the two that decide what the operator does next: role is the §5.2 payer test *per product*, where the Basics roll-up can only give a total, and promotion status is the single field that explains a product missing from the public site. The average rating obeys the §5.5 five-review floor exactly as the public surfaces do — the console is not an exemption. **Read-only**: there is no admin product-edit endpoint, and catalog records still reach AECi only through `POST /api/promote`. Since AECI-769 the Basics "Products by role" count is a link to `?tab=products`, so the product that is the connector is one click from the breakdown. Its visible text is the unchanged count, and a visually-hidden suffix inside the anchor names the Products tab as the destination.

Per §9.3 the four GETs emit no `audit_log` row; the revoke does, in the same `db.batch` as its write, via the shared `revokeSeatStatements` builder — as does the AECI-740 provision, via `provisionSeatStatements` in the same module.

> **Revoking the last seat hands the record back — SHIPPED (AECI-989, 2026-09-23).** Pulled into Stage 2.1 because it gates seating a pilot vendor AECi might need to revoke. When the revoke leaves the vendor with **no** `vendor_admin` profile, banned or not, the same batch also does four things. It returns `vendors.maintained_by`, and each product the vendor owns alone, to `'aeci'`. It clears `claimed_at` on every **live** integration the vendor owns and claimed, so promote writes it again. It returns that integration's marker to `'aeci'` when no live vendor attestation survives. And it re-routes the vendor's open owner contests to AECi for good. Each write has its own audit row. Nothing is deleted, and claims, attestations, `last_reviewed_at`, `origin`, retired rows, the entitlement and the label are untouched. When only **banned** seats remain, the batch moves the open owner contests alone. They move back when the vendor has an unbanned seat again, by an unban or by a new seat grant. A revoke that leaves an unbanned seat changes nothing beyond the profile. The UI is unchanged: the revoke confirm already reads as the admin escape hatch. Contract: `STAGE_2_ATTESTATIONS_SPEC.md` §13.9. Endpoint: `API_CONTRACTS.md` §6.10.

> **Seat provisioning — SHIPPED (AECI-740, 2026-09-03).** `POST /api/admin/vendors/:id/seats`, an "Add a seat" control in the Seats section. It is the **only route that writes `profiles.role = 'vendor_admin'` on its own** — `PATCH /api/admin/claims/:id` writes it as part of a claim grant and `POST /api/seat-invites/:token/accept` writes it when an owner's invite is redeemed; what is new here is a seat with no claim and no invite behind it — and it opens **no `vendor_entitlements` row**, so the public account label never appears — which is the whole point rather than an implementation detail. `STAGE_2_SPEC.md` §8.9(1) gives a pure **connector** vendor a catalogue-maintenance seat without paid vendor access, while every prior path to a seat opened an entitlement on the way; that is why the operator note above says a connector claim is parked rather than granted. Step 9 of that procedure can now be completed.
>
> Three IA notes, in this section's voice:
>
> - **It belongs on the vendor, not on the claim or the person.** `/admin/claims/:id` deliberately does not moderate and does not manage the entitlement (§5.10); `/admin/users/:id` deliberately cannot revoke a seat or edit a role (§5.8). This page is the one that shows the blast radius — the other seats, the entitlement state, the `product_role` breakdown — which is the same reason revoke lives here.
> - **Two write actions, still not one button.** Adding a seat and setting an entitlement stay separate controls. They are the two halves the claim grant fuses, and that fusion is the problem `STAGE_2_VENDOR_PORTAL_SPEC.md` §5.2 exists to route around; merging them here would recreate it on the surface built to avoid it.
> - **It warns and never gates.** On a vendor owning endpoint products the control shows a warning above an **enabled** button, and zero products reads as *unknown* rather than exempt — the AECI-738 rule verbatim, for the same reason: `product_role` is curated upstream, so a mis-roled record must not hard-block a legitimate operator. Since AECI-1041 it also warns on a vendor that owns integrations, because such a vendor pays if it wants to manage them (§8.10) and this seat is free.
>
> Contract in `API_CONTRACTS.md` §6.10; the owning spec section is `STAGE_2_PAID_TIERS_SPEC.md` §5.6.3 (retitled by this issue), as-built in `STAGE_2_VENDOR_PORTAL_SPEC.md` §5.3.

> **Operator note — a connector vendor's claim is neither granted nor rejected (AECI-704).** A vendor **all** of whose products are `product_role = 'connector'`, and that owns no integration it manages, is not sold verification (`STAGE_2_SPEC.md` §8.8 / §8.9). One that does own integrations it manages is a paying third-party owner and takes the ordinary Grant (§8.10, AECI-1017). Since **AECI-1041 (2026-09-22)** both screens show an "Integrations owned" count over both delivered-tier tables, and on a pure connector vendor that owns integrations the banner above Grant/Reject reads as a paying owner instead of a park. On `/admin/claims`, Grant would open a `verified` entitlement and light the badge, and Reject sends a real decline email. The claim is parked `open` and routed to the partnership track out of band, and this screen never opens an entitlement for a pure connector vendor that owns no integrations. Since **AECI-738 (2026-09-02) both screens surface `product_role`**: this one carries a "Products by role" row in its Basics list, and `/admin/claims` carries the same breakdown as a verification signal plus a warning banner above Grant/Reject on a pure-connector vendor. **AECI-739 (2026-09-02) closed the last gap the procedure depended on**: a parked claim now has an in-product home for its reasoning — `/admin/claims/:id` with a free-text operator note (§5.10), rendered read-only on the queue card so a parked claim is legible from the list, plus a named explanation of the duplicate chip §5.2 step 5 warns about and an **In review** filter tab the API could not express before. Full procedure — and why `RoleBadge` is deliberately not reused for the aggregate, why zero products reads as *unknown* rather than exempt, and why the actions stay enabled — is `STAGE_2_VENDOR_PORTAL_SPEC.md` §5.2.
...
>
> - **Two sortable headers on the list, and only two.** `AdminVendorsListQuerySchema.sort` was `created | name | updated` and `created_at` is not on `AdminVendorRowSchema` — so Vendor and Updated got a control and the other five headers stayed plain text with no hover state. A header that looked clickable and reordered the 25 rows on the current page would present a page as a ranking; making more columns sortable is an API change, not a UI one. *(Both halves of this bullet have since been overtaken: all seven columns are sortable, and clicking an active header now reverses it — see the sort-direction note below.)*
> - **The audit trail's action column reads as English.** `describeAuditAction()` (`app/admin/audit/audit-action-labels.ts`) maps the ~40 action strings the API actually writes, with the two templated families expanded (`{category,audience,phase}.created`, `{reviewer,vendor_admin}.{banned,unbanned}`). An unmapped action humanises its token instead of rendering blank — mandatory, because `action` is a plain `z.string()` by contract precisely so a new writer elsewhere cannot 500 this screen, and `audit_log` is excluded from the retention prune (§13), so this reader meets rows written by code that no longer exists. The raw token stays on screen beneath the description, because a row still has to be greppable against a log line. `metadata` is **not** on the wire, so descriptions are derivable from `action` alone; surfacing `source` / `fields` / `reason` would be a handler and schema change.
> - **Audit timestamps are relative** ("4h", "2d") via `<aec-relative-time>` (`app/shared/relative-time/`), with an info control whose **accessible name is the full `medium`/UTC instant** and a `cdkConnectedOverlay` panel on hover, focus or click. The overlay rather than the repo's CSS tooltip because the table lives in an `overflow-x-auto` box that would clip an in-flow panel. `now` is read once at construction, which matches the no-live-updates rule above: the stamp is as fresh as the fetch that produced the row. Every other timestamp on this screen is absolute `medium`/UTC — several were raw ISO strings before AECI-694.
> - **"View public page" is now "View Page" and opens in a new tab** (`target="_blank" rel="noopener"`, with a visually-hidden "opens in a new tab"). An operator opens it to check something against the admin record, so navigating away from that record was the wrong outcome; that is also why it is an `href` rather than a `routerLink`. **(AECI-980: the visually-hidden note moved inside the anchor and gained a drawn `arrow-up-right` beside it, both from the shared `aec-new-tab-icon`; the bordered button became the standalone-link role so it matches the portal's own "View public page". See `DESIGN.md` → "The Link Treatment Rule".)**
...
- **Edit the role or the trust tier.** Neither is writable *from this screen*. Since **AECI-740** an admin route that provisions a `vendor_admin` seat on its own does exist — `POST /api/admin/vendors/:id/seats` — but it lives on `/admin/vendors/:id` (§5.7) for the same reason revoke does: only the vendor roster shows the blast radius that makes the decision safe, and a seat is a fact about a *vendor* as much as about a person. `trust_tier` still has no writer at all, and granting `admin` remains the per-environment SQL runbook in `environments.md` §10.7.
...
> **Rendering revision — SHIPPED (AECI-694).** The list is a table rather than a card list, following §5.7. No endpoint or contract moved.
>
> - **Two sortable headers: Profile created (`created`) and Updated (`updated`)**, both descending, matching `AdminUsersSortSchema` and `resolveAdminUserOrderBy`. The list gained an **Updated column** so the second supported key is reachable at all — `updated_at` was already on `AdminUserRowSchema` and simply had nowhere to go. As with §5.7, clicking an inactive header adopts that key's natural direction — both are naturally descending, newest first — and clicking the active one **reverses** it via `?order=asc|desc` (`AdminUsersListQuerySchema.order`, resolved from `ADMIN_USER_SORT_DEFAULT_ORDER`). *(This bullet originally read "clicking selects a sort rather than toggling a direction, because the endpoint takes no `order`"; the parameter shipped with the §5.7 sort-direction change, and `SortHeader` is shared, so both screens gained the toggle together.)*
...
**This section is *not* read-only, and §2 names it as one of the five exceptions.** The note
...
two endpoint shapes. Recorded here because the route, the IA, the badge and the write are
this doc's business.

**What a row is.** A seated vendor on one end of an integration says one field is wrong.
The row carries the value on record, the proposed value and the vendor's reason.

- **`/admin/contests`** — one list, newest first. Each card shows the pair (linked to the
  public pair page), the integration name, the field, the value on record and the proposal,
  the reason, the vendor that filed it ("Filed by"), the owner snapshot ("Offered by"), and
  the submitted date. A decided row adds the decision date and note. An accepted AECi row
  adds a link to its `REVIEW - ` Linear issue, or "Linear issue pending".
- **Two filters.** Status tabs (Open, Accepted, Declined, Withdrawn; default Open) and
  "Decided by" (AEC Integrations by default, or The owner).

Values render for an operator, not as stored. `direction` arrives as `a_to_b | b_to_a |
both` and reads "Procore sends to Summit"; `owner` arrives as a vendor id and reads as the
vendor's name, or "No owner on record" / "Neither endpoint vendor" for `null`;
`mechanism_kind` uses the public label. The field names reuse the portal's
`vendor-contest-labels.ts`, so the vendor and the operator read the same word for each
field. The owner field is labelled **Owner** (AECI-1003 / AECI-1021), never "Builder".
...
**This section is not read-only, and it is the eighth §2 exception.** The write is
...
- **Pessimistic, with a reload on a lost race.** A success drops the row and decrements
...
- **Edit the integration.** There is no catalog write here. The value changes upstream.
- **Decide an owner-routed contest,** for the reason above.
- **Re-file a missing Linear issue by hand.** The §6.7 reconciliation sweep retries accepted
...
| `GET /api/admin/subscribers` | §5.4a roster — **SHIPPED (AECI-859)** | The only endpoint that returns a `mailing_list` **row**. `PageQuerySchema` + `?status=active\|unsubscribed\|all`, `?search=` (escaped substring on email), `?sort=created_at\|email` + `?order=`. No window, same reasoning as `/feedback`. `unsubscribe_token` is **not** in the projection — it is a bearer capability (§5.4a note 4). Email orders `COLLATE NOCASE` with the `id` tiebreaker (AECI-825) |
...
| `GET /api/admin/vendors/:id/products` | §5.7 Products tab — **SHIPPED** | The vendor's catalogue, paginated and name-ordered. Joins EVERY `product_vendors` row (a co-owned product is owned — §8.8(1)); `is_primary` distinguishes them. `rating_overall_avg` obeys the §5.5 five-review floor. Contract in `packages/shared/src/api/admin-vendors.ts` |
...
| `DELETE /api/admin/vendors/:id/seats/:userId` | §5.7 revoke — **SHIPPED (AECI-652)** | **A write, not a read.** Composes `revokeSeatStatements`, so its `audit_log` row rides the same `db.batch` and that builder names no `vendors` statement. **Since AECI-989 the last seat also hands the record back** in the same batch (`planVendorHandback`: `maintained_by` to `'aeci'`, `claimed_at` cleared on live claimed integrations, owner contests to AECi), and purges the pages whose marker changed |
...
- **`?recompute=1`** — additionally runs all §23.1 checks and the drift count. `algolia_index_drift` *is* the drift check, so the drift runner is invoked **once** and its result feeds both the strip and the suite.
...
tabulated in `DATABASE_SCHEMA.md` §9.3 — **21 keys**: the 9 flow keys the
timeseries endpoint already served, 11 stocks (catalog totals, subscriber
counts, queue depths), and since AECI-869 one **data-quality** key,
`quality.arrival_cf_coverage` (§13 **D20**). That third family is a per-day RATIO
rather than a count, and it is in `ADMIN_SNAPSHOT_QUALITY_METRIC_KEYS` rather than
`ADMIN_METRIC_KEYS` because the endpoint's read path rounds a stored value to an
integer — which would turn a failing 0.94 into a passing 1. The stocks are written from day one but **not yet readable
...
**`page_views_human` means two different things, and both are correct.** In `metrics_daily` the key
`traffic.page_views_human` is the **RAW** human count — `NOT_INTERNAL` + `HUMAN`, nothing subtracted. In the
`GET /api/admin/overview` response the field `page_views_human` is the **POST-AUTOMATION** count, redefined by
AECI-745 (§13 **D17**), with the raw figure moved to `page_views_human_raw`. The filtered *series* is a
separate key, `traffic.page_views_human_after_automation`, which is snapshot-only and deliberately not
backfillable. So a reader reconciling the Traffic chart against the Overview tile will find the same name
carrying different numbers. That is not a defect, and it is stated here because nothing in the types says it:
`AdminMetricKeySchema` and `AdminOverviewTrafficSchema` are separate shapes that happen to share a string.

**One residual the stored series carries and does not correct: duplicate arrivals.** Until 2026-09-01 nothing
guaranteed one document load wrote one `page_views` row (AECI-743). Those rows are still in the table, the
00:15 cron has been counting them with `count(*)` since this section shipped, and the AECI-688 re-backfill
counted them the same way — deliberately. Deduplicating in the backfill alone would put a **second** definition
of "duplicate" in the tree and make the reconstructed days disagree with every live surface, which is the
defect AECI-688 existed to remove, reintroduced at a different date. Purging the rows has no defensible
keep-rule: 589 of the 664 suspected human pairs predate AECI-585 and carry a null `navigation`, and tightening
the detection window from 20 s to 3 s moves the arrival count from 52 to 19 — the stored row cannot distinguish
a double-fire from a genuine reload. Measured effect on the corroborated floor, the one figure a proxy pool
cannot inflate, is **two days**: 2026-08-29 (2 → 1) and 2026-08-18 (4 → 3).
...
Before this table, a cron's outcome existed only as an emitted metric, and the data-quality findings existed only in an email:
...
**And the flag was only two thirds of it — SHIPPED 2026-08-27 (§13 D15).** `is_operator` is written once at ingest and fails open on an expired token, so an operator browsing across a token expiry leaves a contiguous run of unflagged rows that nothing on the row distinguishes from a visitor — 22 of them in one gap on 2026-08-26. `NOT_INTERNAL` therefore carries a **third** half: a correlated `NOT EXISTS` excluding any row that shares a `(user_agent_hash, cf_asn)` pair (§9.8's own definition of a visitor) with a verified operator row within `OPERATOR_PAIR_LOOKBACK_DAYS`. Unlike the first two it is an **inference**, not a fact about the request, so it is the one half the digest and the panel *report* rather than apply silently (`operator_leak_excluded` + the `operator_leak_is_an_inference` note, the note gated on that figure being non-zero — AECI-836). All three live in one predicate for the same reason they always have: a caller that remembered two and forgot the third would report a partly-corrected number, which is worse than any consistent alternative.

**And one of its inputs was taking the client's word for it — SHIPPED 2026-09-11 (§13 D18).** The three halves above exclude the operator. A fourth defect sat one layer down, in the column the digest uses to exclude *automation*: `client_verdict` decided which rule set to apply from the **body's** `navigation` field, so a payload saying `'spa'` was handed `browser` — the strongest verdict the system issues — with every arrival-shape check skipped. Any HTTP client could POST that through the SSR `/api/*` passthrough. Since §13 **D16** a `browser` verdict is also what keeps a row out of the automation exclusion, so a client-settable field sat in front of the headline. The fix is the same trusted-boundary pattern `x-aeci-cf-*` already uses: the SSR Worker stamps `x-aeci-writer` (`ssr-arrival` / `browser-spa`), stripping any client copy first, and the verdict reads that. **What `browser` now requires on an in-app hop:** trusted provenance **and** the headers a same-origin `fetch` from our bundle sends (`Sec-Fetch-Site: same-origin`, `Sec-Fetch-Dest: empty` or `Sec-Fetch-Mode: cors`, `Accept-Language`). Anything short of both is `unknown`, never `non-browser` — see D18 for why that distinction is the whole design.
...
8. **"Visitor" is a defined term.** Absent sessions, a visitor is a distinct `(user_agent_hash, cf_asn)` pair within the selected window. This over-counts (UA changes on browser update) and under-counts (shared NAT). The definition appears in the UI next to the number, not only in this document.
   ...
   **⚠️ One half of that pair was missing on arrivals, 2026-09-07 → 2026-09-11 (AECI-868).** A cache-gateway change replaced `request.cf` on the SSR loopback instead of merging into it (`CACHE_STRATEGY.md` §4a.1), so every `page_views` row with `navigation = 'arrival'` written in that window stored NULL `cf_asn`, `cf_country`, `cf_colo`, `cf_as_organization`, `tls_version` and `http_protocol`. Production went from **1,458 of 1,458** arrivals with an ASN on Sep 6 to **0 of 2,633** on Sep 8. Browser `spa` rows took a different code path and are unaffected. **Staging and preview are affected from 2026-07-19**, when WC-4 landed on `stage-2`.

   Four consequences the panel must not paper over, because none of them is recoverable — nothing in a stored row implies the ASN it never received:

   - The **visitor pair collapses to one half**. Every arrival in the window shares a NULL `cf_asn`, so distinct-pair counts for those days conflate visitors whose UA hash matches.
   - The **§13 D15 operator-pair retro-join cannot match a NULL `cf_asn`**, so a lapsed operator session on an arrival leaks into the human count for those days. `operator_leak_excluded` under-reports rather than over-reports.
   - The **swarm and ASN-rotator groupings do not run on those rows.** `countDistinct(cf_asn)` is 0, so `SWARM_MIN_ASN_RATIO` / `SWARM_RECURRING_ASN_RATIO` fail even for a group with hundreds of views, and `detectAsnRotators` excludes NULL-ASN rows by design.
   - **Networks (§5.3, `dimension=asn`) and Geography are blank for arrivals** in the window, and `asn_registry` (§7.6) has nothing to annotate.

   Arrival rows carry real network metadata again from the first full UTC day after the fix promotes to production. Days inside the window are **not comparable** with days outside it — say so beside the number rather than silently charting across the gap, which is the same obligation §9.8 already imposes on the definition itself. The standing guard is the `arrival_cf_coverage` data-quality check below.
...
> **Superseded for later units (2026-09-11).** `admin-panel` no longer exists on origin — `git branch -r` lists `main` and nothing else long-lived. The Stage 1 units above shipped under the rule as written; everything since (AECI-652, AECI-692, AECI-722, AECI-739, AECI-777, AECI-859) branched from and merged to **`main`**, per `CLAUDE.md` §"Git workflow". That is also the safer line: `admin-panel` never had the `deploy.yml` trigger fix, so a PR into it ran no tests at all.
...
| **P5.1** | AECI-586 | Audience section (mailing list + feedback) — **SHIPPED 2026-08-13** (`GET /api/admin/audience` + `GET /api/admin/feedback` + `/admin/audience`; the §5 nav was complete for the epic's own routes at this point — AECI-859 later added a thirteenth entry, see P5.2). Carries the epic's only schema change outside §7: three lead-capture indexes (§7.5) | P1.4 |
| **P5.2** | AECI-859 | Subscriber roster (§5.4a) — **SHIPPED 2026-09-11** (`GET /api/admin/subscribers` + `/admin/subscribers`; the thirteenth nav entry). No schema change: the AECI-586 indexes already cover it | P5.1 |
...
- **D9 — PostHog reverse proxy: spun out** (was Q5). **AECI-590**, outside the AECI-572 epic, Low priority. Nothing in this epic depends on it. Scoped honestly there: `posthog-js` is not imported until consent is `granted`, so the recoverable population is only *"users who accepted the banner **and** run a blocker"* — smaller than "blocker-lost events" implies.
  ...
  > **Correction (2026-09-13, AECI-590).** The scoping sentence above is **stale in both directions** and is left intact only as the record of what D9 decided. **AECI-643** replaced strict opt-in with the two-mode init (`POSTHOG_MIGRATION_SPEC.md` §3.3 / D2): **Tier 2 boots `posthog-js` for every visitor, DNT/GPC included**, carrying `$exception`, `$web_vitals` and the `app_started` liveness beacon (`apps/web/src/app/analytics/posthog-client.ts`, `docs/ANALYTICS.md` §5). So the blocked population is every visitor, not the consented minority, and two non-analytics things are collateral: `/flags/` (a blocked visitor stands at every feature-flag default) and `/array/{token}/config` (the remote config that actually enables `$web_vitals` and exception autocapture — §8.8).
  >
  > That makes D9's population argument **bigger** than written. What shrank the issue instead was the **measurement** AECI-590's own AC demanded, run 2026-09-13 over 2026-09-07 → 09-11 production: **80 anonymous browser page loads in five days, about 16 a day.** That is the count of persons firing *exactly once*, which under Tier 2's `persistence: 'memory'` is one page load each; the one remaining person in the 121-event pull fired 41 times off a stable `localStorage` id and is the operator's own consented Tier 3 session. **That 121 is the same measurement D21 records as 109** — D21 pulled it on 2026-09-11 with that day still in flight and no host filter, this one on 2026-09-13 over the complete window scoped to `$host = 'www.aecintegrations.com'`; the operator's 41 is identical in both. At PostHog's cited 10–30% blocker recovery that is 2–5 page loads a day. The numbers and the re-open trigger are in `ANALYTICS_BASELINE.md`.
...
- **D15 — Repair the operator flag on read, and report both what that removed and what a referrer corroborates** (settled by AECI-683 on 2026-08-27; §5.1, §9.6, §9.8, and `POST_LAUNCH_MONITORING.md` §3b). D14 shipped **measurement**, not correction — by design. Three days later the measurement produced its own verdict: on **2026-08-26** the digest emailed "up to 102 human views" beside a PostHog floor of "47 page views from 1 person", and *that one person was the operator*. Both bounds were measuring the same person. The real population was **8 views from 7 visitors**.
  ...
  **The note is gated on the figure being non-zero (AECI-836).** Reporting an inference and standing a caveat over a day it never touched are different things. A zero day has no exclusion on the screen for the note to attach to, and a caveat with no referent trains the reader to skip the ones that have a referent. The email and the tile caption suppressed themselves at zero from the day they shipped; `/api/admin/overview` was the fourth surface and the only one that did not, until AECI-836. Contrast `corroborated_is_a_referrer_floor`, which stays unconditional by §13 D15(d) below: **gate a caveat when the thing it describes may not have happened; leave it standing when it describes what a permanently-present figure means.**
...
- **D18 — Writer provenance is a header we stamp, not a field the client sends** (settled by AECI-871 on 2026-09-11; §5.2, §9.6, `API_CONTRACTS.md` §6.9/§6.10, `DATABASE_SCHEMA.md` §9.1). D14 built `client_verdict` to catch a client that does not look like a browser. D16 then made it decide on its own, with no view floor. Neither noticed that the column had a **free upgrade built into it**: `classifyClientSignals` branched on the request body's `navigation` field, and `'spa'` returned `browser` outright, on the reasoning that "a same-origin `fetch` only happens because our JavaScript ran, which is itself proof of a browser".

  The premise was true and the inference was not. Nothing established that the request *was* a same-origin fetch except the request's own say-so, and `navigation` is a **body field** — `PageViewPayloadSchema`, sent by the caller. So any HTTP client could POST `{ route, navigation: 'spa' }` through the SSR `/api/*` passthrough and be handed the strongest verdict the system issues, with every arrival-shape check skipped. After D16 that verdict is also what keeps a row **out** of the digest's automation exclusion, which put a client-settable field directly in front of the headline figure — on the one column built specifically to catch the traffic that would want to forge it.

  **The seam already existed, and is reused rather than invented.** The SSR Worker is the sole writer of the `x-aeci-cf-*` headers and strips client-supplied copies before setting them (`withForwardedCfContext`). Provenance rides the same footing: `x-aeci-writer`, stamped `ssr-arrival` on the SSR Worker's own post-render capture and `browser-spa` on the proxied browser POST, stripped-then-set on the proxy path, with a closed two-value vocabulary so an unrecognized value reads as absent rather than as "some writer". It is stored as `page_views.writer_provenance` (migration `0031`, additive `ADD COLUMN`).

  **Three properties, each load-bearing.**

  *(a)* **Provenance alone is not enough.** It says our Worker proxied a POST, not that a browser issued one — `curl` through the passthrough collects the stamp too. A `browser-spa` write must ALSO carry what a same-origin `fetch` from our bundle sends: `Sec-Fetch-Site: same-origin`, plus `Sec-Fetch-Dest: empty` or `Sec-Fetch-Mode: cors`, plus `Accept-Language`. Those ride through the passthrough untouched, so their absence is meaningful rather than an artifact of the hop. `Accept` is deliberately not tested — Angular's `HttpClient` sends a JSON `Accept`, so the document-navigation rule is exactly wrong here, and pinning its literal default would couple a verdict to an HTTP-client implementation detail.

  *(b)* **Every failure is `unknown`, never `non-browser`.** A body claiming `'spa'` with no provenance, a provenanced write missing the fetch headers, an unrecognized header value: all `unknown`. We were told nothing we can act on, and that is not the same as being told it is automation — the same manufacture-no-verdict-from-an-omission rule D14 already applied to a writer that forwards nothing. It also bounds the blast radius: `unknown` is not in `NON_BROWSER_VERDICTS`, so **this change flags no row it did not flag before**. What it removes is unearned `browser` labels, not rows from the count.

  *(c)* **NULL is no evidence, as ever.** Rows written before 2026-09-11 carry a null `writer_provenance` and are not backfillable — nothing stored on an older row says which writer produced it, since `navigation` is precisely the claim being distrusted. `page-view-predicates.ts` needs no change: no digest or panel predicate keys off this column, so the D16 NULL-safety argument is inherited rather than re-derived.

  **Say what it cannot do.** A real headless browser passes every check here, on both paths, because it genuinely is a browser: Playwright driving Chrome sends `Sec-Fetch-Site: same-origin` on the tracker's POST like any visitor. That is correct rather than a gap. `client_verdict` names **evidence about a request, never an identity** — the same distinction D15(b) drew between a fact about the request and a judgement about who made it. What this column catches is the cheap client that does not bother, which is the majority and always was; what AECI-871 removes is the one branch where not bothering was free.

  **Report-only holds, for the fourth time.** Nothing here writes `is_bot`, no ASN joins `DATACENTER_ASNS`, and the headline stays an upper bound. The new column is an annotation and an admin filter axis (`GET /api/admin/page-views?writer=`), nothing more. The Activity feed's UI was unchanged at the time — it rendered no request-shape column, `client_verdict` included. **AECI-877 (2026-09-17) added that column** (writer plus verdict, §5.2) and the writer select, and put `client_verdict` on `AdminPageViewRow` to feed it.

- **D20 — The headline is a RESIDUAL and is named one; the day says whether its inputs arrived** (settled by AECI-869 on 2026-09-11; §5.1, §7.1, `POST_LAUNCH_MONITORING.md` §3a-ter, `docs/email.md`). D17 got the panel and the email to lead with one number. It did not ask whether the name on that number was true.

  **It was not, and AECI-868 is the proof.** From the 2026-09-07 promote until the gateway fix, the cache gateway forwarded every GET/HEAD to the cached `Renderer` with a `cf` object that **replaced** `request.cf` instead of merging into it, so every full-document arrival stored NULL for all six trusted network columns. `cf_asn` is the input to `DATACENTER_ASNS` classification, to both swarm ratios, to the ASN-rotator grouping and to D15(a)'s operator-pair retro-join — and every one of them treats NULL as *no evidence* rather than as an error. So the exclusions did not fail, they quietly stopped excluding. The headline read **364 / 699 / 680** on 2026-09-08/09/10, the email called all of it "human page views after automation", and four blind days read as a good week.

  Four changes, and they answer different halves of that.

  **(a) The class is renamed to "requests of unresolved origin". The figure is untouched.** Passing the crawler list, the AECI-658 header checks and the swarm thresholds is the **absence of a bot match**. That is only close to "a person" when the detectors have inputs, and the outage showed how far it can open. `humanViewsAfterAutomation` became `unresolvedRequests`; the **wire field stays `page_views_human`** and the **metric key stays `traffic.page_views_human_after_automation`**, deliberately — the key is `metrics_daily.metric` verbatim and renaming it would orphan every stored row, and the wire field had already been redefined once ten days earlier by D17. Both carry the explanation in their own docblocks instead. The word *human* survives on the **corroborated line alone**, which names its evidence in the same sentence; and D15(d)'s two caveats are joined by a third, that it is a **subset** of the headline rather than an addend to it.

  **(b) A telemetry-health line, on both surfaces, gated on the same ratio the nightly check uses.** `collectAnalyticsMetrics` reads `readArrivalCfCoverage` for the reported day **and the prior day** and puts both on `AnalyticsMetrics`, so the email, the tile envelope and the `arrival_telemetry_unavailable` note come from one measurement — the AECI-745 rule applied to a new figure. Below `ARRIVAL_CF_COVERAGE_MIN` it prints *"Arrival network telemetry unavailable for this day; network-based exclusions did not run."*, in the subject line as well as the body, because the failure mode is precisely that nobody opened anything. Absent when healthy, and **absent on a day with no arrivals**: `readArrivalCfCoverage` returns 1 for an empty window, and a warning that fires every quiet night stops being read.

  **(c) A NULL `cf_asn` group is never a network.** `detectNonBrowserClients` keeps the null bucket on purpose (D16 — those views are flagged per row and dropping them would lose them from the triage list), but `swarmNote` then counted `verdictCandidates.length` as a cardinality. On 2026-09-10 all 198 request-shape exclusions carried a NULL ASN and the email reported them as **"from 1 network"**. One SQL NULL group is not a small network or an unnamed one — it is the absence of the observation, and folding it into a count turns missing data into a measurement. `verdictNetworkPhrase` renders it as **"network unknown (N requests)"** beside the count, never inside it. The panel inherits the fix for free: `automation_filter_applied` embeds `swarmNote(...)` verbatim, so there is no second copy.

  **(d) The degraded day is recorded as a RATIO, and derived fresh every time.** `metrics_daily` gains `quality.arrival_cf_coverage` — a third family beside the flows and the stocks, in its own `ADMIN_SNAPSHOT_QUALITY_METRIC_KEYS` list rather than in `ADMIN_METRIC_KEYS`, for four reasons that are all consequences of the `traffic.` prefix: `snapshotSeries` rounds a stored value to an integer for the wire (0.94 → 1, i.e. a failing day passing), `ADMIN_RETROACTIVE_METRIC_KEYS` would re-check a value that cannot move, `metricSupportsInternalFilter` would offer to ASN-filter a measurement OF ASNs, and nothing charts it. **Never a `degraded` boolean**: the threshold is a documented launch tunable and `metrics_daily` is retained indefinitely, so a stored verdict would freeze today's bar into a permanent record that a later tuning could not re-decide. The reader compares; the row remembers.

  It is **backfillable**, which is what makes the outage legible after the fact. The `cf_asn` values are gone forever, but the NULLs are still in `page_views`, so `ops:backfill-metrics-daily` over 2026-09-07 → the fix day marks those days from the rows themselves rather than from a hand-kept list of dates. It is the one series that opts out of the zero-fill pass (`BackfillSeries.zeroFill`), because `0` is a coverage ratio's **worst** value rather than its empty one — an absent row reads as *not assessed*, a zero would read as a total outage on every day that simply had no arrivals. Skipping it does not re-open §7.4's prune hazard: the prune probes whether the DAY is captured, and the eight count series still fill it edge to edge.

  **What this deliberately does NOT do.** No threshold moved — `SWARM_MIN_VIEWS` and every ratio are untouched, because the four-view floor does not explain groups of hundreds and raising it hurts small-group recall; that work waits for AECI-868's data and the classification sub-issue. No population changed: `HUMAN` / `NOT_INTERNAL` / `OPERATOR_PAIR_MATCH` are byte-identical, so every stored day remains comparable with every other on the axis this issue did not touch. And **no historical row was written** — the `metrics_daily` backfill over the AECI-868 window is a queued data operation, sequenced after the code.

- **D21 — Browser starts are reported as STARTS, beside the other figures and never summed with any of them** (settled by AECI-870 on 2026-09-11; §5.1, §6.10, `ANALYTICS.md` §4/§9, `POST_LAUNCH_MONITORING.md` §0a-bis, `docs/email.md`). D20 established that the headline is a residual. It left open what, if anything, could be put beside a residual that a request-shaped bot cannot manufacture.

  **There already was one, and nothing read it.** `app_started` has fired since AECI-643 from `apps/web/src/app/analytics/analytics.ts` — Tier 2, no consent gate, `persistence: 'memory'`, once per full page load. It measures **successful execution of the browser bundle**, which a `curl`, a headless fetch or a rotating-proxy pool that never runs JavaScript structurally cannot produce. The AECI-660 join read only consented `$pageview`, so the one diagnostic in the stack that a non-browser client cannot fake never reached the operator. Adding new instrumentation before reading the existing beacon would have been the more expensive way to learn less.

  Five decisions, and the first four are all refusals.

  **(a) Starts, never people — and the schema is what enforces it.** Tier 2 mints a fresh anonymous distinct id on every page load, so PostHog resolves a fresh person behind each one: over the Sep 7–10 production sample persons ≈ starts, and `uniq(person_id)` over this event is noise dressed as an audience figure. `AdminBrowserStartsSchema` therefore carries **no people or session field at all**. A nullable one, or a documented "do not print this", would be a field someone prints. The test that pins it asserts the emitted HogQL does not contain `uniq(person_id)`.

  **(b) Two exclusions, and neither is `$is_identified`.** The operator is removed by a `$identify` retro-join — resolve the PostHog persons behind the Supabase user ids of every `profiles` row with `role = 'admin'`, over a 30-day trailing window, and exclude those persons. It **has** to be a retro-join: on the operator's own `app_started` rows `$is_identified` is FALSE, because the beacon fires at bootstrap and the merge happens later in the same session. An exclusion keyed on `$is_identified`, on country, or on a raw `distinct_id` match would exclude nothing and look like it worked, which is the failure mode this entry exists to prevent — the operator was **41 of 109 starts**. PostHog's own `$virt_traffic_type = 'Regular'` removes the clients it flagged (`AI Agent` is the value seen here); that is their verdict, not ours, and it narrows the population without making the result a human count. With no admin rows the subquery is **omitted entirely** rather than emitted as `NOT IN ()`, and the figure is honestly unfiltered.

  **(c) It is never summed, and the surfaces say so rather than merely not doing it.** `app_started` and `$pageview` overlap and one of them is not consent-gated; `app_started` and the D1 residual count different things entirely (bundle executions vs requests). So the email and the tile each print the sentence *"never added to or subtracted from any figure above"*, and `analytics-digest.spec.ts` asserts the absence of the specific numbers the arithmetic would produce — 680 − 13, the ratio, the "of which". A rule that is only obeyed is a rule the next change breaks; a rule that is asserted is not.

  **(d) A successful start proves execution, not humanity, and the number has a blocker-shaped hole.** A real headless browser produces a start. And the client posts straight to `us.i.posthog.com` with no reverse proxy, so a browser running a tracker blocker emits nothing — this is a floor with an unmeasured gap, not a census. Both caveats ride the figure on every surface. The strongest thing in the set is the **search-referred subset**: a named search referrer plus a proven bundle execution, which is two independent signals rather than one. 17 of the Sep 8–11 starts carried a google.com or bing.com referrer, all on product or pair pages.

  **(e) On the panel it is `?recompute=1`-gated, and null is never a zero.** The read is one external HTTP request, and §13 **D8** already draws the line that the default dashboard load makes none. So a default load returns `browser_starts: null` under the existing `requires_recompute` note, and only a recompute queries. When a recompute's read fails, `browser_starts_unavailable` carries the transport reason (`posthog_http_503`, `posthog_credentials_missing`, `admin_lookup_failed`) and the caption prints that instead of a number. **Zero is a real value and a real finding** — zero starts on a day with arrivals is a broken bundle or a blocked collector, which is the reading that matters most — so it must not be the shape a failure takes. That reason is a free-form string rather than an `AdminNoteCode`: every code in that enum is fixed vocabulary with a matching UI string, and a vendor status code is not vocabulary.

  **What this deliberately does NOT do.** No new instrumentation, no persistence upgrade, no identifier, no cookie: reading an existing operational event is not permission to make it a visitor tracker, and the Tier 2 contract in `ANALYTICS.md` §5 is untouched. **No migration, no schema change and no `metrics_daily` key** — the figure is not charted, not delta'd and not stored, because it has only existed in production since **2026-09-07** and a series that begins at a deploy reads as a traffic event. The D1 headline population is byte-identical. And per-document acknowledgment — proving a specific page render reached a specific browser — was explicitly left out: it is a separate measurement, consent and cache decision, and it belongs to the classification sub-issue only if aggregate starts turn out to leave real uncertainty.
...
> Nothing below is built, and **no build starts until this text is agreed or amended**. The
> deliverable of AECI-872 is the decision, not the code. Read the four reporting classes as a
> proposal that competes with today's `is_bot` / `client_verdict` pair, not as a description of it.

- **D19 — Versioned, evidence-preserving traffic classification** (PROPOSED by AECI-872 on 2026-09-11; §5.1, §5.3, §6, and `POST_LAUNCH_MONITORING.md` §3c, `DATABASE_SCHEMA.md` §9.7, `API_CONTRACTS.md` §6.10 — each of those three also PROPOSED under this issue). D14 through D18 built five good detectors and one bad record. Every classification this system makes today writes either a **binary answer** (`page_views.is_bot` — one column, decided once, no reason and no version) or a **report-side exclusion** that is recomputed on every read and stored nowhere (`AutomationExclusion`, `NOT_INTERNAL`). Neither form records which rule fired, what version of that rule it was, or what evidence it saw. Three consequences, and all three bit in the same week.

  **(1) A drop in flagged traffic cannot be told from a drop in evidence.** The digests for **2026-09-08 through 2026-09-11** reported less automation, and nothing in the record says whether the automation left or the detectors went blind. The mechanism is verifiable in the tree. `applyCfContextHeaders` (`apps/web/src/server-runtime.ts`) opens with `const cf = …; if (!cf) return;` — an arrival whose request carries no `request.cf` emits no ASN header at all, and the row lands with `cf_asn = NULL`. Such a row is then invisible to three separate things at once: `classifyTraffic(ua, asn)` (`lib/bot-classification.ts`) can only consult the UA half, `detectAsnRotators` (`lib/swarm-detection.ts`) filters `cf_asn is not null` before it groups, and `OPERATOR_PAIR_MATCH` (`lib/page-view-predicates.ts`) is NULL-safe in the KEEP direction so the row survives the operator exclusion too. A null ASN therefore reads as *clean human traffic* on every surface, and no note anywhere says the input was missing. The ingest defect itself belongs to **AECI-868**; what D19 claims is narrower and structural — **the record had no way to express "this row was classified without its network."**

  **(2) Thresholds cannot be compared across days.** `scheduled.ts` writes the detector's *outputs* into `job_runs.detail` (`swarmCandidates`, `asnRotatorCandidates`, `swarmFlaggedViews`, `swarmTruncated`, `verdictFlaggedViews`, `nonBrowserCandidates`). It writes none of the **constants those outputs depend on**. A day flagged under `SWARM_MIN_ASN_RATIO = 0.8` and a day flagged under some later value are stored in the same shape, in the same column, with nothing distinguishing them. `POST_LAUNCH_MONITORING.md` §3b already states the sharp end of this: a threshold change moves `traffic.page_views_human_after_automation` for every stored day with no change to any raw count, so the AECI-827 re-check's trigger set is empty, and that key sits in `NOT_BACKFILLABLE` so no repair tool exists. Today the doc's answer is "record the date in the health report". That is a human remembering, not a record.

  **(3) Nothing can be evaluated against a labelled sample.** There is no held-out set, no false-exclusion rate, and no measured cost of a rule. Every retune in D14 through D16 was justified by a hand-decomposed production day — good evidence, and not a repeatable one. §3b says so about itself: the ASN rotator's "fires exactly once in 30 days" is **two days of evidence, not thirty**, because `client_verdict` did not exist before 2026-08-26 and the gate reads NULL as no evidence.

  **The decision is to stop asking one column to hold a judgement, and to record the judgement as its own versioned, evidence-bearing artifact.** Eight parts follow.

  **(a) Name the business question, because three different answers are all correct.** The question the operator is actually asking is *"how much credible external audience activity did we observe?"* It has no single number, and a single headline hides that. **Requests** counts what arrived at the edge; it is the only exact figure, and it counts a crawler's tenth page as equal to a person's first. **Browser executions** counts documents where our JavaScript demonstrably ran; it is a floor, it is consent-shaped on the PostHog side, and a real person who declines consent is invisible in it. **People, or sessions** is what a reader means by "visitors"; we cannot measure it — §9.8's `(user_agent_hash, cf_asn)` pair over-counts on browser update and under-counts behind shared NAT, and D7 forbids the durable identifier that would fix it. D19's rule: **every reported figure states which of the three it is**, in the same breath as the number. §5.1's measurement-envelope caption is the existing pattern; this generalizes it.

  **(b) Four reporting classes, with internal traffic reported separately rather than folded into any of them.** They are ordered by how much the evidence supports, not by how suspicious the traffic feels.

  | Class | What it means | What is NOT sufficient |
  |---|---|---|
  | **Identified automation** | The client says what it is, or the edge verifies it. Verified crawler identity **where available**, with a **self-declared** crawler UA labelled separately from a verified one — the first is a claim, the second is a check. Split by purpose where the split is useful: indexing · AI training · assistant retrieval · link previews · monitoring. | A guess. Nothing enters this class on inference. |
  | **Suspected automation** | **Multiple consistent signals** agree, and the record names **which** ones. Carries reason codes and the decision version, so the finding can be re-read later. | **Network ownership alone never settles identity, and neither does a shared user-agent.** That is D10's rule about `DATACENTER_ASNS`, D15(c)'s rule about shared NAT, and D15(a)'s rule about the operator's own hash, stated once instead of three times. |
  | **Browser activity with supporting human evidence** | Our app was observed executing, **and** there is plausible navigation or a meaningful action, **after** internal exclusions and automation checks have run. | Certainty. This class is **still heuristic** and must never be labelled "humans" unqualified. It is the successor to today's headline and inherits its caveats rather than shedding them. |
  | **Unresolved** | Evidence missing, insufficient, or **conflicting**. | Nothing. This is where **ordinary headers and the absence of a bot match live** until something corroborates them. A row is not human because no rule matched it. |

  **Internal traffic is a separate report, not a class.** The operator's own views are excluded by *identity*, not judged by evidence (D12, D13, D15(a)); folding them into "identified automation" would be a category error that also makes the exclusion invisible.

  **The point of the fourth class is that it is allowed to be large.** Today every unmatched row silently becomes a human. Under D19 a growing unresolved share is a **finding** — which is exactly the signal the 2026-09-08 window had no way to raise.

  **(c) Observations and classifications are different objects with different lifetimes.** The captured facts of a request — path, timestamp, headers as recorded, network, hashes — stay **immutable** on `page_views`, and no classifier rewrites them. A classification is a **separate versioned record** carrying the class, the reason codes that produced it, the rule or model version, the time it was evaluated, the evidence window it looked at, and an explicit **unknown** rather than a default. `DATABASE_SCHEMA.md` §9.7 carries the proposed shape. Three properties follow, each load-bearing:

  - **Re-evaluation is free and history survives it.** This is D14's own annotation argument — `is_bot` is decided once and costs a one-way backfill, an annotation can be re-read for free — applied to the verdict itself rather than only to its inputs.
  - **One population definition, shared.** The digest and every admin breakdown read the same classification records, which is D17's rule kept exactly. A number on two surfaces is one query, not two agreeing queries.
  - **Days evaluated under different classifier versions are FLAGGED, never blended.** A chart spanning a version change shows the seam. It does not silently average two definitions, which is what `traffic.page_views_human_after_automation` does today.

  **(d) The shadow-mode gate — a new classifier reports beside the old one before it replaces it.** No version becomes the reported one on the strength of a decomposed day. It runs in parallel against a **reviewed sample** that must contain all six of: labelled operator sessions; known bots; **privacy-hardened browsers** (DNT / GPC set, no `Sec-CH-UA`, blocked third-party requests), the population most at risk of being labelled automation for being careful; ambiguous shared-proxy cases; real traffic with unknown labels; and **held-out days the tuning never saw**. Three metrics decide, reported together: **false exclusions** (real browser activity classified as automation), **missed automation**, and **unresolved share**. Tuning happens from those results, not from a day's narrative. The procedure is `POST_LAUNCH_MONITORING.md` §3c.

  **(e) The capture additions worth keeping are small, and none of them is a fingerprint.** D7's prohibition and D14's "carries no identity" constraint both hold unchanged.

  - A **metadata-availability flag per row** — did this row have its network, its headers, its referrer at all? This is the one that would have answered (1).
  - The **matched crawler token**, stored beside the verdict, so "which rule named this Googlebot" is a fact rather than a re-derivation against a regex list that has since changed.
  - **Browser family**, only where a specific read justifies it. `page_views` is the hottest write path in the app and D1 bills indexes as rows written (`DATABASE_SCHEMA.md` §9.1), so speculative columns are not free.
  - **Classifier version and reason codes**, which is the whole point.
  - **Whatever trusted edge bot signal Cloudflare Pro actually exposes to `request.cf`.** *This is UNVERIFIED and must be checked against the live plan before any plan depends on it.* What is verified today: `cf_bot_score` is Enterprise-only and is null on every row we write, `cf.tlsVersion` / `cf.httpProtocol` are available on Pro and are already captured, and `PAGE_VIEW_CF_HEADERS` carries no verified-bot field. Treat a verified-crawler signal as a **question for the plan**, never as an input the design assumes.

  **(f) Two decisions are DEFERRED, and each gets its own spec text before it is built.** Recorded here so their absence reads as a decision rather than an oversight.

  - **A short-lived per-document acknowledgment.** A token scoped to one document render that proves the page executed, with **no durable visitor id**. Deferred because it needs an explicit **measurement, consent and cache contract** first: what it measures that `navigation = 'spa'` does not, whether it is consent-scoped, and how it coexists with a URL-keyed edge cache that must render visitor-state-neutral HTML. Without that contract it is a session id wearing a different name, which D7 and §9.7 both forbid.
  - **AI cohort triage.** Deferred as a **supervised investigation aid** — a way for the operator to ask "what is this cluster?" over rows already on screen. Promoting it to a *repeatable classifier* requires four things this repo does not have: a supported inference integration, saved inputs, recorded model and prompt versions, and an evaluation against the (d) sample. One rule is absolute either way and does not wait for that spec: **unreviewed LLM output never writes `is_bot`, and never writes a classification record's class.** It may propose; a human decides — the same bar D16 already sets before an ASN joins `DATACENTER_ASNS`.

  **(g) Constraints, all inherited rather than new.** **Cloudflare Pro only** — no plan upgrade, and no change to any blocking policy in order to improve a report. A WAF or bot-mode change alters who reaches the site, which is a product decision, and `waf-rate-limits.md` §3b already records how invisible those settings are from the repo. **No new public surface**; this is operator-facing and digest-facing only. And **the 2026-09-07-to-fix window stays degraded** — it is not repairable, because the missing header was never stored — so the classifier must be **able to say so**: a day whose rows lack their network reports as degraded, not as quiet.

  **(h) What this supersedes, and what it keeps.** D19 supersedes nothing that shipped. `is_bot` keeps working, the three detectors keep working, and the four classes are a **reporting layer over** them rather than a replacement for them — which is the only way to introduce this without the one-way backfill of the column D14 spent its whole entry arguing not to touch. What it keeps, explicitly:

  - **The shared-predicate discipline of AECI-683 / AECI-744 / AECI-745.** One population, one definition, imported rather than re-derived (`lib/page-view-predicates.ts`). A classification record is a second *reader* of that population, never a second definition of it.
  - **The report-only rule of AECI-658 (D14).** Nothing in D19 writes `is_bot`, and no ASN joins `DATACENTER_ASNS` on classification evidence. A false positive still silently deletes a real visitor, and that is still worse than counting a crawler.
  - **D15(b)'s gate-the-caveat rule.** Gate a caveat when the thing it describes may not have happened; leave it standing when it describes what a permanently-present figure means. Reason codes make that mechanical instead of per-surface.
...
`lib/data-quality.ts` (04:00 UTC), **fifteen** checks: `products_without_vendor` · `taxonomy_missing_description` (AECI-962, `error` — a live taxonomy term in any of the five tables with a NULL or blank `description`, which is the shape promote's find-or-create mint writes; see §5 of `DATABASE_SCHEMA.md`) · `promotion_status_invariant` (AECI-592, `error` — every `products` and `vendors` row must read `promotion_status='promoted'`) · `vendors_without_products` · `reviews_missing_anonymized_at` · `stale_stats_cache` · `duplicate_vendors` · `duplicate_products` · `logo_404` · `algolia_index_drift` · `entitlement_mirror_drift` (AECI-609, `error` — the §2.1 `vendors.verified` mirror guard) · `arrival_cf_coverage` (AECI-868, `error`) · `retired_integration_unclaimed` (AECI-1010, `error` — a retired integration that is not vendor-held: no `claimed_at` and `origin <> 'vendor'`. It was retired-implies-claimed until AECI-1046 widened it, because an admin may retire a vendor-created row whose claim was cleared. It cannot be a CHECK constraint without recreating `integrations`) · `vendor_integration_unclaimed` (AECI-1011, `warn` — a vendor-created integration with no `claimed_at`, which an AECi `owner` accept that reassigns the row produces, and since AECI-989 so does revoking the creator's last seat; promote can never write such a row, so until the new owner claims it nobody can edit it, and only an AECi admin can retire it (AECI-1046)) · `landing_cf_coverage` (AECI-876, `warn` — the lead-capture sibling of `arrival_cf_coverage`, over `mailing_list.asn`).

**This list is the enumerated inventory, and it is the only place that states the count.** AECI-592 found the number stated nineteen ways across code and docs, drifted twice in two months (AECI-609 and AECI-868 both landed without updating it). Prose elsewhere now says "the §23.1 data-quality checks" with no number. Do not reintroduce one.

**Two checks watch the telemetry pipeline rather than the catalog**, and both are read differently from the other twelve. `arrival_cf_coverage` is the original. It fails when full-document arrivals exist in the last 24 h and fewer than **`ARRIVAL_CF_COVERAGE_MIN` = 0.95** of them carry a `cf_asn`; it passes when there were no arrivals, because an empty window is not a defect. **Its `count` is 1 when tripped, not a row count** — the finding is a ratio, and the numbers live in the line and the note rather than on the `aeci.data_quality.check` gauge, where a NULL-row count would track traffic volume instead of severity. A failing run means the day's traffic figures are unusable (§9.8's degraded-window note), not that a row needs editing. The read is `lib/arrival-coverage.ts`; `POST_LAUNCH_MONITORING.md` §0b is the operator procedure. *(This list read "ten" until AECI-868; `entitlement_mirror_drift` had shipped in AECI-609 and was never added. AECI-592 then folded `ready_products_unpromoted` and `broken_integration_refs` into `promotion_status_invariant`, taking it to eleven; AECI-962 added `taxonomy_missing_description`, taking it to twelve; AECI-876 added `landing_cf_coverage`, taking it to thirteen; AECI-1010 added `retired_integration_unclaimed`, taking it to fourteen; AECI-1011 added `vendor_integration_unclaimed`, taking it to fifteen.)*

`landing_cf_coverage` (AECI-876) is the second, and it watches the **other** path Cloudflare context reaches D1 by. `POST /api/subscribe` fills seven `mailing_list` columns from the trusted `LANDING_CF_HEADERS` the SSR Worker sets, and nothing checked that they arrived. It probes `mailing_list.asn` over a **30-day** window, passes below a **10-row** minimum, and fails under a **0.90** floor. Three things differ from its sibling, all deliberate. **Severity is `warn`, not `error`** — this column's only reader is the §5.4 `/admin/audience` ASN breakdown, a descriptive split no other figure is built on, so a failure is a lost attribute rather than an unusable day, and `warn` routes it to the dashboard and the digest instead of the ERROR alert. **The window is 30 days and carries a row minimum**, because `mailing_list` is deduplicated by email and a day's signups are not a denominator a ratio can stand on; the floor and the minimum were chosen as a pair so one legitimate NULL cannot trip it. **Detection latency is therefore days, not hours** — at a few signups a day a total loss takes roughly three to five days to pull the window under the floor, and no threshold buys both low-volume stability and same-day detection. Its `count` is 1 when tripped, not a row count, for the same reason as its sibling. `feedback` is deliberately not probed: it stores no `asn` column at all, and both tables are filled by the same `readLandingCfFromHeaders` call on the same hop, so one probe covers both.
...
> **Closed out 2026-08-14 by AECI-587**, except one bullet left consciously open and owned
> elsewhere. **That last bullet closed 2026-09-13 with AECI-592** — all seven are now resolved.
> Disposition is recorded per bullet.
...
- `data-quality.ts` check #2's inline note that there is no `promoted_at` column ~~is accurate, but~~ **is now itself false** — AECI-581 added the column (migration `0011`) — and it always **understated the problem**: the check filters `promotion_status='ready'`, and nothing in the repo ever writes `'ready'` to D1, so it can only ever return zero rows. It is unreachable, not proxied (§13 D6). **Partly closed 2026-08-14 by AECI-587**: both comments (the file docblock and the inline note) now said the column exists and that the check is structurally dead rather than approximate. **Fully closed 2026-09-13 by AECI-592**, which deleted the check and replaced it with `promotion_status_invariant` (severity `error`, both tables). AECI-592 also found that `broken_integration_refs` was dead for the identical reason and folded it into the same guard, so §14.1's roster went from twelve to eleven. §23.1 of `STAGE_1_SPEC.md` carries the dated amendment; this section now has **no** open items.
...
| Issue | What | Status |
|---|---|---|
| **AECI-590** | Reverse-proxy PostHog to recover blocker-lost events (D9) | **Measured 2026-09-13** — the population sentence in this row was pre-AECI-643 and is wrong; Tier 2 boots for every visitor. What settled it is volume, not scope: ~16 real browser page loads a day, so recovery is 2–5 a day. See the D9 correction above |
...
| **AECI-592** | `data-quality.ts` check #2 is unreachable; replace it with a promotion-status invariant guard | **Done 2026-09-13.** AECI-587 corrected the comments only; AECI-592 replaced the query, and folded in `broken_integration_refs` (dead for the same reason) |

**Found after closeout, while fixing AECI-752 (2026-09-09).** The first two concern note *prose*,
which §6 item 3 deliberately keeps out of this spec — `code` is the contract and the English
sentence is the UI's. That division is correct, and it has one cost worth writing down. The third,
added 2026-09-11, is the same root cause reaching *behaviour*.
...
- **A note's EMIT CONDITION has no mechanical link to the figure it qualifies either — fixed
  ...
  `operator_leak_is_an_inference` fired whenever `/api/admin/overview` asked for it, with no check
  on `operator_leak_excluded`. So on a day the retro-join removed nothing, the response carried a
  caveat explaining how an exclusion had been inferred, with no exclusion on the screen and no
  number beside it. **Three of the four surfaces already gated** — `analytics-digest.ts` in both
  its text and HTML renderings, and the tile caption at `overview.ts` — and the fourth diverged
  silently, because nothing links the four and no test pinned any of them to each other. The fix
  passes the count rather than a flag (`operatorLeakViews?: number`, `undefined`/`0` emits
  nothing) so the note and the figure come from one value and cannot drift; `params.rows` repeats
  it on the wire. The general rule, now recorded at both the emit site and in D15(b): **gate a
  caveat when the thing it describes may not have happened; leave it standing when it describes
  what a permanently-present figure means.** By that test `corroborated_is_a_referrer_floor`,
  `referrer_source_is_unverified` and `direct_is_mixed_bucket` all correctly stay unconditional —
  AECI-836 checked each and changed none. The residual is that the rule is a comment and a
  convention, not a mechanism: nothing stops the next gated note being written ungated.

### Logo controls on vendor detail and product roster (AECI-955)

Vendor detail includes an inline logo editor. Each product row has an Edit logo disclosure with the same control, in the roster's **Actions** cell beside View page — never in the row's `<th scope="row">`, which every data cell in that row inherits as its header and would therefore re-announce six times per product. Both use `aec-logo-input` for HTTPS URL or validated raster upload, preview and removal, followed by explicit Save logo. Upload alone does not publish a catalog change. Upload and save status goes through the page's existing polite live region. No entitlement is required for an admin.

| Endpoint | Purpose |
|---|---|
| `POST /api/admin/logo` | One bounded multipart file to private R2; returns a public logo path, no D1 write |
| `PATCH /api/admin/vendors/:id/logo` | Save or clear only the vendor logo and claim admin ownership |
| `PATCH /api/admin/products/:id/logo` | Save or clear only the product logo and claim admin ownership |

All require requireAdmin and the authenticated write limiter. The new routes reject cross-origin browser writes. PATCH validates `{logo_url}` strictly, audits before/after in the same batch, and purges affected page tags after commit. Product roster responses now include nullable logo_url. Full contract: STAGE_2_5_SPEC.md §11. This is the seventh exception in §2.
