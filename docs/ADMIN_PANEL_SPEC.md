...
`lib/data-quality.ts` (04:00 UTC), **eleven** checks: `products_without_vendor` · `promotion_status_invariant` (AECI-592, `error` — every `products` and `vendors` row must read `promotion_status='promoted'`) · `vendors_without_products` · `reviews_missing_anonymized_at` · `stale_stats_cache` · `duplicate_vendors` · `duplicate_products` · `logo_404` · `algolia_index_drift` · `entitlement_mirror_drift` (AECI-609, `error` — the §2.1 `vendors.verified` mirror guard) · `arrival_cf_coverage` (AECI-868, `error`).
