...
| Item | Why |
|---|---|
| Run `scripts/ops/backfill-page-view-bots.sql` on production | 17,784 rows currently read as human; every historical traffic chart is wrong until this runs. **Now also a hard prerequisite for the §7.1 metrics backfill**, which refuses a range containing unclassified rows — `metrics_daily` is kept indefinitely, so backfilling first would freeze the wrong human/bot split permanently |
