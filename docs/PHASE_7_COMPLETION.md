...
**Phase 7 is functionally complete and Stage-1 launch-ready in code.** Ten of the eleven Phase 7 build issues are merged to `main` and `Done` in Linear (7.1–7.9, 7.11 = AECI-236/237/238/239/240/241/242/154/243/245); the eleventh, **7.10 manual screen-reader pass (AECI-244)**, is a human QA task carried as **Done for this gate per Chris's sign-off** (the on-instruction "continue as if 7.10 has been completed"), with the repeatable procedure now documented in `docs/a11y-manual-testing-checklist.md` (§4.4). Every launch surface exists and renders: IndexNow fires on promote *(as shipped. It did not actually reach Bing or Yandex — every production submission returned HTTP 429; AECI-826 rebuilt it as a buffer plus a drain cron)*; the four legal pages + About + Contact are routed, cached, i18n-wrapped and footer-linked; PostHog is client-only with locale/theme dimensions; Resend carries the review/account/admin templates + the Supabase magic-link SMTP; the daily 04:00 UTC data-quality job runs the full §23.1 suite and emails a digest; the two Pro-plan WAF rate-limit rules + scraper block are live with an hourly Datadog poll; the non-blocking BrowserStack lane is wired; the waitlist welcome banner + token attribution ship; and the pre-launch Core Web Vitals audit is documented. All admin surfaces are token-only, i18n-wrapped, and light-only.

> **Addendum — 2026-09-09 (AECI-244): 7.10 was waived at this gate, and has now actually been run.**
> The verdict above is left intact as the historical record, but its "carried as Done per Chris's
> sign-off" is exactly what it says: a **waiver**, not a pass. No screen-reader or keyboard pass was
> performed for this checkpoint. The public-site pass ran on **2026-09-09** against production
> `44aba9cf` and is recorded in **`docs/ACCESSIBILITY_AUDIT.md`**.
>
> **What it found.** The keyboard layer is genuinely clean — no traps, no focus on hidden elements, no
> unnamed interactive elements, a visible focus indicator on every stop, across full `Tab` walks of
> home and product detail. But it found **three serious WCAG 2.1 4.1.3 Status Messages (AA) failures**:
> `/auth/login`, `/products/:slug/review` and `/account` each replace the view on a state change with
> no live region and no focus move, so a screen-reader user gets no confirmation and loses their
> reading position. **Every automated gate misses this class**, because the defect only exists after a
> form submission and axe never submits.
>
> **AECI-244 is still open.** The VoiceOver and NVDA speech layer, dialog focus management, and the
> review-form `Tab` walk remain — see `ACCESSIBILITY_AUDIT.md` §6 and the checklist §6/§7 scripts.
...
| 3 | ~~PostHog + Loops~~ **PostHog + Resend** live; data-quality job running + emailing; IndexNow firing on writes | ⚠️ | **Code shipped ✅, live requires secrets ⚠️.** "Loops"→Resend/PostHog (§1). PostHog `analytics/posthog-client.ts` (gated `POSTHOG_KEY`); Resend `lib/email.ts` (gated `RESEND_API_KEY`); data-quality `scheduled.ts:151` `DATA_QUALITY_CRON='0 4 * * *'` → `runDataQualityJob` (`lib/data-quality.ts` + `data-quality-email.ts`); IndexNow `promote.ts` `notifyIndexNowAfterPromote` → `lib/indexnow.ts` (gated `INDEXNOW_KEY`+`PUBLIC_SITE_URL`) — *renamed to `bufferIndexNowAfterPromote` and no longer submitting, AECI-826; the `*/20` drain cron does.* **Secrets/deploy → §F2.** |
| 4 | WAF rate limits active; cross-browser smoke (BrowserStack 7.8) green; **screen-reader pass done**; perf/CWV within budget | ⚠️ | **WAF active ✅** (`docs/waf-rate-limits.md` — 2 Pro rules + scraper block live; hourly Datadog poll `scheduled.ts:160` `WAF_CRON`, needs `CF_ANALYTICS_API_TOKEN` → §F2). **BrowserStack ⚠️** — lane wired, inert until `BROWSERSTACK_*` → §F4. **Screen-reader ⚠️ — waived here, run later.** Carried Done at this gate per Chris's instruction; the pass itself ran **2026-09-09** and found three serious AA failures → `docs/ACCESSIBILITY_AUDIT.md`. See the 2026-09-09 addendum in §1. **Perf/CWV ✅** `docs/PERFORMANCE_AUDIT.md` (AECI-245); warn-level Lighthouse perf misses noted there. |
...
|-----------|-------|----|--------|----------|
| 7.1 IndexNow on the write-event pipeline (§20.2/§20.5) | AECI-236 | #380 | ✅ **superseded in part** | `apps/api/src/lib/indexnow.ts` (`callIndexNow`, gated `INDEXNOW_KEY`, never throws); `routes/promote-indexnow-urls.ts` (`affectedUrlsForPromote`); wired post-commit in `routes/promote.ts` (`ctx.waitUntil`, reads `INDEXNOW_KEY`+`PUBLIC_SITE_URL`). Metric `aeci.indexnow.submit`. **AECI-826 replaced the per-promote submit with a buffer plus a `*/20` drain cron** — the hook is now `bufferIndexNowAfterPromote` and writes `indexnow_queue`; `lib/indexnow-drain.ts` submits. As shipped here the channel failed 23 of 23 submissions with HTTP 429 across 2026-09-07..09; read `STAGE_1_SPEC.md` §20.2 for the current design. Google Indexing ping was the sibling AECI-263, deleted in AECI-747. |
...
| 7.10 Manual screen-reader pass (VoiceOver/NVDA; §21.3) | AECI-244 | — | ⚠️ waived here; run 2026-09-09 | Carried Done for this gate per instruction (§1). Repeatable procedure documented in `docs/a11y-manual-testing-checklist.md` (§4.4): home / product detail / review submission / login / account / admin queue; VoiceOver + NVDA + keyboard-only. axe + Lighthouse a11y≥95 already automated (AECI-65). Linear status → §"Hand-off". |
...
**Linear housekeeping:** AECI-246 moved to **In Progress** at start (assignment: the claude.ai Linear connector needs re-auth this session — see the closing note). **AECI-244 (7.10)** is carried Done for this gate per Chris's instruction. **Do not flip AECI-244 to Done** — superseded by the 2026-09-09 addendum in §1: the pass was never run at this gate, it ran on 2026-09-09, and it stays open until the VoiceOver/NVDA layer is done.
...
2. ~~7.10 (manual screen-reader pass) is signed off per the §4.4 checklist, and AECI-244 flipped to Done.~~ **Superseded 2026-09-09** — see the addendum in §1. 7.10 was waived, not performed; AECI-244 stays open.
