...
- If the diff changes a surface the product docs describe (`apps/web/src/content/docs/**`, rendered at `/docs/*`), does the matching docs page change in the same PR? The vendor guide names portal controls by their labels, so a relabelled button makes it wrong. This is the sync rule of `STAGE_2_PRODUCT_DOCS_SPEC.md` §4 (added with the vendor tranche, AECI-1104).
...
- `Lint: ✅` Dark-theme surface reintroduced: a `dark:` Tailwind variant, a `.theme-dark` block, a `@custom-variant dark` re-declaration, a `prefers-color-scheme: dark` query, or `[data-theme]` switching. AECi ships a single light theme (AECI-226) and dark theme is not roadmapped — the Stage 2 reintroduction was dropped (`STAGE_2_SPEC.md` §9); if it is ever revisited it returns as a semantic-token block, not as scattered utilities. Enforced two ways (AECI-549): `no-restricted-syntax` covers `.ts` including inline templates, and `apps/web/scripts/check-source-constraints.mjs` covers external `.html` and `.css`, which ESLint structurally cannot read as class strings. (This item previously read "renders correctly in light but not dark" — that predates AECI-226 and no longer applies.)
- `Lint: ✅` Color hardcoded instead of using a theme token (`--surface-base`, `--text-primary`, etc.) — a hex literal, `rgb()`/`hsl()`/`oklch()`, a raw Tailwind palette class (`bg-zinc-100`), or `text-white`/`bg-black`. See `ANGULAR_STYLE_GUIDE.md` §20. Enforced two ways since AECI-597, the same split as the dark-theme item above: `no-restricted-syntax` covers `.ts` including inline templates, and `apps/web/scripts/check-source-constraints.mjs` covers external `.html` and `.css`. Scoped to **apps/web** — `apps/api`'s transactional email HTML is legitimately hex, because email clients don't support CSS custom properties. (This item read `Lint: 🟡 review-only` between AECI-549 and AECI-597, and before AECI-549 `STAGE_1_PHASE_2_SPEC.md` claimed an enforcement that did not exist.)
- Still your job here: **which** token. `rgb(0 0 0 / α)`, `bg-transparent`, and `border-transparent` are deliberately allowed, and lint cannot tell you that Clay is over its 5%-of-a-screen budget, that Bone is being used as a page background, or that an accent pairing fails WCAG AA. See `DESIGN.md` §Named Rules.
