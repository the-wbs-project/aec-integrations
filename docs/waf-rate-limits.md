...
| `POST /api/vendor/integrations/:id/contests`, `POST /api/vendor/contests/:id/withdraw`, `POST /api/vendor/contests/:id/decision` (AECI-1008) | `write` (by user) | The same reason as the row above. An owner's accept writes the catalog and purges the pair's cache tags. Since AECI-1092 the submit route also takes a connector-evidenced pair's id (the handler finds the table, as the claim and the edit do on `/integrations/:id`), so contests on pairs share this entry and need no new one. `GET /api/vendor/contests` is a read and carries no limiter (§6.3) |
| `POST /api/vendor/integrations/:id/claim` (AECI-1005) | `write` (by user) | A catalog write: it stamps `claimed_at`, transfers maintenance and purges the pair's cache tags. Registered after `requireVendor()`, so the key is the user, like every vendor write |
| `POST /api/vendor/integrations/:id/retire`, `POST /api/vendor/integrations/:id/restore` (AECI-1010) | `write` (by user) | Catalog writes: each hides or re-shows the row on every public read, recomputes both endpoint counts in the batch, runs a by-id Algolia sync and purges seven cache tags. Registered after `requireVendor()`, so the key is the user |
| `POST /api/vendor/integrations` (AECI-1011) | `write` (by user) | A catalog write that goes live with no moderation: it inserts an integration, recomputes both endpoint counts in the batch, runs a by-id Algolia sync of four records, purges seven cache tags and queues three URLs for re-crawl. It is also the one vendor write that adds to `integration_count`, a ranking input (`SEARCH_RANKING.md` §3.1), so an unbounded loop here is a ranking lever as well as a purge and Algolia loop. Registered after `requireVendor()`, so the key is the user. The `GET` on the same path is a read and carries no limiter (§6.3) |
| `PATCH /api/vendor/integrations/:id` (AECI-1006) | `write` (by user) | A catalog write that goes live with no moderation: it writes the owner's fields, transfers maintenance, runs a by-id Algolia sync of the integration and purges the pair's cache tags. An unbounded loop here is both an edge-cache purge loop and an outbound Algolia loop. Registered after `requireVendor()`, so the key is the user. `GET /api/vendor/integrations` is a read and carries no limiter (§6.3) |
| `PUT` / `DELETE /api/vendor/integrations/:id/links/:productId/:kind` (AECI-1007) | `write` (by user) | The same reason: each write transfers maintenance and purges the pair's cache tags. The links reach readers through `GET /api/vendor/integrations`, a read, which carries no limiter |
...
| **every `requireAdmin()` write except the six below** | none | Hand-granted role with no anonymous path to it, and every write emits an `audit_log` row in the same batch. A limiter would risk 429-ing a moderation burst, which is the legitimate workload |
...
fan out to seven scope refetches (six before AECI-1008 added `contests`), so a limiter on it trips inside a minute — and the failure
