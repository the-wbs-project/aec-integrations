...
|---|---------------|--------|----------|
| 1 | Every page in §3.1 renders with real data from Supabase | ✅ | Routes `apps/web/src/app/app.routes.ts` (products, vendors, integrations, categories, `categories/:slug`, `audiences/:slug`, `phases/:slug`, `integrations/:id`, 404 `**`); per-page resolvers via `core/create-detail-resolver.ts`; e2e: `products-{index,detail}`, `vendors-{index,detail}`, `integrations-{index,detail}`, `taxonomy-index`, `taxonomy-browse`, `not-found`. *(Live data presence is env-seed-dependent — see §5, note A.)* |
