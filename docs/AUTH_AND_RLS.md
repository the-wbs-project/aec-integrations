...
| **#4b** claimant provisioning | Create an `auth.users` row when the claimant — or, since **AECI-740**, the operator-named seat holder — has no account yet. | `lib/supabase-admin.ts` → `createAuthUser` | `POST /auth/v1/admin/users` | as #4a |
...
including the role/vendor exclusivity check (§3.2, `STAGE_2_SPEC.md` §8.3(3)). It has **two**
callers since AECI-740: that grant, and `POST /api/admin/vendors/:id/seats`, which maps the
same outcomes to the same statuses on purpose — two admin paths must not tell different
stories about one account. Seam #1
...
**Since AECI-740 there is a SECOND grant path, and its defining property is what it omits.**
...
| `GET /api/admin/claims/:id` (AECI-739) | Hard-required | `admin` | — reads only, no `audit_log` row (ADR 0022). The §5.2 claim detail. It answers **422**, not 404, when the id resolves to a `kind='correction'`, matching `PATCH /api/admin/claims/:id` on the same path and message: the row exists and moderates through `/api/admin/requests`, so one id must not tell two stories depending on the verb. `duplicate_siblings` carries `submitter_email` whole, on the same volunteered-contact-information reasoning as `GET /api/admin/requests`, and `/admin/*` is `private, no-store`. `ADMIN_PANEL_SPEC.md` §5.10 |
| `PATCH /api/admin/claims/:id/notes` (AECI-739) | Hard-required | `admin` | `vendor_claim.note_updated` (`entity_type: 'vendor_request'`, `entity_id` = the **claim** id, `metadata.source: 'admin-moderation'`), in the same `db.batch` as its `UPDATE`. Writes the free-text operator note §5.2 step 6 needed and had nowhere to keep. An **annotation**, not a decision: no status change, no grant, no email, and it is never claimant-facing. `before_state` / `after_state` carry the full old and new note, which is what makes the ledger the note's HISTORY — so an unchanged re-save is a **200 no-op that writes nothing**, deliberately. **No `workflow_instances` row** (a note is not a transition, and that CHECK is closed) and **no cache purge** (`/admin/*` is uncacheable and no public surface renders a claim). Writable at every status, including the terminal two |
| `GET /api/admin/contests` (AECI-1008) | Hard-required | `admin` | No (reads only) |
...
| `PATCH /api/admin/connector-catalogs/:id` (AECI-720) | Hard-required | `admin`; **never vendor-self-serve** — a `vendor_admin` gets a flat 403, asserted at `admin-connector-catalogs.spec.ts` | `connector_catalog.managed_by_vendor` / `.managed_by_review` (`entity_type: 'connector_catalog'`, `metadata.source: 'admin-connector-catalog'`), in the same `db.batch` as a guarded `UPDATE … AND managed_by = :from`. A **governance** write: it decides which system may author a catalogue and writes no catalogue content. `vendorId` records who it was handed to and **grants nothing** — the row says `seat_not_granted: true` out loud, because §8.9(2) fences the connector seat off from `vendor_entitlements` entirely. **No `workflow_instances` row** (that CHECK is closed) and **no cache purge** — since AECI-892 a cacheable surface *does* read these tables (`CACHE_STRATEGY.md` §3 rule 5), but `managed_by` is a governance flag that moves no reach count |
| `GET /api/admin/reindex` (AECI-946) | Hard-required | `admin` | — reads only, no `audit_log` row (ADR 0022). The Google re-crawl worklist (`ADMIN_PANEL_SPEC.md` §5.11). Ordering is server-fixed and carries no `sort` parameter |
| `DELETE /api/admin/reindex/:id` (AECI-946) | Hard-required | `admin`; a row already cleared is a flat **404**, which is the ordinary two-tab outcome rather than an error condition | `reindex.cleared` (`entity_type: 'gsc_recrawl_queue'`, `entity_id` = the row id, `metadata.source: 'admin-panel'`), in the same `db.batch` as the DELETE. **Attributed to the admin, never to `'system'`** — §26.1's scheduled-deletion exception allows one summary row per run and it does **not** reach an operator clicking Done, so this audits per row. `before_state` carries the whole cleared row, because the row is gone afterwards and the trail is the only surviving copy. **No `rateLimit()`**, like every other admin write (`waf-rate-limits.md` §6.2). No cache purge and **no `workflow_instances` row** — no public surface reads the table, and clearing a worklist row is not a transition |
| `GET /api/admin/connector-catalogs` + `/:id`, `/:id/stubs`, `/:id/pairs`, `/:id/audit` (AECI-722) | Hard-required | `admin` | — reads only, no `audit_log` row (ADR 0022). The first readers of the connector lane. `handover` is derived from `audit_log` rather than from a column, and is `null` once a lane is reclaimed. Mapping decisions are **not** writable through these reads. Authoring shipped in AECI-724 as the next row. `ADMIN_PANEL_SPEC.md` §5.9 |
| `PATCH /api/admin/connector-stub-mappings/:id` (AECI-724) | Hard-required | `admin`. Then the catalogue gate: **409 `CATALOG_REVIEW_MANAGED`** unless `connector_catalogs.managed_by = 'vendor'`, re-checked inside the batch by a sentinel so a lane reclaimed mid-request writes nothing. A `vendor_admin` gets a flat 403 here; the seat has its own route below | `connector_mapping.updated` (`entity_type: 'connector_catalog'`, `entity_id` = the catalogue, `metadata.source: 'admin-connector-mapping'`, `metadata.mapping_id`), in the same `db.batch` as the UPDATE. The first catalog-**content** write on the admin console. Stamps `decided_by = 'aeci-operator'`. Purges `product:{slug}` for both endpoints and the connector when the row was or becomes publishable. No `workflow_instances` row and no `rateLimit()`. An unchanged body is a 200 no-op that writes nothing |
...
| `POST /api/vendor/seats/invites/:id/resend` (AECI-927) | Hard-required | same **+ `seat_owner`**; a spent, EXPIRED or cross-vendor id is a flat **404** | `vendor_seat.invite_resent` — refreshes `expires_at` and stamps `last_sent_at`/`send_count`, never the `token` (rotating would kill a link the invitee may already hold, and the redeem is bound to the mailbox rather than the token). Capped by a 5-minute per-invite cooldown (429, exact `Retry-After`) and a lifetime 4 sends (**422**, because waiting never clears it) |
...
| `GET /api/vendor/contests` (AECI-1008) | Hard-required | same — scoped by `vendorContestsWhere` (submitted ∪ owner-routed received) | No (reads only) |
...
| `POST /api/vendor/contests/:id/decision` (AECI-1008) | Hard-required | same **+ snapshot owner of an owner-routed row**; anyone else, the submitter included, is a flat **404**. Not capability-gated | `integration.contest.accepted \| declined`; an accept also writes `integration.updated` (§13.9 transfer) |
| `POST /api/vendor/integrations/:id/claim` (AECI-1005; both tables since AECI-1089) | Hard-required | same **+ caller's vendor = `built_by_vendor_id`**, on an `integrations` row or a `connector_evidenced_pairs` row (found `integrations` first, as promote's `locateEdge` does). A vendor with neither endpoint nor ownership, or an unknown id, is a flat **404** (on a pair, "endpoint" is `product_a` / `product_b`, never the connector); an endpoint vendor that is not the owner is **403 `INTEGRATION_NOT_OWNER`**, or **409 `INTEGRATION_OWNER_UNKNOWN`** when nobody is on file. **Entitlement exception (AECI-1040 ruling 2):** the owner of a connector-powered row also needs an active entitlement, else **403 `INTEGRATION_ENTITLEMENT_REQUIRED`**, asked only after ownership settles (`requireActiveEntitlement` / `hasActiveEntitlement`, `lib/integration-entitlement.ts`, DB-free over the session's `entitlementTier`). Not a capability. On every other row a seat is the whole gate (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.5.2) | `integration.claimed` (§13.9 transfer; entity `integration` or `connector_evidenced_pair`) + a `notification.sent` per other endpoint vendor, in one batch |
| `POST /api/vendor/integrations/:id/retire` and `/restore` (AECI-1010; both tables since AECI-1091) | Hard-required | same **+ caller's vendor = `built_by_vendor_id` + `claimed_at IS NOT NULL`**, behind `rateLimit('write')`. The id is looked up in `integrations`, then in `connector_evidenced_pairs` (whose endpoints are `product_a_id` / `product_b_id`). Asked in the claim's order: flat **404** for a row the caller neither owns nor has an endpoint on, or an unknown id; **403 `INTEGRATION_NOT_OWNER`** or **409 `INTEGRATION_OWNER_UNKNOWN`** for an endpoint vendor; then, on a connector-powered row (every pair), **403 `INTEGRATION_ENTITLEMENT_REQUIRED`** for an owner whose vendor holds no active entitlement (AECI-1091, the AECI-1040 carve-out; `INTEGRATION_CONNECTOR_POWERED` before it); **409 `INTEGRATION_NOT_CLAIMED`** for an owner that has not claimed. Then state: retiring a retired row is **409 `INTEGRATION_RETIRED`**, restoring a live row is **409 `INTEGRATION_NOT_RETIRED`**, restoring a row an AECi admin retired is **403 `INTEGRATION_RETIRED_BY_AECI`** (AECI-1046), and a lost race with nothing left to refuse is **409 `INTEGRATION_CHANGED_WHILE_SAVING`**. **Not** capability-gated: on an ordinary row a seat is the whole gate, and on a connector-powered row the entitlement step is the named exception (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.6, §4.6.1). Nothing is deleted | `integration.retired` / `integration.restored` (entity type `connector_evidenced_pair` on a pair) + a `notification.sent` (`kind: 'integration_retire'`) per other endpoint vendor + the endpoint count recomputes (and the connector's, on a pair), in one batch behind a guarded `UPDATE` and its race sentinel. A retire also closes every open contest on the row (either anchor) as `integration.contest.withdrawn` in the same batch. A refused call writes nothing |
| `POST /api/admin/integrations/:id/retire` and `/restore` (AECI-1046; pairs since AECI-1091) | Hard-required | `requireAdmin()`, then `rateLimit('write')`, then a required `reason`. The id may name an `integrations` row or a `connector_evidenced_pairs` row. **404** for an unknown id; **409 `INTEGRATION_NOT_VENDOR_HELD`** for a row (in either table) that is neither claimed nor `origin = 'vendor'` (promote owns it); **409 `INTEGRATION_RETIRED`** / **`INTEGRATION_NOT_RETIRED`** for state; **409 `INTEGRATION_RETIRED_BY_OWNER`** for a restore of an owner retire, which only the owner may undo. Nothing is deleted | `integration.retired` / `integration.restored` with the admin as actor and the reason in `metadata`, a `notification.sent` (`kind: 'integration_retire'`, `retiredBy: 'aeci'`) to the owner and every endpoint vendor, and the count recomputes (the connector's too, on a pair), in one batch behind a guarded `UPDATE` and its race sentinel. A retire closes every open contest as withdrawn in the same batch |
| `POST /api/vendor/integrations` (AECI-1011) | Hard-required | same **+ `product_id` is a promoted product the caller's vendor holds through `product_vendors`, and `counterpart_product_id` is a promoted product**. Either miss is a flat **404**; equal ids are **400**; a connector-delivered kind (`iPaaS`, `integrator`) is **422 `INTEGRATION_INVALID_VALUE`** (decision 9). The body is `.strict()`, so a caller cannot name `built_by_vendor_id`, `origin` or `powered_by_product_id`: the handler sets the owner to the caller. **Not** capability-gated: a seat is the whole gate (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.7). Duplicates warn and never refuse | `integration.created` (`reason: 'vendor-create'`, `possibleDuplicateIds`, §13.9 transfer) + a `notification.sent` (`kind: 'integration_create'`) per other endpoint vendor + both endpoint count recomputes, in one batch |
| `PATCH /api/vendor/integrations/:id` (AECI-1006) | Hard-required | same **+ caller's vendor = `built_by_vendor_id` + `claimed_at IS NOT NULL`**, asked in the claim's order: flat **404** for a row the caller neither owns nor has an endpoint on; **403 `INTEGRATION_NOT_OWNER`** or **409 `INTEGRATION_OWNER_UNKNOWN`** for an endpoint vendor; **403 `INTEGRATION_ENTITLEMENT_REQUIRED`** for the owner of a connector-powered row (either table, AECI-1090) whose vendor holds no active entitlement; **409 `INTEGRATION_NOT_CLAIMED`** for an owner that has not claimed. All before the body is parsed. **Not** capability-gated: a seat is the whole gate (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.5.6), except the entitlement on a connector-powered row | `integration.updated` (`reason: 'owner-edit'`, before/after of the changed fields, §13.9 transfer) + a `notification.sent` (`kind: 'integration_update'`) per other endpoint vendor, in one batch behind a guarded `UPDATE` and its race sentinel. A body that changes nothing writes nothing |
| `PUT` / `DELETE /api/vendor/integrations/:id/links/:productId/:kind` (AECI-1007) | Hard-required | same **+ the caller's vendor holds `:productId` through `product_vendors`, and `:productId` is an endpoint of the row**. Ownership of the integration is **not** required. Every miss, including the other endpoint's side, is a flat **404**; a `PUT` on a connector-powered row is **403 `INTEGRATION_CONNECTOR_POWERED`** after that, while a `DELETE` of the caller's own link passes, so a link stranded when promote made the row connector-powered in place can be removed; a retired row is **409 `INTEGRATION_RETIRED`** for both. **Not** capability-gated: a seat is the whole gate (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.5.7). Stores no profile id, so erasure has nothing to null | `integration.link_set` / `integration.link_removed` (§13.9 transfer, except a DELETE on a connector-powered row, which writes none and marks `connectorPowered`), in one batch. A DELETE of an unset link writes nothing |
...
| `PATCH /api/vendor/products/:id` | Hard-required | same **+ ownership + `product.edit`**, **+ `product.taxonomy.edit`** when the body carries any facet array, and **+ `product.usefulness.edit`** when it carries `usefulness` (AECI-963). All three gates run **after** ownership settles, so a non-owner still gets the flat 404 | `product.updated` (`metadata.source: 'vendor-portal'`) |
| `GET /api/vendor/products/:id/versions` (AECI-607) | Hard-required | same **+ ownership** | No (reads only) |
| `PATCH /api/vendor/connector-stub-mappings/:id` (AECI-724) | Hard-required | `vendor_admin` + non-null `vendor_id`, not banned, behind `rateLimit('write')`. **+ the caller's vendor holds the catalogue's connector product through `product_vendors`, and that product is `connector`-role.** Every miss is a flat **404**, asked before the managed-by check and before the body is parsed, so a non-owner cannot learn whether a catalogue was handed over. Then **409 `CATALOG_REVIEW_MANAGED`** on the owner's own review-managed catalogue. **Not** capability-gated and **no `vendor_entitlements` read**: the connector catalogue seat is deliberately not an entitlement row (`STAGE_2_SPEC.md` §8.9(2)), so a zero-entitlement seat that resolves to `unclaimed` passes | `connector_mapping.updated` as for the admin row, with `metadata.source: 'vendor-portal'` and `metadata.vendor_id`, which is how `/admin/vendors/:id`'s audit tab reaches it. Stamps `decided_by = 'vendor:{vendor slug}'`. Same purge and same no-op rule |
| `GET /api/vendor/products/:id/connectors` (AECI-1013) | Hard-required | same **+ ownership** — no `vendors.verified` gate and no `requireCapability` (reading your own product's data is not the gated capability) | No (reads only). Outside the `GET /api/vendor/updates` cursor, so it has no scope predicate to keep in step |
| `GET /api/vendor/products/:id/connector-catalog` (AECI-1083) | Hard-required | `vendor_admin` + non-null `vendor_id`, not banned. **+ ownership + the product is `connector`-role**, each a flat **404**. The catalogue is resolved through `ownedConnectorCatalogIds`, the PATCH's ownership clause as a subquery. No `requireCapability`, no entitlement read (`STAGE_2_SPEC.md` §8.9(2)), no `rateLimit()` | No (reads only). Inside the `GET /api/vendor/updates` cursor as `catalogue`, which imports the same predicate. Drops `notes` and ships `decided_by` as a kind |
...
| `GET /api/vendor/integrations` (AECI-301) | Hard-required | same **+ §4.2a endpoint authority** (the surface is filtered to it; never a 404). **Plus `owned` (AECI-1089):** rows in either table whose `built_by_vendor_id` is the caller's vendor, minus the rows the endpoint surface already carries, under `ownedIntegrationsWhere` / `ownedEvidencedPairsWhere` (`lib/owned-integrations.ts`). Not entitlement-gated: reads never are | No (reads only) |
