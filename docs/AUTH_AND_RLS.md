...
| `POST /api/vendor/integrations/:id/retire` and `/restore` (AECI-1010) | Hard-required | same **+ caller's vendor = `built_by_vendor_id` + `claimed_at IS NOT NULL`**, behind `rateLimit('write')` and asked in the claim's order: flat **404** for a row the caller neither owns nor has an endpoint on, or an unknown id; **403 `INTEGRATION_NOT_OWNER`** or **409 `INTEGRATION_OWNER_UNKNOWN`** for an endpoint vendor; **403 `INTEGRATION_CONNECTOR_POWERED`** for the owner of a connector-powered row (decision 9, v1); **409 `INTEGRATION_NOT_CLAIMED`** for an owner that has not claimed. Then state: retiring a retired row is **409 `INTEGRATION_RETIRED`**, restoring a live row is **409 `INTEGRATION_NOT_RETIRED`**, restoring a row an AECi admin retired is **403 `INTEGRATION_RETIRED_BY_AECI`** (AECI-1046), and a lost race with nothing left to refuse is **409 `INTEGRATION_CHANGED_WHILE_SAVING`**. **Not** capability-gated: a seat is the whole gate (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.6). Nothing is deleted | `integration.retired` / `integration.restored` + a `notification.sent` (`kind: 'integration_retire'`) per other endpoint vendor + both endpoint count recomputes, in one batch behind a guarded `UPDATE` and its race sentinel. A retire also closes every open contest as `integration.contest.withdrawn` in the same batch. A refused call writes nothing |
| `POST /api/admin/integrations/:id/retire` and `/restore` (AECI-1046) | Hard-required | `requireAdmin()`, then `rateLimit('write')`, then a required `reason`. **404** for an unknown id; **409 `INTEGRATION_NOT_VENDOR_HELD`** for a row that is neither claimed nor `origin = 'vendor'` (promote owns it); **409 `INTEGRATION_RETIRED`** / **`INTEGRATION_NOT_RETIRED`** for state; **409 `INTEGRATION_RETIRED_BY_OWNER`** for a restore of an owner retire, which only the owner may undo. Nothing is deleted | `integration.retired` / `integration.restored` with the admin as actor and the reason in `metadata`, a `notification.sent` (`kind: 'integration_retire'`, `retiredBy: 'aeci'`) to the owner and every endpoint vendor, and both endpoint count recomputes, in one batch behind a guarded `UPDATE` and its race sentinel. A retire closes every open contest as withdrawn in the same batch |
