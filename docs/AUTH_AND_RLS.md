...
| `POST /api/vendor/integrations/:id/claim` (AECI-1005) | Hard-required | same **+ caller's vendor = `built_by_vendor_id`**. A vendor with neither endpoint nor ownership, or an unknown id, is a flat **404**; an endpoint vendor that is not the owner is **403 `INTEGRATION_NOT_OWNER`**, or **409 `INTEGRATION_OWNER_UNKNOWN`** when nobody is on file; the owner of a connector-powered row is **403 `INTEGRATION_CONNECTOR_POWERED`** (decision 9, v1). **Not** capability-gated: a seat is the whole gate (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.5) | `integration.claimed` (§13.9 transfer) + a `notification.sent` per other endpoint vendor, in one batch |
