...
| `PATCH /api/vendor/integrations/:id` (AECI-1006) | Hard-required | same **+ caller's vendor = `built_by_vendor_id` + `claimed_at IS NOT NULL`**, asked in the claim's order: flat **404** for a row the caller neither owns nor has an endpoint on; **403 `INTEGRATION_NOT_OWNER`** or **409 `INTEGRATION_OWNER_UNKNOWN`** for an endpoint vendor; **403 `INTEGRATION_CONNECTOR_POWERED`** for the owner of a connector-powered row; **409 `INTEGRATION_NOT_CLAIMED`** for an owner that has not claimed. All before the body is parsed. **Not** capability-gated: a seat is the whole gate (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.5.6) | `integration.updated` (`reason: 'owner-edit'`, before/after of the changed fields, §13.9 transfer) + a `notification.sent` (`kind: 'integration_update'`) per other endpoint vendor, in one batch behind a guarded `UPDATE` and its race sentinel. A body that changes nothing writes nothing |
