...
| `PUT` / `DELETE /api/vendor/integrations/:id/links/:productId/:kind` (AECI-1007) | Hard-required | same **+ the caller's vendor holds `:productId` through `product_vendors`, and `:productId` is an endpoint of the row**. Ownership of the integration is **not** required. Every miss, including the other endpoint's side, is a flat **404**; a connector-powered row is **403 `INTEGRATION_CONNECTOR_POWERED`** after that. **Not** capability-gated: a seat is the whole gate (`STAGE_2_VENDOR_PORTAL_SPEC.md` §4.5.7). Stores no profile id, so erasure has nothing to null | `integration.link_set` / `integration.link_removed` (§13.9 transfer), in one batch. A DELETE of an unset link writes nothing |
