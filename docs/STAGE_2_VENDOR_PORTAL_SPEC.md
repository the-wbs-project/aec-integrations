...
| One batch on a pair | The same rows as on `integrations`: the guarded `UPDATE` of `connector_evidenced_pairs` keyed on `built_by_vendor_id = <caller> AND claimed_at IS NOT NULL AND retired_at IS NULL`, `ownerWriteSentinel`, the `integration.updated` audit row (`entity_type` `connector_evidenced_pair`, as promote and the AECI-1089 claim write it for a pair), and one `integration_update` notification (`entity_type` `integration`, like the claim's) per vendor of either endpoint other than the owner. |
