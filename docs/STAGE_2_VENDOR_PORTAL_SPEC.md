...
> **Owner carve-out on connector-powered rows: ruled 2026-09-23 (AECI-1040 follow-ups). The claim and the owner's read are built (AECI-1089); edit, retire and contests are pending.** Chris ruled that the owner of a connector-powered row may claim it, edit its standard fields, and retire or restore it (`STAGE_2_SPEC.md` §8.10(8), ADR 0035). It covers rows in `integrations` and in `connector_evidenced_pairs`. It covers every owner, endpoint vendors included. Each carve-out write needs an active entitlement, a `vendor_entitlements` row with `status = 'active'`. **The schema and the promote fence are built (AECI-1088, migration `0049`).** `connector_evidenced_pairs` carries `claimed_at`, `origin`, `retired_at` and `retired_by`, promote fences a vendor-held pair (`REVIEW_APP_PROMOTE_API.md` §4b, §4c), and the ops lanes exempt one (§4.5.5). **The claim across both tables, its entitlement gate and the owned-rows read are built (AECI-1089, §4.5.2).** The claim is the only route that sets `claimed_at` on a pair. The last-seat hand-back clears it (AECI-989, §4.5.3). The edit, retire and restore routes still answer `403 INTEGRATION_CONNECTOR_POWERED` on a connector-powered row until AECI-1090 and AECI-1091 ship. What changes where:
>
...
> | Standard-field edit | Allowed on the same terms. `mechanism_kind` stays frozen on a connector-powered `integrations` row. An evidenced pair has no such column. | §4.5.6 |
...
**Connector-powered rows: ruled 2026-09-23, build pending (AECI-1040 follow-ups).** The owner may edit a connector-powered row once it has claimed it and holds an active entitlement. On an `integrations` row, `mechanism_kind` is frozen, because it decides routing. The other ten standard fields are editable. On a `connector_evidenced_pairs` row the same ten are editable, and there is no `mechanism_kind` column. `direction` there is in the canonical A/B frame (`DATABASE_SCHEMA.md` §9a.6). The rest of the table above is unchanged.
