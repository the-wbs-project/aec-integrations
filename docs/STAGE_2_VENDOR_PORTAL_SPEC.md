...
**9. When the surface exists.** AECI-722 (admin-only screen) then AECI-724 (the vendor seat): grant the catalogue-maintenance seat there, authorized by `profiles.role = 'vendor_admin'` + `profiles.vendor_id` with **no entitlement row** (§8.9(2)) — so the badge never lights. Flipping that catalogue to vendor-managed is operator-executed and freezes the review lane for that iPaaS (AECI-720's `managed_by` cutoff). Note that §8.9(3) leaves **provisioning** open: no route writes `role = 'vendor_admin'` today, which is the whole reason this note parks rather than grants.
