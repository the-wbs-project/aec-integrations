...
- **503 until AECI-530.** Because the AECI-519 grant reports `DEPENDENCY_FAILURE` (503) in every deployed env until `SUPABASE_SERVICE_ROLE_KEY` is bound, the surface renders that as "Grant unavailable — the identity service isn't configured. Reject still works." Reject needs no resolution and works today.
