...
> **Tell the claimant what the seat does in v1.** Decision 9 of epic AECI-1003 stays as written for v1 (ruled 2026-09-21). A third-party owner's integrations are connector-powered, so its seat reaches them only to claim them. If the open point in `STAGE_2_SPEC.md` §8.10(5) is ruled as recommended, it cannot claim them either in v1. It cannot edit, retire or create them, and it never sets per-side links because it owns neither side. The seat does carry its vendor profile, its connector product listing, taxonomy, the "How teams use it" narrative and analytics. Editing and retiring its own connector-powered integrations is **AECI-1040**, not yet built. Say so before arranging payment, so nobody pays for a capability that does not exist yet.
>
