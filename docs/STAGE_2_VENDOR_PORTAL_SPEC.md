...
**An owner with no active seat gets no owner-routed contest (AECI-989, ruled 2026-09-23).** While the owner vendor has no unbanned `vendor_admin` seat, a content contest that would route to the owner routes to `aeci` instead. It is stamped with `owner_seat_lapsed_at` and the owner gets no notice. A ban of the owner's last active seat, or a revoke that leaves only banned seats, moves its open owner contests to AECi the same way, stamped (`integration.contest.rerouted`, `metadata.reason = 'owner-seat-lapsed'`). An unban routes every stamped open contest back, if the row is live and has been claimed by that owner since before the stamp (`reason = 'owner-seat-restored'`). Revoking the owner's last seat moves them to AECi for good, unstamped, as part of the hand-back (§4.5.3). A ban never changes `claimed_at`. Builders: `lib/vendor-handback.ts`. Contract: `STAGE_2_ATTESTATIONS_SPEC.md` §13.9.
