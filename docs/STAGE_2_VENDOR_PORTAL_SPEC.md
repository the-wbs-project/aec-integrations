...
> **Known console gap — still open, now tracked separately.** `/admin/claims` surfaces **no role signal** — `AdminClaim` carries `domain_match`, `has_auth_account`, `duplicate_of_request_id`, `existing_seats` and `related_requests`, and nothing else — and `/admin/vendors/:id` reports product **counts**, not roles. So the check still happens outside the console: the public `/vendors/:slug` and product surfaces render the Connector / Hybrid chip (`apps/web/src/app/products/role-badge.ts`; `application` deliberately renders nothing, so *no chip* means endpoint). **AECI-722 did not close this**, deliberately: the gap is on `/admin/claims` and `/admin/vendors/:id`, not on the connector screen, and folding it in would have coupled an unrelated payload change to the connector lane's merge. It has its own issue, and it needs nothing from the connector lane — `product_role` is already on `ProductListItemSchema` and `RoleBadge` is a drop-in.
...
- **The connector-vendor catalogue-maintenance seat** — the return side of the `STAGE_2_SPEC.md` §8.8 payer carve-out, settled as policy in **§8.9** (AECI-704). **The admin half SHIPPED 2026-08-31** (AECI-722 — `/admin/connectors`, `ADMIN_PANEL_SPEC.md` §5.9); the vendor seat is still **AECI-724**, and *provisioning* the `vendor_admin` role is now its own issue rather than either of them (§8.9(3)). Deliberately **not** an entitlement row and **not** a capability id: it authorizes on this doc's existing `vendor_admin` + `vendor_id` primitive, which `findVendorProfile`'s `leftJoin` already supports for a vendor with no entitlement. Until those ship, §5.2 is the manual operator procedure — and §8.9(3) notes the open sub-question, that **no route writes `role = 'vendor_admin'`** today.
