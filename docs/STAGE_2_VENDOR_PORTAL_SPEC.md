...
3. an UPDATE of an unclaimed curated row that changes any field in the key: its endpoints (as a pair), its connector, its `mechanism_kind` or its owner. A kind change (`api` to `native` beside a vendor's `native` row) or an owner change (to unknown, or to the vendor's own id) creates a match as surely as a re-point. An UPDATE that changes none of the four, a direction swap included, leaves the key as it was, cannot create a new match, and is not asked. So a curated row that already twins a vendor row keeps being written on every re-promote that leaves its key alone.
