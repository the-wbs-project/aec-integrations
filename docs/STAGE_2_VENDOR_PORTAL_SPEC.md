...
**`STAGE_2_SPEC.md` §8.8 / §8.9.** A pure **connector** vendor is not invoiced for verification and is never sold it; what it gets instead is a catalogue-maintenance seat on the connector admin surface (AECI-722's screen, AECI-724's seat). Neither exists yet, so this is a **manual operator step** — and because §5's console offers only Grant and Reject, both of which are wrong here, the procedure has to be written down rather than inferred.
...
> **Known console gap (for AECI-722).** `/admin/claims` surfaces **no role signal** — `AdminClaim` carries `domain_match`, `has_auth_account`, `duplicate_of_request_id`, `existing_seats` and `related_requests`, and nothing else — and `/admin/vendors/:id` reports product **counts**, not roles. So the check happens outside the console: the public `/vendors/:slug` and product surfaces render the Connector / Hybrid chip (`apps/web/src/app/products/role-badge.ts`; `application` deliberately renders nothing, so *no chip* means endpoint).

**2. Do not press Grant.** Approve is **unconditional**: `approveClaim` composes `grantSeatStatements` with `activateEntitlementStatements` in one batch at `GRANT_TIER = 'verified'` (`apps/api/src/routes/admin-claims.ts`), which opens a `vendor_entitlements` row, flips the `vendors.verified` mirror, and hands the seat **every** capability in `TIER_CAPABILITIES.verified`. There is no partial grant, and approve-then-clear is not a workaround — it lights the badge, bumps `vendors.updated_at` in both directions so the nightly Algolia push can carry a verified vendor record in between, and leaves an audit trail that reads as a customer granted and then revoked.
...
**4. Park the claim as `open`, and route out of band.** Leave the request in `open`, reply to the claimant directly naming the partnership track, and **keep the linked Linear issue in an unstarted-category state**. `STATE_TYPE_TO_STATUS` (`apps/api/src/routes/webhooks.ts`) maps `triage` / `backlog` / `unstarted` → `open`, so the issue may sit in any of them indefinitely. Only `started` → `in_review`, and **`GET /api/admin/claims` cannot show an `in_review` claim at all** — it filters on an *exact* status and its query enum offers only `open | resolved | rejected` — so moving the issue to In Progress makes the claim vanish from every queue view. `completed` → `resolved` is worse still: the claim then reads as approved in the Resolved tab with no seat and no grant behind it.

**5. Expect the duplicate chip to fire on later claims.** A parked-open claim stays inside the `openClaims` group-by that computes `is_duplicate` (`apps/api/src/routes/admin-claims.ts`), so **every subsequent claim on that vendor renders with the duplicate signal** — including a legitimate one from a real endpoint contact. Never dismiss a new claim on the chip alone while a parked one is open.

**6. The conversation record lives in Linear, because there is nowhere else.** There is **no** `/admin/claims/:id` detail route — `apps/web/src/app/app.routes.ts` registers `claims → ClaimQueue`, a list — and no free-text operator note on a claim (`aec-admin-notes` is the data-caveat envelope, not an annotation surface). Claims do auto-create a Linear issue in the Vendor Requests project carrying the `claim` label (`apps/api/src/lib/linear.ts`); comment there. Stated explicitly so nobody looks for an in-product home that does not exist.
...
**9. When the surface exists.** AECI-722 (admin-only screen) then AECI-724 (the vendor seat): grant the catalogue-maintenance seat there, authorized by `profiles.role = 'vendor_admin'` + `profiles.vendor_id` with **no entitlement row** (§8.9(2)) — so the badge never lights. Flipping that catalogue to vendor-managed is operator-executed and freezes the review lane for that iPaaS (AECI-720's `managed_by` cutoff — **the mechanism SHIPPED 2026-08-31**: `PATCH /api/admin/connector-catalogs/:id` behind `requireAdmin()`, audited per row, and the promote arm then refuses that catalogue's pages with `CATALOG_VENDOR_MANAGED`. AECI-722 builds the screen that calls it; the endpoint deliberately grants **no** seat, so it does not close §8.9(3) below.) Note that §8.9(3) leaves **provisioning** open: no route writes `role = 'vendor_admin'` today, which is the whole reason this note parks rather than grants.
...
- **The connector-vendor catalogue-maintenance seat** — the return side of the `STAGE_2_SPEC.md` §8.8 payer carve-out, settled as policy in **§8.9** (AECI-704) and built by **AECI-722** (admin-only connector screen) then **AECI-724** (the vendor seat). Deliberately **not** an entitlement row and **not** a capability id: it authorizes on this doc's existing `vendor_admin` + `vendor_id` primitive, which `findVendorProfile`'s `leftJoin` already supports for a vendor with no entitlement. Until those ship, §5.2 is the manual operator procedure — and §8.9(3) notes the open sub-question, that **no route writes `role = 'vendor_admin'`** today.
