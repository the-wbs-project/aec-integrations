...
| When links are lost | Only with their row. On an **unclaimed** row that is a retraction, a datatool prune, an `ops:retract-product` of an endpoint, or a promote cross-table move into `connector_evidenced_pairs`. The move means the row became connector-powered, which takes no links. A vendor-held row is refused by every one of those lanes (§4.5.5). |
