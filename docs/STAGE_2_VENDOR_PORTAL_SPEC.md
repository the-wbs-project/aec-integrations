...
| Connector-powered rows | **Not claimable in v1** (decision 9, ruled 2026-09-22, AECI-1005 Q1 option A). "Connector-powered" is `isConnectorPoweredEdge`: `powered_by_product_id` set, or a connector `mechanism_kind` such as `iPaaS`, which includes Convention-A self-references. In v1 `connector_evidenced_pairs` had no claim column at all. Since migration `0049` (AECI-1088) it has `claimed_at` and `origin`, and promote fences a pair that sets either, but no route claims a pair yet: that route is AECI-1089. A claim under decision 9 as written would freeze the row to promote (decision 5) while decision 9 freezes it to the owner, leaving nobody able to correct it. AECI-1040 delivers claim, edit and retire on these rows together. **Ruled 2026-09-23, build pending:** the owner may claim such a row in either table if it holds an active entitlement (§4.5 note). |
