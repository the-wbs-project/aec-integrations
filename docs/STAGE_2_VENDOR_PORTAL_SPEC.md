...
> **Owner carve-out on connector-powered rows: ruled 2026-09-23, build pending (AECI-1040 follow-ups).** Chris ruled that the owner of a connector-powered row may claim it, edit its standard fields, and retire or restore it (`STAGE_2_SPEC.md` §8.10(8), ADR 0035). It covers rows in `integrations` and in `connector_evidenced_pairs`. It covers every owner, endpoint vendors included. Each carve-out write needs an active entitlement, a `vendor_entitlements` row with `status = 'active'`. **The schema and the promote fence are built (AECI-1088, migration `0048`); the vendor routes are pending.** `connector_evidenced_pairs` now carries `claimed_at`, `origin`, `retired_at` and `retired_by`, promote fences a vendor-held pair (`REVIEW_APP_PROMOTE_API.md` §4b, §4c), and the ops lanes exempt one (§4.5.5). No route writes those columns yet, so the "As built" tables below still describe the routes, including every `403 INTEGRATION_CONNECTOR_POWERED`. What changes where:
>
> | Write | Ruled change | Section |
> |---|---|---|
> | Claim | Allowed for the owner of a connector-powered row in either table, with an active entitlement. | §4.5.2 |
...
> | Retire and restore | Allowed on the same terms. An evidenced pair has `retired_at` and `retired_by` since migration `0048` (AECI-1088) and will reuse AECI-1010's route and batch (AECI-1091). AECi gets an admin retire and restore there too. | §4.6 |
...
>
> **A third-party owner cannot see its rows yet.** `GET /api/vendor/integrations` and the `integrations` cursor are scoped by `ownedEndpointJoin` to the endpoint vendors. The build adds an owned-rows read and a matching cursor statement (`STAGE_2_REALTIME_SPEC.md` §2.2).
...
| Connector-powered rows | **Not claimable in v1** (decision 9, ruled 2026-09-22, AECI-1005 Q1 option A). "Connector-powered" is `isConnectorPoweredEdge`: `powered_by_product_id` set, or a connector `mechanism_kind` such as `iPaaS`, which includes Convention-A self-references. In v1 `connector_evidenced_pairs` had no claim column at all. Since migration `0048` (AECI-1088) it has `claimed_at` and `origin`, and promote fences a pair that sets either, but no route claims a pair yet: that route is AECI-1089. A claim under decision 9 as written would freeze the row to promote (decision 5) while decision 9 freezes it to the owner, leaving nobody able to correct it. AECI-1040 delivers claim, edit and retire on these rows together. **Ruled 2026-09-23, build pending:** the owner may claim such a row in either table if it holds an active entitlement (§4.5 note). |
...
A row is **vendor-held** when it is claimed or `origin = 'vendor'`. "No upstream record points at it" is expected for such a row, not evidence that it is residue. The test is the same in both anchor tables: `integrations` since migration `0044`, and `connector_evidenced_pairs` since migration `0048` (AECI-1088).
...
- **One line and at most one action, by state.**
  ...
  - Owner of a connector-delivered row (`attestable: false`): a sentence that it cannot be claimed or edited yet (decision 9). No button. *AECI-1040 changes this state once built (§4.5 note).* `attestable` is read off the wire, never re-derived.
