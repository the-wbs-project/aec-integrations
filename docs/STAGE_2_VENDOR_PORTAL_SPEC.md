...
> **Owner carve-out on connector-powered rows: ruled 2026-09-23 (AECI-1040 follow-ups). The claim and the owner's read are built (AECI-1089), so are the standard-field edit (AECI-1090, §4.5.6), contests (AECI-1092, §11b.13) and retire and restore (AECI-1091, §4.6).** Chris ruled that the owner of a connector-powered row may claim it, edit its standard fields, and retire or restore it (`STAGE_2_SPEC.md` §8.10(8), ADR 0035). It covers rows in `integrations` and in `connector_evidenced_pairs`. It covers every owner, endpoint vendors included. Each carve-out write needs an active entitlement, a `vendor_entitlements` row with `status = 'active'`. **The schema and the promote fence are built (AECI-1088, migration `0049`).** `connector_evidenced_pairs` carries `claimed_at`, `origin`, `retired_at` and `retired_by`, promote fences a vendor-held pair (`REVIEW_APP_PROMOTE_API.md` §4b, §4c), and the ops lanes exempt one (§4.5.5). **The claim across both tables, its entitlement gate and the owned-rows read are built (AECI-1089, §4.5.2).** The claim is the only route that sets `claimed_at` on a pair. The last-seat hand-back clears it (AECI-989, §4.5.3). **Retire and restore are built (AECI-1091, §4.6):** the owner retires its claimed connector-powered rows in either table with an active entitlement, and AECi retires vendor-held pairs (§4.6.4). What changes where:
>
> | Write | Ruled change | Section |
> |---|---|---|
> | Claim | **Built (AECI-1089).** Allowed for the owner of a connector-powered row in either table, with an active entitlement. | §4.5.2 |
> | Standard-field edit | **Built (AECI-1090).** Allowed on the same terms. `mechanism_kind` stays frozen on a connector-powered `integrations` row. An evidenced pair has no such column. | §4.5.6 |
> | Retire and restore | **Built (AECI-1091).** Allowed on the same terms. An evidenced pair uses AECI-1010's route and a pair form of its batch, a soft retire of `retired_at` and `retired_by` (migration `0049`). AECi has the admin retire and restore there too. Every count, id set and public read filters retired pairs. | §4.6 |
...
> | Contest | **Built (AECI-1092).** AECI-1008's path and routing, with rulings A, B, C and E. Evidenced pairs are contestable through the same `POST /api/vendor/integrations/:id/contests`, in the same promote as their claim. | §11b.13 |
>
...
| Rule | As built |
|---|---|
| Gate | `requireVendor()` → `rateLimit('write')` → ownership in the handler. **A seat is the whole gate** (decision 15): no `requireCapability`, no Verified check. The same named exception to `API_CONTRACTS.md` §6.14 that §11b.2 made for contests. **One named exception to decision 15 (AECI-1089, AECI-1040 ruling 2):** the owner of a connector-powered row also needs an active entitlement. `hasActiveEntitlement` in `apps/api/src/lib/integration-entitlement.ts` reads the session's resolved tier, so it costs no read. It is not a capability and compares no tier. |
| Who may claim | Only the vendor in `built_by_vendor_id`, with **no approval** (decision 1). **Decision 9's predicate is the gate, not the owner's relationship to the endpoints** (ruled 2026-09-22): a third-party owner of a row that is not connector-powered may claim it, because that row has an editor. Such rows should not exist under the two-question test, so the route does not special-case them. In practice a third-party owner's rows are connector-powered, so it needs an active entitlement to claim them (next row). |
| Refusals, in order | An unknown id, or a row the caller neither owns nor has an endpoint on, is the same `404`. An endpoint vendor that is not the owner gets `403 INTEGRATION_NOT_OWNER`. An endpoint vendor on a row with no owner gets `409 INTEGRATION_OWNER_UNKNOWN`. The owner of a connector-powered row with no active entitlement gets `403 INTEGRATION_ENTITLEMENT_REQUIRED`, with `details: { tier, status }` (AECI-1089; it answered `403 INTEGRATION_CONNECTOR_POWERED` before). A claimed row is `409 INTEGRATION_ALREADY_CLAIMED`. On an evidenced pair "endpoint" means `product_a_id` or `product_b_id`: the connector product's vendor is not an endpoint vendor and gets the `404`. |
| Connector-powered rows | **Claimable by an entitled owner, in either table (AECI-1089, shipped).** The route finds the id the way promote's `locateEdge` does: `integrations` first, then `connector_evidenced_pairs` (`locateClaimTarget`). "Connector-powered" is unchanged: `isConnectorPoweredEdge` (`powered_by_product_id` set, or a connector `mechanism_kind` such as `iPaaS`, which includes Convention-A self-references), or any row in `connector_evidenced_pairs`. A claimed pair is vendor-held, so promote's fence (AECI-1088) keeps off it. *History:* in v1 (decision 9, ruled 2026-09-22, AECI-1005 Q1 option A) the claim was refused, because a claim freezes the row to promote (decision 5) while decision 9 froze it to the owner, leaving nobody able to correct it. AECI-1040 lifted that for the owner. Until AECI-1090 ships, a claimed connector-powered row has no edit route, so the owner holds it and AECi's catalogue no longer updates it. |
| One batch | The guarded `UPDATE` on whichever table holds the row (`claimed_at`, plus §13.9's maintenance transfer; `updated_at` moves through `$onUpdate`), a race sentinel, the `integration.claimed` audit row, and a `notification.sent` row (`metadata.kind: 'integration_claim'`) for every vendor of either endpoint other than the owner. The audit row's entity is `integration` or `connector_evidenced_pair`, the vocabulary promote uses. On a connector-powered row its metadata adds `connectorPowered: true` and `anchor`. A lost race writes nothing and answers the re-read's refusal, or `409 INTEGRATION_CHANGED_WHILE_SAVING` when there is none (promote moved the row to the other table, say). |
| After commit | Purges `pair:{a}__{b}` and both `product:` tags, and queues the pair re-crawl, because the maintenance marker on the pair page changed. An evidenced pair also purges its connector's `product:` tag, the set `ops:retract-product` derives for the same rows (`CACHE_STRATEGY.md` §2). |
...
- **Two paths un-claim a row.** An AECi admin accept of an `owner` contest that reassigns it to a different vendor or to "neither" clears `claimed_at`, because the new owner has not acted (§11b.6). Since **AECI-989 (2026-09-23)**, revoking the owner's last `vendor_admin` seat clears it on every live row the owner claimed, in either table since AECI-1089 made evidenced pairs claimable. Since **AECI-1106** erasing that seat's account does the same, for the same reason: nobody who has acted owns the row now (`STAGE_2_ATTESTATIONS_SPEC.md` §13.9). A retired row keeps its claim, and a vendor-created row stays fenced by `origin`. Both paths re-route the old owner's open contests to AECi. Nothing else, promote included, clears it. **A ban un-claims nothing** (ruled 2026-09-23). It only moves the owner's open contests to AECi while the vendor has no unbanned seat (§11b.4).
...
**Only an endpoint vendor can assert.** Contests require an endpoint seat (§11b.2), so a third-party vendor cannot file one. That is fine in v1: a non-endpoint owner's rows are connector-powered, decision 9 keeps the claim off them anyway, and such a vendor reaches AECi through the partnership track (§5.2). *Since AECI-1089 an entitled non-endpoint owner claims its connector-powered rows directly (§4.5.2), so it needs no owner-unknown assertion for a row it already owns.*

What an AECi accept writes in every other case is §11b.6.

#### 4.5.5 The ops lanes (AECI-1005, AECI-1088)

A row is **vendor-held** when it is claimed or `origin = 'vendor'`. "No upstream record points at it" is expected for such a row, not evidence that it is residue. The test is the same in both anchor tables: `integrations` since migration `0044`, and `connector_evidenced_pairs` since migration `0049` (AECI-1088).

| Lane | `integrations` | `connector_evidenced_pairs` (AECI-1088) |
|---|---|---|
| Strand audit (`scripts/ops/2026-09-stranded-row-audit/`) | Counts vendor-held rows and never reports one as source-gone or puts it in the `--ids-out` list. A stranded ENDPOINT on one is still a finding, marked `vendorHeld: true`. | The same, out of `evidencedPairSourceGone`, through the same `classifyRows`. |
| Datatool prune (`apps/datatool/src/prune-integrations.ts`) | Lists vendor-held ids in the plan and refuses the run with `409 VENDOR_HELD`. No guard acknowledgment overrides it. Each DELETE re-checks at write time. | The prune never deletes a pair. An id that names a vendor-held pair is still listed and refused with `409 VENDOR_HELD`, rather than showing as `missing`. |
| Retraction consumer (`scripts/ops/2026-09-retraction-consumer/consume.mjs`) | Refuses the whole run, loudly, when any resolved row is vendor-held and not on `HOLD`. A held entry is never deleted and never confirmed. Each DELETE re-checks at write time. | The same refusal. The pair DELETEs re-check at write time too (`guardedAnchorDeleteSql`); before AECI-1088 they carried no guard. |
| `ops:retract-product` (`apps/api/src/lib/retract-product.ts`) | Refuses to retract a product whose deletion would cascade (or detach `powered_by` on) a vendor-held integration. A refusal, so `--force` does not override it. The delete plan re-checks at write time and aborts before any write (AECI-1088 review). | Refuses a product that is the connector or an endpoint of a vendor-held pair, even with `--delete-evidenced-pairs` and `--force`. The delete plan re-checks at write time and aborts before any write (AECI-1088 review). |

All of them probe the live DDL for the columns (`vendor-held.mjs`, and `ddlHasVendorHeldColumns` in `retract-product.ts`), per table, because migrations `0044` and `0049` reach production only at the next prod promote and a query naming a missing column would fail every run until then. A tier without a table's columns cannot hold a vendor-held row in that table, so the probe degrades to "none held" there. An empty DDL read is "could not check" and throws, in `ops:retract-product` too since the AECI-1088 review. Since AECI-1010 the same holds for `retired_at`: the datatool prune and reindex, the retraction consumer's count repair and both reconcile CLIs (`reconcile-product-counts.ts`, which runs daily against production, and `reconcile-algolia-drift.ts`) read the `integrations` DDL first and use `liveIntegrationSqlIf`, which degrades to always-true without the column. An empty DDL read is "could not check" and throws.
...
**Connector-powered rows (AECI-1090 — 2026-09-23, the AECI-1040 carve-out).** The owner edits a connector-powered row once it has claimed it (AECI-1089, §4.5.2) and its vendor holds an active entitlement.

| Rule | As built |
|---|---|
| Entitlement | `hasActiveEntitlement` (`lib/integration-entitlement.ts`), asked after ownership settles and only on a connector-powered row: `isConnectorPoweredEdge` on `integrations`, or any `connector_evidenced_pairs` row. A seat with no `vendor_entitlements` row, or a `pending`, `expired` or `revoked` one, gets `403 INTEGRATION_ENTITLEMENT_REQUIRED` with `details { tier, status }`. It is asked before the claim check, because a claim needs the same entitlement. |
| Frozen on an `integrations` row | `mechanism_kind` (ruling 5). A body that changes it, or clears it, is `422 INTEGRATION_INVALID_VALUE` naming the field, and nothing is written. A body that repeats the stored value drops it and saves the rest. It is the only editable field that decides a row's lane: `isConnectorPoweredEdge` and the public page's connector-lane grouping both read it. The other routing inputs are not editable by any owner write. The endpoints and `powered_by_product_id`, which promote routes between the two tables on (a resolved connector that is neither endpoint moves the edge to `connector_evidenced_pairs`), sit outside the `.strict()` body. |
| A `connector_evidenced_pairs` row | The ten fields in `CONNECTOR_POWERED_EDIT_FIELDS`. The table has no `mechanism_kind` column, so a body that sends one is `422 INTEGRATION_INVALID_VALUE` naming it, never a silent drop. The routing columns (`connector_product_id`, `product_a_id`, `product_b_id`) are not editable. The two endpoints for the ownership answers are `product_a_id` and `product_b_id`. The connector's vendor holds no endpoint, so it gets the `404`. `direction` is stored in the canonical A/B frame (`DATABASE_SCHEMA.md` §9a.6). The wire is caller-relative against `context_product_id`, with A in the source seat. An omitted frame means the caller's own endpoint, or A for a third-party owner. A retired pair (`retired_at`, written by AECI-1091) is `409 INTEGRATION_RETIRED`. |
| One batch on a pair | The same rows as on `integrations`: the guarded `UPDATE` of `connector_evidenced_pairs` keyed on `built_by_vendor_id = <caller> AND claimed_at IS NOT NULL AND retired_at IS NULL`, `ownerWriteSentinel`, the `integration.updated` audit row (`entity_type` `connector_evidenced_pair` and `metadata { connectorPowered: true, anchor: 'evidenced_pair' }`, as the AECI-1089 claim writes them for a pair), and one `integration_update` notification (`entity_type` `connector_evidenced_pair`, like the claim's since AECI-1089) per vendor of either endpoint other than the owner. |
| After commit on a pair | The by-id Algolia sync (the integrations index holds evidenced pairs, and the by-id path reads both tables). Purge `pair:{a}__{b}`, both endpoint `product:` tags and the connector's `product:` tag. Queue the pair re-crawl. The `updated_at` bump moves the owner's owned-rows cursor statement, `MAX(connector_evidenced_pairs.updated_at)` under `ownedEvidencedPairsWhere` (AECI-1089, `STAGE_2_REALTIME_SPEC.md` §2.2). |
...
### 4.6 The owner retires and restores an integration (AECI-1010 — 2026-09-22; AECi retire AECI-1046; connector-powered rows and evidenced pairs AECI-1091 — 2026-09-23)
...
**Connector-powered rows and evidenced pairs (AECI-1091 — 2026-09-23, the AECI-1040 carve-out and ruling D).** The owner retires and restores its claimed connector-powered rows, in either table, with an active entitlement. The `:id` may name an `integrations` row or a `connector_evidenced_pairs` row: the route reads `integrations` first and then the pair table (`lib/retire-target.ts`), as the claim and the edit do. What is built:

| Rule | As built |
|---|---|
| Gate on a connector-powered row | After ownership, `hasActiveEntitlement`: a seat with no active entitlement gets `403 INTEGRATION_ENTITLEMENT_REQUIRED` (`details { tier, status }`), on retire and on restore. It replaces the old `403 INTEGRATION_CONNECTOR_POWERED`. Every evidenced pair takes this step. An ordinary row keeps the seat as its whole gate. |
| Ownership on a pair | The same answers as on `integrations`, with `product_a_id` and `product_b_id` as the two endpoints. A third-party owner holds neither endpoint and is still the owner. The connector's vendor holds no endpoint, so a non-owner connector vendor gets the `404`. |
| Soft retire only | `retired_at` and `retired_by` on the pair (migration `0049`). **Never a delete:** the table is a cascade parent of `claims`, and `attestations` cascade from `claims`. |
| One batch on a pair | `buildPairRetireBatch` in `integration-retire-write.ts`: the guarded `UPDATE` of `retired_at`, `retired_by` and `updated_at` (keyed on the caller as owner and `claimed_at IS NOT NULL`, and `retired_by = 'owner'` on restore), `retireRaceSentinel`, the `integration.retired` / `integration.restored` audit row with entity type `connector_evidenced_pair` and `metadata { anchor: 'evidenced_pair', connectorPowered: true, connectorProductId }`, one `notification.sent` row per vendor of either endpoint other than the owner (same entity type), and **three** `integration_count` recomputes: both endpoints and the connector (§12.5 option B counts a pair toward its connector). |
| Contests on a pair | **Closed as `withdrawn` on retire, in the same batch**, exactly as on `integrations`: every open contest anchored on the pair (`evidenced_pair_id`, AECI-1092) gets its guarded UPDATE and sentinel, its workflow closure, an `integration.contest.withdrawn` audit row (`metadata.anchor: 'evidenced_pair'`) and a `closed_by_retire` notice to the submitter. Then `noOpenContestsSentinel` over `evidenced_pair_id`, so a contest filed between the read and the batch aborts it. AECI-1092's contest submit carries the mirror sentinel (`contestAnchorLiveSentinel`). Restore reopens none. |
| After commit on a pair | The by-id Algolia sync of the pair (the delete arm removes a retired pair's record, the upsert arm restores it), all three products and the owner vendor; purge `pair:{a}__{b}`, both endpoint `product:` tags, the connector's `product:` tag, `vendor:{owner}`, `index:products`, `taxonomy` and `sitemap`; queue the pair re-crawl. |
| Freshness cursor | The `updated_at` bump moves the owner's owned-rows statement (AECI-1089, `STAGE_2_REALTIME_SPEC.md` §2.2), which reads retired rows too. |
| Read side | **Every count, id set and public read filters retired pairs**, through `liveEvidencedPairWhere` / `liveEvidencedPairSql`: the evidenced arm of each `LOCKSTEP_SITES` entry (`apps/api/src/lib/count-lockstep.spec.ts`, which asserts both arms at every site), the integrations list and so the sitemap, the pair page and its timeline, the product page's evidenced relations (endpoint and connector), `readPairCounterpartSlugs`, `resolveMovedPair`, the AECI-1013 Connectors section, the admin connector screens and the Algolia sync's delete arm. `STAGE_1_5_SPEC.md` §13.5 rule 2 now reads "both arms". Tests: `vendor-integration-retire-connector.spec.ts`, `retired-evidenced-public-reads.spec.ts`. |
...
| Gate | `requireVendor()` → `rateLimit('write')` → in the handler: ownership (the claim route's `404` / `403 INTEGRATION_NOT_OWNER` / `409 INTEGRATION_OWNER_UNKNOWN`), then on a connector-powered row `403 INTEGRATION_ENTITLEMENT_REQUIRED` without an active entitlement (AECI-1091; `403 INTEGRATION_CONNECTOR_POWERED` before it), then `409 INTEGRATION_NOT_CLAIMED`. On an ordinary row a seat is the whole gate (decision 15). |
...
> **Promotion gate (AECI-1010 review, 2026-09-22), closed by the 1011 twin guard, which covers the insert, de-route and UPDATE paths.** The gate: do not promote AECI-1010 to production before AECI-1011's `VENDOR_OWNED_TWIN` promote guard. A retired row is claimed, so the §4.5.3 fence keeps promote off it. But three promote writes never touch the retired row and can still put a fresh, live twin beside it, undoing the retire in public with a row the owner never saw: an insert under a **new** upstream id for the same pair, a **de-route** that moves a connector-evidenced pair back into `integrations`, and an UPDATE of an unclaimed curated row that **changes a key field** (re-points it onto the pair, or changes its kind or owner to match). The AECI-1011 guard (§4.7.3, `REVIEW_APP_PROMOTE_API.md` §4c) skips all three when the result would strongly match any vendor-held row, **retired rows included**, and reports it. Both are on `main`: AECI-1010 at `8cc6daf8` and AECI-1011 at `99642693`, which descends from it. Neither is in production yet. **Any production promote at or after `99642693` carries both, which satisfies the order rule.** A promote of a SHA between the two would carry the retire without the guard, so do not promote one.
...
**AECI-1091.** On a claimed connector-delivered card the section offers Retire only when the vendor holds an active entitlement (the tier `GET /api/vendor/me` resolved, the test the server makes). Without one it offers nothing on a live row, and on a retired row it says restoring needs a plan instead of offering Restore. The same action sits on each row of "Integrations your company offers" (§6.15, `vendor-owned-retire.ts`), which is the only place a third-party owner sees its pairs. There the confirmation names both products and, on a pair, the connector whose count also drops.
...
**Evidenced pairs (AECI-1091 — 2026-09-23, ruling D).** AECi retires and restores a vendor-held `connector_evidenced_pairs` row through the same two routes.
...
3. an UPDATE of an unclaimed curated row that changes any field in the key: its endpoints (as a pair), its connector, its `mechanism_kind` or its owner. A kind change (`api` to `native` beside a vendor's `native` row) or an owner change (to unknown, or to the vendor's own id) creates a match as surely as a re-point. An UPDATE that changes none of the four, a direction swap included, leaves the key as it was, cannot create a new match, and is not asked. An UPDATE that changes a key field is skipped only for a **new** twin (ruled on AECI-1012): a vendor-held row the stored row already twinned does not count. The guard exists to stop promote creating a twin, so an already-twinned curated row keeps receiving curator updates, the owner backfill included. The row being updated is never its own twin either, so a claim on it mid-promote aborts as `INTEGRATION_CLAIMED_DURING_PROMOTE`, not as a twin.

When the result has a strong match that is **vendor-held** (claimed, or `origin = 'vendor'`), **live or retired**, promote writes nothing for that edge (no partial update) and reports `{ kind: 'integration', reason: 'VENDOR_OWNED_TWIN', existingId }` in `skipped[]`, with a `promote.blocked` audit row. An in-batch sentinel covers a vendor create that lands between the plan read and the commit: the promote aborts with `409 VENDOR_OWNED_TWIN_CREATED_DURING_PROMOTE` and a re-push gets the skip, unless the edge is an UPDATE of a row that already matched the new vendor row, which the re-push writes. It never deletes. `REVIEW_APP_PROMOTE_API.md` §4c is the review-app contract, and AECI-1047 is the review-side follow-up. This guard, covering all three writes, is what closes the §4.6.2 promotion gate. **Its evidenced counterpart (AECI-1088)** skips a curated insert, move in or key-changing UPDATE into `connector_evidenced_pairs` that lands on a vendor-held pair's (connector, A, B) key, with the same `VENDOR_OWNED_TWIN` entry, instead of letting it fail the whole promote on the unique index (`REVIEW_APP_PROMOTE_API.md` §4c).
...
| Data quality | New check `vendor_integration_unclaimed` (`warn`): a vendor-created row with no claim. Two paths lead to it: the last-seat hand-back (AECI-989, §4.5.3), or an AECi `owner` accept that reassigns the row. Until the new owner claims it nobody can edit or retire it, and promote never will, because the fence keys on `origin = 'vendor'` too (§4.7.3) (`ADMIN_PANEL_SPEC.md` §23.1). |
...
- **Is it the recorded owner of any live integration?** The owner is `built_by_vendor_id`, shown publicly as "Offered by". Read **"Integrations owned"** on the claim card, on `/admin/claims/:id` or in the Basics list of `/admin/vendors/:id` (AECI-1041). It counts **both** delivered-tier tables. A third-party owner's rows are mostly in `connector_evidenced_pairs`, because promote routes an edge whose connector is a third product there (§8.10(5)). A count over `integrations` alone reads zero for exactly the vendors this step exists for.
...
> **Tell the claimant what the seat does in v1.** Decision 9 of epic AECI-1003 stays as written for v1, and it also blocks the claim (ruled 2026-09-21 and 2026-09-22, `STAGE_2_SPEC.md` §8.10(5)). A third-party owner's integrations are connector-powered, so in v1 its seat does nothing to them. It cannot claim, edit, retire or create them, and it never sets per-side links because it owns neither side. The seat does carry its vendor profile, its connector product listing, taxonomy, the "How teams use it" narrative and analytics. Claiming, editing and retiring its own connector-powered integrations is **AECI-1040**, not yet built. Say so before arranging payment, so nobody pays for a capability that does not exist yet. *Ruled 2026-09-23 (AECI-1040):* an owner with an active entitlement may claim, edit and retire or restore its connector-powered rows (`STAGE_2_SPEC.md` §8.10(8)). **The claim is built (AECI-1089):** an entitled owner can claim today, and sees its rows in the portal (§6.15). **Edit (AECI-1090) and retire and restore (AECI-1091) are built too,** on the same entitlement. A §8.9 seat without an entitlement row still cannot claim, edit or retire.
>
> **Console gap CLOSED — AECI-1041, 2026-09-22.** The console now answers the first question. `AdminClaim` and `GET /api/admin/vendors/:id` carry `owned_integrations` (`{ integrations, connector_evidenced, total }`, `API_CONTRACTS.md` §6.10). `total` is the number to read. What changed on screen:
>
> - **The claim card and `/admin/claims/:id`** show an "Integrations owned" signal beside "Products by role".
> - **The banner above Grant and Reject has three forms on a pure connector vendor.** If it owns live integrations, it reads as a paying owner: Grant is right if the claimant wants to manage them, and the catalogue-only alternative and the v1 caveat above are named. If it owns none, it is AECI-738's banner unchanged: do not Grant or Reject. If the count could not be read, it tells the operator to check this step before parking.
> - **`/admin/vendors/:id`** labels the row "Integrations owned" and shows the split. The pure-connector chip changes for an owner. The "Add a seat" control warns when the vendor owns integrations (step 9).
> - **Every one of these warns and none gates.** The console answers only "is it an owner". Whether it wants to manage those integrations is still the claimant's answer, which is the second question above.
>
> The count is one module, `apps/api/src/lib/vendor-owned-integrations.ts`, read by both screens. It takes the AECI-1010 live predicate on `integrations` (`retired_at IS NULL`) and, since AECI-1091, the same predicate on `connector_evidenced_pairs`. It is lockstep site 14b in `apps/api/src/lib/count-lockstep.spec.ts`.
>
> **Fallback, if the console is unavailable.** The same count as a read-only query against the environment's D1. Replace `VENDOR_ID` with the vendor's id.
>
> ```sql
> SELECT 'integrations' AS tbl, COUNT(*) AS owned FROM integrations WHERE built_by_vendor_id = 'VENDOR_ID' AND retired_at IS NULL UNION ALL SELECT 'connector_evidenced_pairs', COUNT(*) FROM connector_evidenced_pairs WHERE built_by_vendor_id = 'VENDOR_ID';
> ```
>
...
>
> **What this does NOT change** is the rest of the procedure. Grant is still wrong (step 2), Reject is still wrong (step 3), and the claim is still not moved to `resolved` by provisioning — that status would read as approved in the Resolved tab with a paid account behind it, which is not what happened. Record the handover in the operator note (step 6).

> **Seat surface SHIPPED — AECI-724, 2026-09-23.** The seat now does something, and it says what it is. Three changes, in the order an operator meets them.
>
> - **Hand the lane over first.** The seat edits mappings only on a catalogue whose `managed_by = 'vendor'`, which is AECI-720's flip on `/admin/connectors/:id`. On a review-managed catalogue every edit is **409 `CATALOG_REVIEW_MANAGED`**, because the review app's next sync page would overwrite it. So the full handover is: provision the seat (this step), then flip the catalogue. Either order works; the seat is inert until both are done.
> - **The seat's route.** `PATCH /api/vendor/connector-stub-mappings/:id` edits one mapping's product pointer and depth (`status`, `product_id`, `confidence`, `evidence_url`). Authorization is the seat plus ownership: the caller's `vendor_id` must hold the catalogue's `connector`-role product through `product_vendors`. **No `requireCapability`, no entitlement read** (§8.9(2)). A non-owner gets a flat 404 before any other check. Contract in `API_CONTRACTS.md` §6.14; scope in `AUTH_AND_RLS.md` §4.4. **The portal screen shipped with AECI-1083 (2026-09-24):** a Catalogue tab on the connector product, read-only while the catalogue is review-managed (§6.16).
> - **The plan panel no longer upsells the seat.** A seat with no entitlement row whose vendor holds a `connector`-role product lands on a **`catalogue`** state: a "Catalogue maintenance seat" chip, one sentence on what it maintains, and no call to action. It replaces the `none` panel's "Ask about vendor access"; it does not soften it (`STAGE_2_SPEC.md` §8.9(5)). The signal is `entitlement.status === null` plus `products[].product_role` on `GET /api/vendor/me`. No tier and no capability. **The read-only notices use the same signal (AECI-1082).** For this seat the overview's paused row and the profile, product and facet form notices say profile and product details stay with the AECi team, with no "back on" and no renewal path. Ordinary vendors keep the paused copy.
>
> Operators edit the same mappings from `/admin/connectors/:id` (`ADMIN_PANEL_SPEC.md` §5.9). An admin edit stamps `decided_by = 'aeci-operator'`; a seat edit stamps `vendor:{slug}`. Both make a `mapped` row publishable on the public reach line.
...
- **One line and at most one action, by state.**
  ...
  - Owner of a connector-delivered row (`attestable: false`), **since AECI-1089**: with an active entitlement, the claim hint and **Claim this integration**. Without one, a sentence that claiming an integration delivered through a connector needs an active plan, and no button. Once claimed (AECI-1090): **Edit details** with an active entitlement. The form leaves out the type picker and says AEC Integrations sets the type, and only the ten other fields are ever sent. Without an active entitlement, a line that it owns the row and a sentence that editing it needs an active plan, with no form. `INTEGRATION_ENTITLEMENT_REQUIRED` has its own save error. The entitlement is read through `vendorHasActiveEntitlement` (`vendor-capabilities.ts`), the client half of the server's gate. Before AECI-1089 this state was a sentence that it could not be claimed or edited, with no button. `attestable` is read off the wire, never re-derived.
...
- **Seat-only.** Never gated on `canWrite` or the entitlement, like the contest form (§11b.10). The one exception is the connector-delivered claim above, which follows the server's AECI-1089 entitlement gate.
...
- **Preview.** `/preview/vendor-dashboard` claims and edits against the fixture (`preview-vendor-api.ts`). The Autodesk Build card under Summit Field Issues is the owned, unclaimed row. The Acumatica (Agave) card under Summit Model Coordination is a claimed connector-delivered row (AECI-1090). `?edit=<integration id>` on a child route renders that card's form open on first paint, for the design detector.
...
- **Component:** `components/vendor-owned-integrations.ts`, mounted by `sections/vendor-integrations-page.ts` between the integrations list and the Connectors section (§6.13). It renders nothing, and takes no space, when the vendor owns no row that belongs on this tab.
- **Read:** `owned` on `GET /api/vendor/integrations` (`API_CONTRACTS.md` §6.14), held by `VendorPortalStore.ownedIntegrations`. It rides the same fetch as the list, so the `integrations` cursor (`STAGE_2_REALTIME_SPEC.md` §2.2) keeps it live.
- **Which tab lists a row:** the tab of any product the row touches as an endpoint or as its connector. A row that touches none of the vendor's products has no natural tab, so it is listed on every tab rather than hidden on all of them (`ownedRowsForProduct`).
- **One card per row.** The title is the row's name, or "{A} and {B}" when it has none. A line says which two products it connects and through which connector. Then one status line and at most one action:
  - unclaimed, with an active entitlement (or on a row that is not connector-delivered): the claim hint and **Claim this integration**;
  - unclaimed, connector-delivered, no active entitlement: a sentence that a plan is needed, and no button, so nothing collects a 403;
  - claimed: a line that the vendor owns it, and **Edit details** (AECI-1090). It opens the card's edit form (`components/vendor-integration-edit-form.ts`, shared with §6.14), seeded from the row's `contestable_fields` and framed against `product_a`. On a connector-delivered row the type is left out and never sent. On a claimed connector-delivered row with no active entitlement, a sentence that editing needs an active plan instead, and no button;
  - retired: who retired it.
  - **Retire and restore (AECI-1091, `components/vendor-owned-retire.ts`).** Below the state line of a claimed row: **Retire integration**, which opens an in-page confirmation naming both products and, on a pair, the connector whose count also drops. Only its own button sends. On a retired row the owner retired: **Restore integration**. On an AECi retire: a sentence that only AEC Integrations can restore it. On a connector-delivered row the vendor needs an active entitlement for either: without one, a live row offers nothing more (the plan sentence above covers it) and a retired row says restoring needs a plan. A retired row stays listed (the read never filters `retired_at`), so the owner can always reach Restore. Pessimistic: it announces, revalidates `integrations` and `contests`, and moves focus to the row's status line after a retire and to Retire after a restore.
- **Pessimistic.** The claim waits for the `200`, announces through `VendorPortalAnnouncer`, revalidates `integrations` and moves focus to the row's Edit trigger that replaces the button, or to its status line when the row offers none. The edit does the same, and returns focus to its trigger when the form closes. Each refusal renders its own sentence in a `role="alert"` on that row (`claimErrorMessage`, which now maps `INTEGRATION_ENTITLEMENT_REQUIRED`). No browser dialog.
- **Design anchor.** The sibling Connectors section on the same tab (§6.13): its section heading, card border and spacing. The action reuses the claim button of §6.14. No new Mobbin reference.
- **Preview.** `/preview/vendor-dashboard/products/summit-model-coordination/integrations` shows two owned evidenced pairs (one unclaimed, one claimed) and an owned connector-delivered card on the list. The "No access" preset shows the plan sentence instead of the Claim button.

**Tests.** `vendor-owned-integrations.spec.ts` (the list's `owned` rows from both tables, the exclusion of rows the list already carries, cross-vendor isolation, and the cursor statement against the list for four vendors), `vendor-integration-claims.spec.ts` (the claim on both tables, the entitlement refusal and its order, the race, the owner re-point, notifications and tags), `vendor-owned-integrations.component.spec.ts` and `vendor-integration-ownership.component.spec.ts` (every state, the claim, each refusal, focus, and since AECI-1090 the edit on both surfaces). Since AECI-1091, `vendor-owned-retire.component.spec.ts` (each retire state, the confirmation, the entitlement cases, focus) and `vendor-integration-retire.component.spec.ts` (the card on a connector-delivered row).

### 6.16 As built — the connector catalogue seat's Catalogue tab (AECI-1083 — 2026-09-24)
...
- **Two states, by `managed_by`.** `vendor`: every mapping has an **Edit** button. `review`: a sunken note says the AECi team maintains the catalogue for now, so it is read-only, and no Edit renders, because the PATCH would be `409 CATALOG_REVIEW_MANAGED` (the §5.9 rule: a control that can only fail is not rendered). The note does not say "paused" or "back on": this seat is never sold access (AECI-1082).
- **The edit** (`components/vendor-connector-mapping-form.ts`). The admin `MappingEditControl`'s four fields, inline under the row: Status (with a one-sentence meaning under it), Product on AECi (found through the public product search), Confidence, and an optional https evidence link. Hints are in `aria-describedby` from first render and an error is appended to them. The form says saving records the company as the decider, because that is what makes a matched row count publicly. One form at a time. Opening it moves focus to Status, and closing it returns focus to Edit.
- **Save is pessimistic** (a form-shaped write, `STAGE_2_REALTIME_SPEC.md` §5). The PATCH echo is already in the vendor mapping shape (AECI-1127: no `notes`, `decided_by` as a kind) and is spliced into the row, with no refetch, and the save is announced through the portal's one live region. Each refusal is a `role="alert"` beside Save: `MAPPING_CONFLICT`, `VALIDATION_FAILED` on the product, `NOT_FOUND`, `RATE_LIMITED`, an ended session, and a generic retry. `CATALOG_REVIEW_MANAGED` is different: the lane went back to AECi mid-edit, so the form closes, the list re-reads as read-only, and a list-level alert takes focus to say the change was not saved.
- **What it cannot do.** Add a match to a listing with none: AECI-724's PATCH edits existing rows and there is no create. The tab says a listing with no match is matched by the AECi team, and renders no control on it. Tracked as AECI-1126.
- **Live.** Inside the AECI-516 cursor as the `catalogue` scope (`STAGE_2_REALTIME_SPEC.md` §2.2), because the vendor's own seats move it. The store turns a moved scope into a tick (`VendorPortalStore.catalogueRevision`), and the tab re-reads its open page without blanking it. With a form open it does not: it shows "This catalogue changed elsewhere" and a **Reload the list** button, and it catches up when the form closes.
- **Design anchor.** The portal itself: the product list's bordered row card (§6.11), the Connectors section's heading (§6.13), the overview's quiet pill, and the portal's form field classes. No new Mobbin reference. Disabled buttons swap to the sunken surface rather than dimming (DESIGN.md, AECI-982).
...
- **Since AECI-989 (2026-09-23), a ban that leaves the vendor with no unbanned seat also moves its open owner contests to AECi, and they move back when the vendor has an unbanned seat again, by the unban or by a new seat grant** (§11b.4). It hands no record back and leaves `claimed_at` and the maintenance marker alone, because a ban is reversible. Revoking the last seat is what hands the record back (`STAGE_2_ATTESTATIONS_SPEC.md` §13.9).
...
**As revised (AECI-1131, 2026-09-24).** Readers did not understand what "Account active" meant, and its only explanation was a hover-only `title` tooltip, which touch and keyboard users never saw. The label now reads **"Active on AECi"** in every rendering, and the **compact variant is gone**: the component takes a `public` (default) or `portal` variant instead. On the vendor detail hero (`public`), the label is followed by a visible link, "What this means", to `/docs/vendors/plans-and-the-account-label`, replacing the removed tooltip. In the vendor portal's plan panel (`portal`, `vendor-plan-panel.ts`), the vendor sees the identical label text with no link, because the panel already carries its own framing sentence — this keeps `STAGE_2_PAID_TIERS_SPEC.md` §8.1 item 3 true. The badge was **removed** from the product-pair page rails (both endpoints), the product detail page's vendor card, and the `/search` Vendors-tab card (see §8.2's revision below). It now renders in exactly two places: the vendor detail hero and the vendor portal plan panel. The Algolia vendor record still carries `verified`; nothing reads it for display, and the field stays so dropping it later needs no reindex. "Managed" and "maintained" were deliberately avoided as label wording, to keep clear of the separate "Vendor-maintained" `MaintenanceMarker` concept used elsewhere in the product.
...
The account label landing on the detail pages exposed a copy contradiction: the Actions-sidebar claim CTA (AECI-128) was **unconditional**, so an active vendor account rendered the account label and "Claim this listing" on the same page. **As revised (AECI-1131, 2026-09-24)**, the badge itself no longer renders on the product page (§8.1 above), but the CTA logic below is unaffected: it reads `verified` directly, not the badge's presence, so the reworded copy still applies on both the vendor and product pages.
...
- **Vendor record only, account label on the `/search` Vendors tab.** The legacy `verified` bit is denormalized onto the Algolia **vendor** record, and the reused `aec-vendor-account-badge` (§8.1) renders under the company name on `SearchVendorCard` (`apps/web/src/app/search/search-vendor-card.ts`). The **product** search card is deliberately left untouched. It is also the only freshness-clean choice: the §3 grant bumps `vendors.updated_at`, so the vendor index catches the flip on the next nightly window, but never touches `products.updated_at`. A product-card label would therefore go stale until every product was separately reindexed. This resolves the Linear AC's "product (and vendor, if applicable)" to **vendor**.
  ...
  **As revised (AECI-1131, 2026-09-24).** The label no longer renders on `SearchVendorCard`. Comparing vendors in a results list is the same "no reader benefit" case as the product-pair rail, so the §8.1 revision above removed it there too. The Algolia vendor record still carries `verified` (nothing else in this bullet changes: the field, the four lockstep edits, and the nightly freshness contract all stand), it is simply unread by any card now.
...
- **The connector-vendor catalogue-maintenance seat** — the return side of the `STAGE_2_SPEC.md` §8.8 payer carve-out, settled as policy in **§8.9** (AECI-704). **The admin half SHIPPED 2026-08-31** (AECI-722 — `/admin/connectors`, `ADMIN_PANEL_SPEC.md` §5.9); the vendor seat is **AECI-724, SHIPPED 2026-09-23** (§5.2 step 9). ***Provisioning* the `vendor_admin` role SHIPPED as AECI-740 (2026-09-03)** — `POST /api/admin/vendors/:id/seats`, §5.3 — so the sentence this bullet used to carry, that *no admin route writes `role = 'vendor_admin'` on its own*, is no longer true: exactly one does, and it opens no entitlement row. Deliberately **not** an entitlement row and **not** a capability id: it authorizes on this doc's existing `vendor_admin` + `vendor_id` primitive, which `findVendorProfile`'s `leftJoin` already supports for a vendor with no entitlement. AECI-724 shipped §8.9(5)'s plan-panel replacement and the seat's mapping edit as an API route (`PATCH /api/vendor/connector-stub-mappings/:id`). What remains deferred is a portal **screen** for that edit.
...
**An owner with no active seat gets no owner-routed contest (AECI-989, ruled 2026-09-23).** While the owner vendor has no unbanned `vendor_admin` seat, a content contest that would route to the owner routes to `aeci` instead. It is stamped with `owner_seat_lapsed_at` and the owner gets no notice. A ban of the owner's last active seat, or a revoke that leaves only banned seats, moves its open owner contests to AECi the same way, stamped (`integration.contest.rerouted`, `metadata.reason = 'owner-seat-lapsed'`). When the vendor has an unbanned seat again, by an unban or by a new seat grant (the admin provision, the claim grant or the invite redeem), every stamped open contest routes back, if the row is live and has been claimed by that owner since before the stamp (`reason = 'owner-seat-restored'`). Revoking the owner's last seat moves them to AECi for good, unstamped, as part of the hand-back (§4.5.3). A ban never changes `claimed_at`. Builders: `lib/vendor-handback.ts`. Contract: `STAGE_2_ATTESTATIONS_SPEC.md` §13.9.
...
`integration_id` is `ON DELETE CASCADE`. A retraction deletes the `integrations` row and takes its contests with it. This is accepted because it can now happen only to unclaimed rows, which carry no owner-side state. AECI-1005 closed the other half: promote refuses the cross-table move on a claimed row, and the retraction consumer refuses to delete a claimed row (§4.5.5).

**A promote cross-table move no longer deletes contests (AECI-1110, built 2026-09-24).** The move (AECI-721 in, AECI-888 out) drops the source row, which used to cascade every contest on the edge away, open or closed, protest history included. Now the move's own batch re-anchors each contest onto the destination row before the drop, in both directions, the way it already re-homed claims (`REVIEW_APP_PROMOTE_API.md` §3.4a). The rules, in `apps/api/src/routes/promote-contests.ts`:

- **Every contest moves, whatever its status.** The id is preserved across the move, so the contest's wire `integration_id` does not change. Only `anchor` does. A closed contest keeps its decision, its protest columns and its workflow. Routing is untouched: it stays frozen at submit (§11b.4).
- **A `direction` contest is re-framed.** Its `current_value` and `proposed_value` flip (`a_to_b` ↔ `b_to_a`) when the payload's source sorts second, the AECI-996 rule the claims use, so the values stay in the stored frame of the table they now sit in (§11b.13).
- **An open `mechanism_kind` contest is closed on a move into `connector_evidenced_pairs`.** That table has no `mechanism_kind` column, so the field cannot be contested there (§11b.13), and the move is upstream's answer to the question. It closes as `withdrawn` by the system, with its workflow closed and an `integration.contest.withdrawn` audit row carrying `metadata.reason = 'edge-moved-to-connector-tier'`. It then moves like every other contest, so its history survives. A closed `mechanism_kind` contest simply moves. No notification is sent: the submitter sees the new status in its Submitted list, because `updated_at` moves the `contests` cursor. A move back into `integrations` closes nothing, because every pair field exists there.
- **Each re-anchor writes `integration.contest.reanchored`** (entity type `integration_field_challenge`, actor `system`, before and after anchor columns, `metadata.fromAnchor` / `toAnchor`, `reframed: true` on a flipped `direction`), in the promote batch. `updated_at` moves on every moved row.
- **A race rolls the whole promote back.** Each guarded UPDATE is followed by a `changes() = 0` sentinel, and a last sentinel just before the source DELETE aborts the batch if any contest still sits on the source. A contest filed, withdrawn or decided between the plan read and the batch therefore writes nothing and loses nothing: the job ends `errored` with `CONTEST_CHANGED_DURING_PROMOTE` (409), and a re-push moves the new set.

Only an unclaimed, AECi-seeded row can move at all. The AECI-1005 and AECI-1088 fences still refuse the move on a vendor-held row, and the §4c twin guard still skips a move onto a vendor-held pair, before any contest statement is planned.
...
- **Shown when `!integration.is_owner`, and on nothing else.** It is not gated on `canWrite` (the `attestation.author` capability gate since AECI-623), not on the entitlement, and not on `attestable`. A vendor without active access, or on a connector-powered edge, can still ask for a wrong public fact to be fixed. That is §11b.2 carried to the UI.
...
### 11b.12 Protest to AECi (AECI-1009 — built 2026-09-22)

**Built 2026-09-22, as this contract.** It encodes Chris's rulings of 2026-09-22 on AECI-1009 and the orchestrator's rulings of the same day, which settled every question the first draft of this subsection left open (§11b.12.15). The code:

- API: `apps/api/src/routes/vendor-contest-protests.ts` (file, reply, withdraw), `apps/api/src/routes/admin-contest-protests.ts` (decide) and `apps/api/src/lib/contest-protests.ts` (eligibility, the wire mapping, the two contest-submit refusals, the race runner).
- Shared: the protest shapes, constants and window helper in `packages/shared/src/api/integration-contests.ts`, the five events in `packages/shared/src/api/vendor-notifications.ts`, six error codes in `packages/shared/src/errors/codes.ts`.
- Web: `apps/web/src/app/vendor/components/vendor-contest-protest.ts`, mounted in each row of `vendor-contests-list.ts`, and the Protests view of `apps/web/src/app/admin/contests/contest-queue.*`.
- Migration `0047_quick_makkari.sql`.
...
**Nothing about a protest is public** (ruled). No public read selects a protest column, and `apps/api/src/lib/contest-protests.spec.ts` is a source scan that fails if a module outside an allowlist of non-public ones names a protest column, or names the contests table at all. The pair page, the product pages, search and `/methodology` never say that a protest exists or how it ended. `/methodology` and `/legal/listing-accuracy` describe the process, never an instance of it (§11b.12.13).
...
| `protest_status IS NULL` | One protest per contest, by structure. A withdrawn protest cannot be refiled (ruled). |
| a **basis** exists, below | The owner declined, or stayed silent. |

**Two bases**, stored on the row as `protest_basis`:

- **`declined`.** `status = 'declined'`, and `decided_at <= now < decided_at + 30 days`. The owner declined through `POST /api/vendor/contests/:id/decision`.
- **`silence`.** `status = 'open'`, and `created_at + 30 days <= now < created_at + 60 days`. Silence counts as a decline (ruled), so the **silence-decline date** is day 30 after the contest was filed. The ruled 30-day filing window runs from that date, so it closes on day 60 (ruled).

The silence clock runs from `created_at`, which is when routing was frozen (§11b.4). A row that left the owner's inbox by re-routing is no longer owner-routed, so its clock no longer matters. `contestProtestWindow` in `@aeci/shared` is the one implementation of both windows. The API calls it, and the portal renders the timestamps the API derives from it rather than recomputing them.

#### 11b.12.3 The silence rule is computed at submit time, with no cron

**Decision: "30 days of silence" is never stored and never swept.** It is computed from `created_at` whenever something needs it. The protest route checks it, and the vendor list sends the window as timestamps so the portal can show it. The contest moves from `open` to `declined` only inside the protest batch, at the moment the submitter files, and its `decided_at` is set to the silence-decline date, not to the filing time.
...
| `withdrawn` | The submitter withdrew it. No cooldown, and no refile (ruled). | the submitter |
...
**ADD COLUMN only, hand-authored.** Migration `0046` is AECI-1046's `integrations.retired_by`, so this is `0047`, stacked on the AECI-1046 branch for the migration journal. `integration_field_challenges` is a cascade child of `integrations` and is never recreated. Four new columns carry a `REFERENCES` clause and two carry a CHECK. drizzle-kit rendered the columns as plain `ADD`s but dropped the `ON DELETE` clause from each `ADD … REFERENCES` (`docs/migrations.md` §0, "When drizzle-kit wants to recreate a table"). So `schema.ts` declares the columns with **no** table-level `check()`, the snapshot and journal are drizzle-kit's, and the SQL body is hand-authored with full FK clauses and column-level constraints, as `0044_slippery_edwin_jarvis.sql` did for `integrations.origin`. `pnpm db:generate` reports no change afterwards.
...
One index, also additive: `integration_field_challenges_protest_idx` on `(protest_status, protested_at) WHERE "protest_status" IS NOT NULL`. It serves the admin Protests view and the badge. The two new checks at contest submit read by (integration, field, submitter vendor) and are served by the existing `integration_field_challenges_submitter_idx` at launch volume.

`src/test/migration-0047.spec.ts` is the tripwire. It asserts the file is fifteen `ADD`s and one `CREATE INDEX` with every FK carrying `ON DELETE SET NULL`, that a seeded contest survives the migration unchanged, that each new CHECK rejects a bad value, and that deleting a profile nulls each of the three new profile columns. The cascade-child list in `d1.spec.ts` does not change, because no table is added.
...
**Shared Zod shapes** in `packages/shared/src/api/integration-contests.ts`: `FileContestProtestSchema` (`reason`, 1 to 2,000 characters after trimming, and `evidence_urls`), `ReplyContestProtestSchema` (`reply` and `evidence_urls`) and `DecideContestProtestSchema` (`decision: 'uphold' | 'reject'` and a **required** `note`, because the reasons are the whole of what an advisory ruling gives either vendor). `evidence_urls` is `ProtestEvidenceSchema`: at most three absolute `http(s)` URLs, each at most 2,048 characters, defaulting to `[]`. The server deduplicates them in order. No body carries the proposed value, which stays frozen at what the owner declined.
...
3. Eligibility (§11b.12.2). Otherwise `409 PROTEST_NOT_AVAILABLE`, with `details.reason` one of `aeci_routed`, `owner_unknown`, `already_protested`, `not_declined`, `owner_not_silent_yet` (with `details.opens_at`) or `window_closed`. Then `contest_open` when the submitter has **another** open contest on the same integration and field (ruling 8 from the protest side; §11b.12.8). A lost race answers the same code with `contest_changed` or `contest_open` (§11b.12.7).
...
6. The live column still holds `current_value`, compared so that `NULL` equals `NULL`. Otherwise `409 CONTEST_VALUE_STALE`. The owner changed the field since, so the submitter contests the new value instead.

A retired integration is **not** refused here (§11b.12.4).

**`POST /api/vendor/contests/:id/protest/reply`**: the snapshot owner replies, once. The row is scoped by `receivedContestsWhere(caller)` and must carry a protest, otherwise `404`. Then `409 PROTEST_NOT_OPEN` when the protest is decided or withdrawn, `409 PROTEST_REPLY_EXISTS` when a reply is on file, and `409 PROTEST_REPLY_CLOSED` when `now >= protest_reply_due_at` (ruled). Then the body. **The reply does not re-check current ownership.** It writes nothing to the catalog, and the vendor that decided is the right one to explain the decision.

**`POST /api/vendor/contests/:id/protest/withdraw`**: the submitter withdraws an open protest. No body. `404` for anyone else, and `409 PROTEST_NOT_OPEN` when the protest is absent or not open.
...
**New error codes**, all `409`, in `packages/shared/src/errors/codes.ts` and `API_CONTRACTS.md` §4: `PROTEST_NOT_AVAILABLE`, `PROTEST_NOT_OPEN`, `PROTEST_REPLY_EXISTS`, `PROTEST_REPLY_CLOSED`, and, on the contest submit, `CONTEST_PROTEST_OPEN` (`details.contest_id`) and `CONTEST_COOLDOWN` (`details.until`). Reused unchanged: `CONTEST_INTEGRATION_CHANGED`, `CONTEST_VALUE_STALE`, `NOT_FOUND`, `VALIDATION_FAILED` and `RATE_LIMITED`.
...
|---|---|
| **File** | The protest instance insert, first because the new FK points at it. The guarded `UPDATE`: `WHERE id = ? AND protest_status IS NULL AND status = 'declined'`, or `status = 'open'` for a silence basis, which also sets `status = 'declined'` and `decided_at` = the silence-decline date and leaves `decided_by` and `decision_note` NULL. The `changes()` sentinel. `contestIntegrationStateSentinel(claimed: true, ownerVendorId: owner_vendor_id)`. `contestValueUnchangedSentinel(field, current_value)`. `noOtherOpenContestSentinel`, which aborts when the submitter has another open contest on the same integration and field, excluding this row. **Silence only:** the contest's own instance closed as `declined` / `rejected` with transition reason `owner silent 30 days`, and an `integration.contest.lapsed` audit row. The protest transition `null → open`. The `integration.contest.protested` audit row. A `notification.sent` row to the owner, event `protested`. |
...
Every step sets `updated_at = now` on the contest row. Every audit row uses `entity_type = 'integration_field_challenge'` and `entity_id` = the contest id, like §11b.7. `lapsed` is its own action, not `declined`, so the log never shows the submitter's seat as the one that declined. The admin audit trail labels all six actions (`audit-action-labels.ts`).

**Telling the aborts apart.** `isContestRaceError` matches any `json()` abort. `runGuardedContestBatch` decides which sentinel fired by re-reading `status === 'open'`, and that test is wrong for a protest, whose row is `declined`. So the protest handlers use `runGuardedProtestBatch`, which takes a classifier. After an abort the file route re-reads the contest and the integration and answers, in this order: `PROTEST_NOT_AVAILABLE` (`already_protested`) when a protest landed first, `PROTEST_NOT_AVAILABLE` (`contest_changed`) when the contest's `status` moved (the owner decided first), `PROTEST_NOT_AVAILABLE` (`contest_open`) when a contest on the same field was filed meanwhile, `CONTEST_INTEGRATION_CHANGED` when the claim or owner moved, and `CONTEST_VALUE_STALE` when the column moved. The reply route answers `PROTEST_NOT_OPEN`, `PROTEST_REPLY_EXISTS` or `PROTEST_REPLY_CLOSED`. Withdraw and decide answer `PROTEST_NOT_OPEN`. Anything that is not an abort rethrows.

**After commit:** the §26.5 forward of every audit row through `afterVendorWrite(c, [], audits)`, or the admin route's forwarders. **No cache purge, no re-crawl, no Algolia write and no Linear issue** (ruled). Nothing public changed, and an advisory ruling gives the review app nothing to apply.

#### 11b.12.8 Time rules, exactly

Every duration is a whole number of 24-hour periods (`86_400_000` ms) added to a stored UTC ISO timestamp by `addContestDays`. There are no calendar days, time zones or business days. The constants live in `@aeci/shared`: `CONTEST_OWNER_SILENCE_DAYS = 30`, `CONTEST_PROTEST_FILING_DAYS = 30`, `CONTEST_PROTEST_REPLY_DAYS = 14`, `CONTEST_PROTEST_COOLDOWN_DAYS = 90`.

| Rule | Open while | At the boundary instant |
|---|---|---|
| Declined basis | `decided_at <= now < decided_at + 30d` | closed at `decided_at + 30d` |
| Silence basis | `created_at + 30d <= now < created_at + 60d`, with `status = 'open'` | protestable at day 30, closed at day 60 |
...
1. **`409 CONTEST_PROTEST_OPEN`** when a row on the same integration, field and submitter vendor has `protest_status = 'open'`. One live dispute per field per vendor (ruled). The rule holds from the other side too: a protest is refused with `409 PROTEST_NOT_AVAILABLE` (`contest_open`) while the submitter has another open contest on the same field (§11b.12.6), on the handler read and inside the file batch.
2. **`409 CONTEST_COOLDOWN`** when a row on the same integration, field and submitter vendor has `protest_status = 'rejected'`, `protest_decided_at > now - 90d`, **and** a `current_value` equal to the field's live value now, in storage form. `details.until` is the latest such `protest_decided_at + 90d`.
...
**`GET /api/vendor/contests`.** `VendorContestSchema` gains five fields. Each defaults to `null` for deploy skew (§11b.10 explains why the web client still sees `undefined` on an API-only rollback):

- `protest`: `ContestProtestSchema` or `null`, on both sides. It carries `status`, `basis`, `reason`, `evidence_urls`, `protested_at`, `reply_due_at`, `reply`, `reply_evidence_urls`, `replied_at`, `decision_note` and `decided_at`. No profile id is on the wire.
- `protest_opens_at`, `protest_closes_at` and `protest_basis`: submitted side only. They are the window of §11b.12.8, sent while the row has no protest, passes the §11b.12.2 conditions, is not past its closing instant, **and** passes the two live checks of the file route (the same owner holds the claimed row, the value is unchanged). `protest_opens_at` may be in the future, which is how the portal says "from {date}". All three `null` means the row is not protestable and will not become so by waiting.
- `cooldown_until`: submitted side only. Set on a rejected protest while it still blocks a new contest on the field.

Both vendors see the whole record, including the other side's text, evidence and AECi's note (ruled).

**The list order is `updated_at DESC, id ASC`** on both sides (ruled). It was `created_at DESC`, capped at 100 rows per side. A protest can land on a contest filed months ago, and under the old order that row could sit past the cap and never show while the cursor still moved for it.

**The `contests` cursor needed no change.** Every protest step moves the row's `updated_at`, and the row is already inside `vendorContestsWhere` for both parties: the submitter through `submitter_vendor_id`, the owner through `routed_to = 'owner' AND owner_vendor_id`. The predicate is untouched, so the invariant in `STAGE_2_REALTIME_SPEC.md` §2.2 holds with no new SELECT.

**One client refetch was added** (ruled). `VendorPortalStore.revalidate` also refetches `contests` when an `integrations` refetch runs and the contests have already been loaded. Both `cooldown_until` and protest eligibility depend on the live field value, and an owner edit moves `integrations`, not `contests`. It never loads contests from cold.

**`GET /api/admin/contests`.** The query gains `protest_status` (optional). When it is present, the list filters on `protest_status` alone and ignores `status` and `routed_to`, because every protested row is `declined` and owner-routed. It orders by `protested_at DESC, id ASC` on the new index. `AdminContestSchema` gains `protest` (default `null`) and `owner_changed` (default `false`), which is true when the integration is no longer claimed by `owner_vendor_id`.

**The badge.** `pending_contests` adds `OR protest_status = 'open'` (ruled). The new term is disjoint from the existing ones, because an open protest's `status` is `declined` and the existing terms all require `open`. It stays one badge for one screen.
...
|---|---|---|---|
| `protested` | owner | "A vendor asked AEC Integrations to review a contest on your integration" | "You can reply once, by {date}, under Field contests in Messages. Nothing about it is public." |
...
| `protest_rejected` | submitter | "AEC Integrations agrees with the owner" | "You can't contest this field again until {date}, unless its value changes." |
...
**Deploy skew.** `contestNotificationTitle` in `vendor-contest-labels.ts` now has a `default` branch, "An update on a field contest", so a web build older than the API still renders a title for an event it does not know.

#### 11b.12.11 The admin queue (`/admin/contests`)

- **A "Show" switch** (Contests / Protests) sits before the existing filters. Protests replaces them with its own status tabs: Open (the default), Agreed with the submitter, Agreed with the owner, and Withdrawn. They map onto `protest_status`, and the intro sentence changes to say what a protest is and that the answer is advice.
- **A protest card shows both sides in full.** The contest as before (pair, field, the value on record, the proposal, the contest reason, the owner's decline note). Then the protest: why it came here ("The owner declined the contest." or "The owner did not answer the contest within 30 days."), the submitter's case and links, the owner's reply and links or "Reply due {date}" or "No reply by {date}", and our note once decided. "On the integration now" appears when the live value differs, and "The owner has changed since the decision" when `owner_changed` is true.
...
**Submitter: Field contests → Submitted** (`vendor-contest-protest.ts`, mounted in each row).

- A row inside its window shows **Ask AEC Integrations to review**. It opens an inline form that names the basis ("The owner declined this contest." or "The owner did not answer within 30 days, so this counts as a decline."), says AEC Integrations reads both sides and says which it agrees with, that its view is advice, that only the owner can change the listing, that the owner can reply once and that nothing about a review is public, and gives the deadline. It asks for a required reason and up to three links, added one at a time. An empty reason or a link that is not a web address is refused before anything is sent.
- A row before day 30 shows "You can ask AEC Integrations to review this from {date} if the owner has not answered."
- A protested row shows the record, headed "Review by AEC Integrations", with a status pill ("With AEC Integrations", "AEC Integrations agreed with the contest", "AEC Integrations agreed with the owner", "Review request withdrawn"). An open one has **Withdraw review request** behind an inline confirmation, never `confirm()`, with focus moved to the confirm button and back.
- A decided protest shows what it means from this seat and AECi's note. A lost one shows the cooldown end.
- A contest declined by silence shows the pill "No answer in 30 days" rather than "Declined".
- Errors map to plain copy (`protestErrorMessage`). A refusal that means the row moved reloads the list so it shows the state that won.

**Owner: Field contests → Received.**

- A row with an open protest shows the whole protest and a one-time reply form (a reply and up to three links) with the due date. Past the due date, or once a reply exists, the form is gone and the reply, or "No reply", shows.
- A decided protest shows what it means from the owner's seat and AECi's note. An upheld one links "Edit the integration under {product}" to that product's Integrations tab, where the ordinary owner edit lives. It is not a new write.
- **Overview.** "What needs you" gains one Needs-you-now row for open protests with no reply and the due date still ahead, linked to Messages.

**The contest form.** A field with an open protest is a disabled option reading "(with AEC Integrations for review)". A field inside a cooldown is disabled with its end date. `CONTEST_PROTEST_OPEN` and `CONTEST_COOLDOWN` map to plain copy for a race the form could not see.

**The preview** (`/preview/vendor-dashboard`) carries a protestable submitted contest, a received contest with an open protest, and a submitted contest with a lost protest, with windows relative to the real clock, and fixture-side versions of the three vendor routes.

Every string is `$localize` or an `i18n` attribute. The theme is light only. Writes are pessimistic. Outcomes go through `VendorPortalAnnouncer` (§11b.10's patterns).

#### 11b.12.13 GDPR erasure and copy
...
**Copy changed in this build.** Before it, every page below described a decline as the end of the contest.

| File | Change |
|---|---|
| `apps/web/src/content/methodology.md`, "Contests" | A paragraph, "Asking us to review an owner's decision": the 30-day windows (from a decline, or from 30 days of silence), one owner reply within 14 days, advice that does not change the integration, the 90-day block unless the value changes, and nothing public. |
| `apps/web/src/content/legal/listing-accuracy-policy.md` | The "Declined" bullet points to the review route, and a subsection "Asking us to review an owner's decision" states the same rules in the policy's register. The orchestrator ruled that counsel review does **not** block the merge: the legal documents are already under AECI-308's counsel review, and this wording joins that list. |
| `apps/web/src/content/docs/reviewers/requests-and-corrections.md` | The "Declined" item gains the review route. |
| `apps/web/src/content/docs/vendors/owning-an-integration.md`, "Contests you receive" | A paragraph on the one reply, and that AECi's view is advice. |
| `vendor-contest-labels.ts` | The `declined` note offers the review when the row carries a date (§11b.12.10). |
| `methodology.component.spec.ts`, `legal-page.component.spec.ts` | Assert the new sentences. The legal spec's old assertion that the policy never mentions a protest was replaced. |

The marketing repo's vendor copy (AECI-1045) lives outside this repo. Its owner is told when this ships.

#### 11b.12.14 Tests

- **API.** `migration-0047.spec.ts` (§11b.12.5). `routes/vendor-contest-protests.spec.ts` covers: both bases at each window boundary; every `PROTEST_NOT_AVAILABLE` reason; the 404s; `CONTEST_INTEGRATION_CHANGED` and `CONTEST_VALUE_STALE` on the handler read and, through a racing factory, inside the batch; the race with an owner's late decision in both orders; a retired integration accepted; the reply's once-only rule and due instant; withdraw with no cooldown and no refile; the required note; both notifications with `recipientRole`, and none to a deleted owner; no `integrations` write and no purge; the Protests list view and the badge; `CONTEST_PROTEST_OPEN`; `CONTEST_COOLDOWN` at its boundary, lifted by an owner edit, not started by an upheld protest; the window on the submitter's list only; the feed metadata; the cursor for both parties and not a third; and the batch size. `lib/contest-protests.spec.ts` is the public-read source scan: every module that names a protest column, or the contests table at all, must be on an allowlist of non-public modules. The file route's tests also cover `contest_open` on the read and, through the racing factory, inside the batch.
- **Shared.** `integration-contests.spec.ts` covers both windows, the half-open phase at each boundary, the evidence rule, the required note and the day arithmetic.
- **Web.** `vendor-contest-protest.component.spec.ts` (the request form, its refusals, the not-yet line, withdraw, the decided record and cooldown, the owner's reply, the reply past due, the upheld edit link), new cases in `contest-queue.component.spec.ts` (the Protests view, the required note, the badge, `PROTEST_NOT_OPEN`), `vendor-notifications-list.component.spec.ts` (every protest event from each seat, the dated decline note, the fallback), `vendor-contest-form.component.spec.ts` (the cooldown option), `vendor-overview-model.spec.ts` (the Needs-you-now row) and `audit-action-labels.component.spec.ts` (the six actions).

#### 11b.12.15 Rulings

Chris ruled on 2026-09-22 that the ruling is advice, that the windows are 30 days to file, 14 days to reply and a 90-day cooldown, that 30 days of silence counts as a decline, that only owner-routed contests are protestable, and that nothing is public. The orchestrator ruled the rest the same day, within Chris's rulings:

1. **The silence window closes.** The silence-decline date is day 30 after the contest was filed, and the 30-day filing window closes on day 60.
2. **A protest carries a reason and up to three evidence URLs**, and so does the owner's reply.
3. **AECi's note is required** on both outcomes.
4. **A reply at or after the due date is refused.**
5. **The submitter may withdraw an open protest**, with no cooldown and no refiling of that contest.
6. **A retire leaves an open protest alone, and a retired integration can still be protested.** The shipped rule that a retire withdraws open contests and a restore does not reopen them stays unchanged, recorded as a known v1 gap (§11b.12.4). Chris is being asked about it separately.
7. **A protest needs the same owner and the same value** as the decision it protests, or it is refused with `CONTEST_INTEGRATION_CHANGED` or `CONTEST_VALUE_STALE`.
8. **One live dispute per field per vendor**, from both sides: `CONTEST_PROTEST_OPEN` on a new contest, and `PROTEST_NOT_AVAILABLE` (`contest_open`) on a protest (added after the independent review of 2026-09-22).
9. **Both vendors see the whole record.**
10. **No Linear issue on any protest step.**
11. **The vendor lists sort by last update**, and an `integrations` refetch also refetches loaded contests.
...
- **Accepts write the anchor's own table, the way the AECI-1090 owner edit does.** An owner accept and an AECi accept on a pair write `connector_evidenced_pairs`. The audit row is `integration.updated` (`integration.claimed` for an owner approval) with entity type `connector_evidenced_pair`, in the same batch. On a connector-powered row in either table it carries `metadata { connectorPowered: true, anchor }`, the markers the AECI-1089 claim and the AECI-1090 edit write. An owner accept also stamps `updated_at`, which moves the owner's owned-rows cursor. A pair write purges `pair:{a}__{b}`, both endpoint `product:` tags and the connector's `product:` tag, because the connector's page lists the pair. Every accept that writes the row here then runs the owner edit's by-id Algolia sync behind `dispatchHook`, on either table, because the integrations index holds pairs too. The owner decision's entitlement gate is the same `requireActiveEntitlement` test the owner edit applies. The promote fence (AECI-1088) holds: only a claimed pair is written here, and promote never writes a claimed pair.
...
1. **A `mechanism_kind` contest on a connector-powered `integrations` row always routes to AECi (ruling A).** It never routes to the owner, even on a claimed row. That settles the conflict with ruling 5: the owner can neither edit that column nor accept a contest that writes it. An AECi accept follows §11b.6 as on any row. The rule also looks forward: a `mechanism_kind` contest whose proposed value would make a row connector-powered (`iPaaS`, `integrator`) routes to AECi, because the owner edit refuses that change (`422`) and an owner accept must not make it either. The owner decide route refuses such an accept with the owner edit's `422 INTEGRATION_INVALID_VALUE` on `mechanism_kind`, which covers a contest routed before AECI-1092; declining stays open. **A row that becomes connector-powered through an AECi accept** (a `mechanism_kind` contest to `iPaaS` or `integrator` on a claimed row) takes both rulings in the accept's own batch: every open owner-routed `mechanism_kind` contest on it moves to AECi (ruling A), and every other open owner-routed contest moves too when the owner holds no active entitlement (ruling B). Each move writes `integration.contest.rerouted` (`metadata.reason = 'became-connector-powered'`) and an `open → open` transition. An entitled owner keeps its content contests; the batch then carries `ownerEntitlementActiveSentinel` and touches their `updated_at`, so a concurrent clear either aborts the accept or re-plans against the now connector-powered row.
...
- **A contest submitted during a clear.** The clear's batch starts with a guard. It aborts unless the vendor's open owner-routed contests still have the count and the newest `updated_at` the handler read. That fingerprint binds two values, so it cannot run past D1's bound-parameter cap. The handler re-plans and retries once, then answers `409 CONTEST_INTEGRATION_CHANGED` with nothing written.
- **A clear during a submit.** When a contest routes to an owner on a connector-powered row, its batch carries `ownerEntitlementActiveSentinel`. A clear that commits first aborts the submit, which re-reads and routes the contest to AECi. A second lost race answers `409 CONTEST_INTEGRATION_CHANGED` with nothing written.
...
**Reconciled with AECI-989 (2026-09-23).** AECI-989 (main #813) routes a contest away from an owner with no unbanned `vendor_admin` seat, stamps it with `owner_seat_lapsed_at`, and routes it back when a seat returns. The orchestrator reconciled the two mechanisms from rulings 2, B and E and the AECI-989 rulings: a contest routes to, or returns to, the owner only when the owner has an unbanned seat AND, on a connector-powered row, an active entitlement, and a `mechanism_kind` contest on a connector-powered row never goes to the owner. Concretely: the submit stamps only when the missing seat is the one reason. The seat return (`planOwnerSeatReturn`, both anchor tables) returns a connector-row contest only to an entitled owner and never a `mechanism_kind` one, applies the submit's forward-looking ruling A (a `mechanism_kind` contest proposing `iPaaS` or `integrator` is never returned, even on an ordinary row; `routesAsConnectorPowered`), and clears the stamp on the rest (`integration.contest.seat_stamp_cleared`, `reason = 'owner-may-not-decide'`). The claim grant counts the entitlement it activates in the same batch. The entitlement clear, and an accept that makes a row connector-powered, clear the stamp on the contests they make AECi's for good, and every re-route to AECi clears it too. The races: a return of a connector-row contest carries `ownerEntitlementActiveSentinel`, and a seat writer that loses to a clear answers `409 VENDOR_SEATS_CHANGED` with nothing written. Every guarded return UPDATE and every stamp clear is followed by `contestRowChangedSentinel`, which aborts the batch when the UPDATE matched nothing, so a losing concurrent grant writes no `rerouted` or `seat_stamp_cleared` row and no transition, and answers the same 409. The clear's guard fingerprints the stamped set as well as the owner-routed one, so a seat event landing mid-clear makes it re-plan. Tests: `routes/contest-seat-reconcile.spec.ts`.

**Telemetry.** The admin decision forwards its audit rows and its transition in ONE `logBatchToPosthog` request (`forwardModerationBatch` in `routes/admin-contests.ts`, also used by the protest decision), and the entitlement clear batches its re-route rows the same way. An accept can carry a claim, a notification per endpoint vendor and several re-routes beside the purge, the Algolia sync and the Linear filing, so one request per row would run past the Worker connection limit. The owner decision already forwards through `afterVendorWrite`, which batches.

**Cross-table moves (AECI-1110).** A promote that moves an edge between the two tables re-anchors its contests onto the destination row instead of letting the source DELETE cascade them away. An open `mechanism_kind` contest closes as `withdrawn` on a move into the pair table, where that field does not exist. §11b.9 has the rules.
