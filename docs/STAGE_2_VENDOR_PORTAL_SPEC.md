...
> **Superseded (AECI-609, 2026-08-14).** That was the AECI-513 launch shape and the Paid Tiers epic is the successor it named. `vendors.verified` is now a **denormalized mirror** of a `vendor_entitlements` row — `true` iff an `active` row exists — and the arrangement lives in the row **as well as** the audit trail (the audit log stays the renewal ledger). The "no new schema" clause held for this epic and was spent by that one: `0019_easy_sandman` is the single migration. **Nothing AECI-513 shipped against this bullet changed** — every reader still reads `verified`, and no public or read path may query the entitlement table. See `docs/STAGE_2_PAID_TIERS_SPEC.md` §2 and `STAGE_2_SPEC.md` §8.5.
