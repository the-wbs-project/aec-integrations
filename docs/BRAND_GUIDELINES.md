...
- **Transactional email logo banner:** `apps/web/public/branding/email-logo-banner.png` — the **1120×182** wordmark (Bone type, clay connector) on a **transparent** field that heads every transactional email, served at the **hardcoded production URL** `https://www.aecintegrations.com/branding/email-logo-banner.png` and referenced by `EMAIL_LOGO_URL` in `apps/api/src/lib/email-layout.ts` and by `docs/email-templates/magic-link.html`. PNG, not SVG: Outlook and Gmail do not render SVG at all. **The background must stay transparent (AECI-924)** — with a baked Forest field the header renders as two greens in any client that dark-mode-shifts the band's CSS colour, which it cannot apply to pixels inside an image. The URL is production, never per-tier, because non-prod sits behind Cloudflare Access and would 403 in a mail client. **Do not rename, move or delete that path** — a client fetches it months after the send, and nothing in CI would notice. The repo copy at `branding/email-logo-banner.png` and the served copy must stay identical; see `branding/logo-construction.md` and `docs/email.md` (§House layout).
