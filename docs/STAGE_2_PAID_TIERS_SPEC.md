...
This is the deliberate contrast with `STAGE_2_VENDOR_PORTAL_SPEC.md` §1.2 ("no migration required"). AECI-513 could ride entirely on the §18 reserved hooks; this epic cannot, because there is no reserved entitlement table. **§2 ships migration `0024_easy_sandman`** — generated by `pnpm db:generate`, applied by `wrangler d1 migrations apply` (`docs/migrations.md`; never `drizzle-kit migrate`/`push`). It generated as `0006_*` and was renumbered **twice**: to `0018` in AECI-622, because `main` had independently taken `0006`–`0015`; then to `0019`, because the AECI-514 epic landed `0016`/`0017` **and** `0023_chilly_joseph`. `migrations.md` §0 "Renumbering a migration" is the procedure, and §0 "Reserved numbers" records the collision history.
