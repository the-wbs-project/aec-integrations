...
| `aeci.indexnow.submit` | count | `apps/api/src/routes/promote.ts` (`notifyIndexNowAfterPromote`, AECI-236 / §20.2) — the post-commit hook, dispatched via `dispatchHook` beside the Algolia sync and the cache purge | `source` (`promote`), `outcome` (`ok` / `failed`). One count per submission the hook **attempted** — a network or DNS error that never reached `api.indexnow.org` is `outcome:failed` at status `0`, not silence; `ok` covers both 200 (accepted) and 202 (received, validation pending). **A zero series is ambiguous, which is what AECI-801 was filed over.** The hook returns before the metric on three paths and none of them emit: `INDEXNOW_KEY` or `PUBLIC_SITE_URL` absent, the promote touched no public URL, and `PUBLIC_SITE_URL` set but not parseable as a URL (that last one alone leaves a trace — the `invalid_public_site_url` warn log). So "no data" means "no key", "no crawl-worthy write", "a malformed `PUBLIC_SITE_URL`", or "the PostHog metrics transport is not reporting" — only the second is benign. **Do not use this metric to answer "is the key set?"** — run `wrangler secret list --env production` against **both** Workers instead (`docs/launch-cutover-runbook.md` §2). `outcome:failed` carries its reason on the paired `aeci.api.promote.indexnow_failed` warn log. **Live state 2026-09-09: this series is 100% `outcome:failed` in production** — 23 attempts across 2026-09-07..09, every one HTTP **429**, zero successes (AECI-826). So a non-zero `failed` count here is currently the expected reading, not a new incident; the finding would be a return to `ok`. Do not cite this metric as evidence Bing discovery is working until AECI-826 lands |
