...
| `aeci.api.promote.stale_id` | count | `apps/api/src/routes/promote.ts` (`logPromoteStaleIds`, AECI-568) | `source` (`promote`), `kind` (`vendor` / `product` / `integration`) — **value = per-kind count, query with `sum:`**. The caller sent a `supabaseId` whose row no longer exists, so the ingest **created** a replacement instead of no-op-updating. Self-healing, but it means the review app was holding a dead pointer — a sustained non-zero series says the two sides are diverging. **The `integration` kind narrowed on 2026-09-14 (AECI-888):** an edge id is now looked up in **both** anchor tables, so an id that lives in `connector_evidenced_pairs` is a live pointer and is no longer counted here — it moves the row instead. A step down in this series at that date is the fix landing, not the review app repairing itself; a step down at any other time still means what it always did |
| `aeci.api.promote.unresolved_link` | count | `apps/api/src/routes/promote.ts` (`logPromoteUnresolvedLinks`, AECI-730) | `source` (`promote`), `field` (`powered_by` / `built_by`) — **value = per-field count, query with `sum:`**. An integration was written **without** its `poweredByProduct` / `builtByVendor` FK because that record isn't promoted. Distinct from `skipped` on purpose: the row DID land. **Expected to be permanently non-zero** — some connectors are never promoted (Make, n8n, Boomi), so every promote of an edge naming them fires this. Zapier and Workato were the largest source while AECI-700 parked them; AECI-1064 reversed that on 2026-09-23. **Do not alert on presence; alert (if ever) on a rise.** Paired log is `info`, not `warn`, for the same reason |
| `aeci.api.promote.taxonomy_created` | count | `apps/api/src/routes/promote.ts` (`logPromoteTaxonomyCreates`, AECI-970) | `source` (`promote`), `kind` (`category` / `audience` / `phase`), `slug` — **value = 1 per minted term**. A promote created a category / audience / phase because the value matched no stored slug and no stored name. The term is a real public browse page with no description and no display order. Paired `warn` log of the same name lists every `{ kind, slug, id }`. `slug` is a tag on purpose: mints are rare (11 in production's history to 2026-09-22), so cardinality is negligible and the series names the term. **Steady state is zero; any point is worth a look.** Goes structurally dead if AECI-970 option B makes these facets find-only |
...
| `aeci.indexnow.submit` | count | `apps/api/src/lib/indexnow-drain.ts` (`drainIndexNowQueue`, AECI-236 / AECI-826 / §20.2) — the `*/20` drain cron. **It moved**: until AECI-826 this was emitted from the promote's post-commit hook with `source:promote`, and the promote no longer submits at all | `source` (`cron`), `outcome` (`ok` / `failed`). One count per **drain tick that attempted a submission**, not per HTTP request — a throttled tick that retried three times still counts 1, which is why this series can never be read as a request count (the distinction AECI-833 corrected everywhere else). A network or DNS error that never reached `api.indexnow.org` is `outcome:failed` at status `0`, not silence; `ok` covers both 200 (accepted) and 202 (received, validation pending). **429 is the case to know.** IndexNow rate-limits per host, the limit is undocumented, and a 429 means the batch reached the aggregator and was refused — it proves DNS, TLS and egress are fine and proves **nothing** about the key, because Bing throttles before it fetches `<key>.txt`. **Since AECI-833 a bare 429 is NOT retried** — only a 5xx, a transport error, or a 429 naming a `Retry-After` inside ten seconds is, twice with backoff (`isRetryableStatus`). Either way the failed rows stay buffered for the next tick, so a `failed` count here is a submission that survived whatever retry it was entitled to. **A zero series is ambiguous and always has been**: the drain emits nothing when the buffer is empty, and nothing when `INDEXNOW_KEY` / `PUBLIC_SITE_URL` are absent. Read it beside `aeci.indexnow.drain` (always emitted, so absence there is real) and `aeci.indexnow.pending` (depth, so a stuck channel climbs). **Do not use this metric to answer "is the key set?"** — run `wrangler secret list --env production` against **both** Workers (`docs/launch-cutover-runbook.md` §2). **History: 100% `outcome:failed` in production 2026-09-07..09** — 23 attempts, every one 429, zero successes, and nothing alerted because no alert existed (AECI-826). The alert now exists; see `docs/RUNBOOKS.md` → "IndexNow submissions refused" |
| `aeci.indexnow.drain` | count | `apps/api/src/scheduled.ts` (`runIndexNowDrainJob`, AECI-826) | `trigger` (`cron`), `outcome` (`ok` / `refused` / `failed` / `skipped`). **Always emitted, so this doubles as the cron-liveness heartbeat** — it fires on the paths that make no request at all (empty buffer, no creds), which is exactly the property that makes its *absence* meaningful. `aeci.indexnow.submit` could never serve this purpose: a quiet twenty minutes legitimately produces no submission. `skipped` = no `INDEXNOW_KEY` / `PUBLIC_SITE_URL`, which is the correct pre-launch and preview posture rather than a fault. `refused` = a submission was attempted and IndexNow rejected it (a 429 included) or was unreachable. `failed` = a local fault, today only an unparseable `PUBLIC_SITE_URL`. The split exists because the combined cron-failure alert pages on any `outcome:failed`, and a throttled tick is routine (AECI-864). Refusals are judged by the ratio alert over `aeci.indexnow.submit`. A tick that **throws** (a D1 error) emits nothing here, because the count runs after the drain returns; the liveness sweep catches it. `job_runs` keeps its closed `ok` / `failed` / `skipped` set, so a `refused` tick is a `failed` row there |
...
| `aeci.indexnow.queued` | count | `apps/api/src/routes/promote.ts` (`bufferIndexNowAfterPromote`, AECI-826) and `routes/vendor-shared.ts` (`bufferVendorRecrawl`, AECI-944) | `source` (`promote` \| `vendor`). URLs a promote appended to the buffer, **after dedupe** — the value is the number of rows actually inserted, so a re-promote of an already-queued product contributes 0. Not emitted when the promote touched no public URL or the creds are absent. **AECI-944 gave the buffer a second appender and the metric a second tag value.** A vendor-portal write emits the same count tagged `source:vendor` from `bufferVendorRecrawl` (`routes/vendor-shared.ts`), so split the series by `source` before reading it — the untagged total is two arms added together. A rate that under-reports by an unknown factor would be worse than no rate at all, which is why the tag is not optional. Read `aeci.indexnow.pending` for the whole buffer's depth |
| `aeci.gsc_recrawl.queued` | count | `apps/api/src/routes/promote.ts` (`bufferIndexNowAfterPromote`, AECI-945) — the **same** post-commit hook as the IndexNow half, not a second one, because both queues are fed by one event and gated on one pair of vars. Also `routes/vendor-shared.ts` (`bufferVendorRecrawl`, AECI-944) | `source` (`promote` \| `vendor`). Rows a promote wrote to `gsc_recrawl_queue`, the Google re-crawl worklist (§20.2, `DATABASE_SCHEMA.md` §9.8). **This counts inserts AND priority-raising updates together, so it is not a dedupe-miss signal** the way `aeci.indexnow.queued` is. That is deliberate: this table's conflict path is `DO UPDATE` taking `min(priority)`, and a URL promoted from tier 4 to tier 2 is real work even though no row was added. The two halves carry independent try/catch blocks, so a Google-queue failure never suppresses a successful IndexNow buffer and vice versa. A failure logs `aeci.api.promote.gsc_recrawl_failed` and the promote still succeeds. Also emitted by the vendor arm tagged `source:vendor`, same as the sibling above. **Absence is ambiguous and always will be**: a promote touching no public URL, absent creds, and a dead metric look identical here. The unambiguous read is the `pending_reindex` count on `GET /api/admin/summary`, which is a depth rather than a rate |
...
| `aeci.algolia.sync` | count | `apps/api/src/scheduled.ts` (daily cron) + `apps/api/src/routes/promote.ts` (`syncAlgoliaAfterPromote`) + `apps/api/src/routes/vendor-integration-retire.ts` (by-id re-index after a retire or restore, AECI-1010) | `trigger` (cron / promote / vendor), `entity` (products / vendors / integrations / all), `outcome` (ok / failed / skipped_no_creds) |
...
| `aeci.algolia.sync.records` | count | `apps/api/src/lib/algolia-sync-metrics.ts` (`emitAlgoliaSyncMetrics`, from the cron, the promote hook and the retire route) | `trigger` (cron / promote / vendor), `entity` (products / vendors / integrations), `op` (saved / deleted) |
| `aeci.algolia.sync.duration_ms` | distribution | `apps/api/src/lib/algolia-sync-metrics.ts` (`emitAlgoliaSyncMetrics`, from the cron, the promote hook and the retire route) | `trigger` (cron / promote / vendor) |
...
| `aeci.linear.issue` | count | `apps/api/src/lib/linear.ts` (`createLinearIssueForRequest`, AECI-211 — the request→Linear `ctx.waitUntil` task) | `outcome` (ok / failed / skipped_exists), `kind` (claim / correction, and `contest` since AECI-1008 for the `REVIEW - ` issue `createLinearIssueForContest` files on an AECi contest accept), `reason` on failure (http_error / graphql_error / timeout / network / empty_response / db_error) |
...
| `aeci.linear.reconcile.attempt` | count | `apps/api/src/lib/reconciliation-sweep.ts` (`runReconciliationSweep`, AECI-214; `runContestIssueReconciliation`, AECI-1008) | `outcome` (cleared / still_failing) — submits the **row count** as the value, so query with `sum:`. The contest pass adds `kind:contest`; the request pass carries no `kind` tag, so filter on `kind:contest` or its absence |
...
| `aeci.entitlement.action` | count | `apps/api/src/routes/admin-entitlements.ts` (`emitEntitlementAction`, AECI-532 — the `PATCH /api/admin/vendors/:id/entitlement` set/renew/clear handler) | `action` (`set` / `renew` / `clear`), `outcome` (`ok` / `invalid_state` / `forbidden`) — one count per attempt, alongside the `vendor_entitlement.set` / `.renewed` / `.cleared` `audit_log` row written in the same `db.batch`. `invalid_state` is the §5.1 422 gate (a `set` on an already-active entitlement, a `renew`/`clear` on one that is not active); `forbidden` is the zero-capability-tier guardrail. **This is the only metric that moves when the public account label appears or disappears**, so `action:clear` is the series to watch — `outcome:ok` there means a vendor's account label is coming off every cached page and out of the next nightly Algolia push |
| `aeci.claim.moderation.action` | count | `apps/api/src/routes/admin-claims.ts` (`emitClaimModeration`, AECI-519; `action:note` added by AECI-739) | `action` (`approve` / `reject` / `note`), `outcome` (`ok` / `noop` / `invalid_state` / `conflict` / `unavailable`) — one count per attempt against a vendor claim. `approve` + `ok` activates a vendor account, so it moves in lockstep with `aeci.entitlement.action`'s grant path; `unavailable` is the 503 that fires wherever `SUPABASE_SERVICE_ROLE_KEY` is absent, which is expected on PR previews and a real incident anywhere else. `action:note` is the AECI-739 operator note (§5.2 step 6) — an annotation, so it grants nothing and its `noop` simply means the submitted text was unchanged. **Undocumented until AECI-739**: the series has existed since AECI-519 and was never catalogued here |
| `aeci.contest.moderation.action` | count | `apps/api/src/routes/admin-contests.ts` (`emitModeration`, AECI-1008) | `action` (`accept` / `decline`), `outcome` (`ok` / `not_open` / `routed_to_owner`) — one count per `PATCH /api/admin/contests/:id` attempt. `not_open` includes the loser of a two-admin race. An `accept` + `ok` is followed by one `aeci.linear.issue{kind:contest}`; a gap between the two is a stuck `REVIEW - ` issue, which the 15-minute sweep retries. No dashboard or alert; read it beside `aeci.linear.issue` |
...
| `aeci.api.ratelimit` | count | `apps/api/src/rate-limit-middleware.ts` (`rateLimit()`, AECI-773 / ADR 0026) | `bucket` (token / write), `outcome` (limited / unconfigured), `key_source` (user / vendor / ip / absent) — one count per event, never per request. **Nothing at all is emitted on the allow path**, which is the hot path and where one `submitCount` would be one outbound connection (AECI-666). Deliberately **no `endpoint` tag**: it would multiply the series by the guarded-route count, and the paired `console.warn` already carries path and method — the same argument that dropped the raw `status` tag in AECI-642. Ceiling 2 × 2 × 4 = 16 series against the 1,000-series guardrail. Two readings that matter: **`outcome:unconfigured` on any deployed tier means that tier lost its binding and is limiting nothing**, and it is emitted **once per isolate**, so read it as presence/absence and NEVER as a rate; a rising `key_source:absent` on `bucket:token` means `cf-connecting-ip` stopped arriving and the anonymous limiter has collapsed to a single shared counter (still a limit, a far blunter one). This is the APP-layer series — `aeci.waf.ratelimit.blocked` above is the EDGE one, and they answer different questions (`docs/waf-rate-limits.md` §6.4) |
...
| `aeci.analytics_digest.email` | count | `apps/api/src/scheduled.ts` (`runAnalyticsDigestJob` → `lib/email.ts` `sendEmail`, AECI-526, daily 05:00 UTC = noon Jakarta cron) | `outcome` (sent / failed / skipped) — the daily operator digest: the **requests of unresolved origin** headline (`is_bot IS NOT 1` + `NOT_INTERNAL`, less the automation filter; the raw server-side count beside it is the UPPER bound — AECI-869 retired the word "human" here) + the AECI-660 PostHog `$pageview` observation + the AECI-870 browser-starts observation (`app_started`, operator and PostHog-detected bots excluded) + the AECI-683 corroborated floor, top products, new/total sign-ins, pending-moderation depth, and a Crawler-activity breakdown (`is_bot = 1`, grouped by `bot_name`). This metric carries only the send outcome — the figures themselves land in `job_runs.detail` (`AnalyticsDigestSummary`), which is where to look when a number moves and you want the history rather than one morning's email. **`skipped`** when `RESEND_API_KEY` / `EMAIL_FROM` / `ANALYTICS_DIGEST_EMAIL_TO` are unset (fail-open); one count per run, so the always-emitted series doubles as the cron-liveness signal (`outcome:failed` also covers a pre-send crash) |
...
- **`job_runs` owns the record.** Outcome, duration, and a per-job payload, in
  ...
  payload is the whole result set, which is a thing no metric can carry.
...
**Since AECI-827 the same cron emits a SECOND family, `aeci.metrics_snapshot.recheck.*`** (ADR 0027).
The 00:15 job now runs a correction-only pass after the capture, re-checking the trailing ~33 days
because `NOT_INTERNAL`'s operator retro-join makes a stored day a function of rows that arrive later.
Read the two families separately and do not sum them: the primary family answers *did we capture
yesterday*, the re-check family answers *is the last month still converging*. Three readings matter.
`recheck.run{outcome:ok}` with zero `recheck.correction` counts is the healthy steady state.
`recheck.run{outcome:skipped}` is the guard refusing to rewrite a permanent record — the named repair
is `ops:backfill-metrics-daily`, dry-run first — and `recheck.refused` says which guard fired. A
`recheck` **failure** collapses the job's `job_runs.outcome` to `failed` in step with §7.2; a refusal
does not.
...
> **AECI-944 made that alert more sensitive, and this is a behaviour change rather
> than a defect.** `indexnow-failure-rate` fires on >90% of `aeci.indexnow.submit`
> failing over 24 h, with a **≥3-submission floor**. The floor exists because
> coalescing makes submissions sparse on purpose: an empty buffer sends nothing, so a
> quiet day could sit under three attempts and never evaluate. The buffer now has a
> second appender, every vendor-portal write rather than only promotes, so it is non-empty
> on days it would previously have been empty, the drain attempts more submissions,
> and the floor clears more often. **The threshold and the window are unchanged. What
> changed is how often the alert gets to run at all.** Expect it to become louder in
...
|---|---|---|---|
| Project **annotation** | the vertical line PostHog draws across every insight — what makes "the p95 stepped up at 14:02" readable at a glance | personal `phx_` (`POSTHOG_CLI_API_KEY`) + a numeric project id, via the **management** host `us.posthog.com` | **Yes** — the key was provisioned 2026-09-04; it warn-skipped before that |
