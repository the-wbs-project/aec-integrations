...
| `aeci.indexnow.submit` | count | `apps/api/src/lib/indexnow-drain.ts` (`drainIndexNowQueue`, AECI-236 / AECI-826 / §20.2) — the `*/20` drain cron. **It moved**: until AECI-826 this was emitted from the promote's post-commit hook with `source:promote`, and the promote no longer submits at all | `source` (`cron`), `outcome` (`ok` / `failed`). One count per outbound submission **attempted** — a network or DNS error that never reached `api.indexnow.org` is `outcome:failed` at status `0`, not silence; `ok` covers both 200 (accepted) and 202 (received, validation pending). **429 is the case to know.** IndexNow rate-limits per host, the limit is undocumented, and a 429 means the batch reached the aggregator and was refused — it proves DNS, TLS and egress are fine and proves **nothing** about the key, because Bing throttles before it fetches `<key>.txt`. The transport retries a 429 (and any 5xx) twice with backoff and the failed rows stay buffered for the next tick, so a `failed` count here is a submission that survived both. **A zero series is ambiguous and always has been**: the drain emits nothing when the buffer is empty, and nothing when `INDEXNOW_KEY` / `PUBLIC_SITE_URL` are absent. Read it beside `aeci.indexnow.drain` (always emitted, so absence there is real) and `aeci.indexnow.pending` (depth, so a stuck channel climbs). **Do not use this metric to answer "is the key set?"** — run `wrangler secret list --env production` against **both** Workers (`docs/launch-cutover-runbook.md` §2). **History: 100% `outcome:failed` in production 2026-09-07..09** — 23 attempts, every one 429, zero successes, and nothing alerted because no alert existed (AECI-826). The alert now exists; see `docs/RUNBOOKS.md` → "IndexNow submissions refused" |
