...
| `aeci.cache.purge` | count | `apps/web/src/server/cache-purge-queue.ts` (WC-5 queue consumer — `promote`/`moderation`); `apps/web/src/server/routes/admin-purge.ts` (`manual`, HTTP until WC-6) | `source` (promote / moderation / manual), `outcome` (`ok` / `purge_failed` / `no_cache` / `noop`; `cf_failed` on the legacy `/admin/purge` path) |
