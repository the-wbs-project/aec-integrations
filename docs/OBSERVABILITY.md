...
| `aeci.cache.purge` | count | `apps/web/src/server/routes/admin-purge.ts` (native `ctx.cache.purge()` since WC-6 / AECI-320) | `source` (manual / ci-taxonomy-seed / future webhook), `outcome` (`ok` / `failed` / `skipped` — `skipped` = native cache disabled on the tier), `mode` (`tags` / `path_prefixes` / `combined` / `everything`) |
