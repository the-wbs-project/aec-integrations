...
| `aeci.indexnow.submit` | count | `apps/api/src/routes/promote.ts` (`notifyIndexNowAfterPromote`, AECI-236 / §20.2) — the post-commit hook, dispatched via `dispatchHook` beside the Algolia sync and the cache purge | `source` (`promote`), `outcome` (`ok` / `failed`). One count per submission that actually reached `api.indexnow.org`; `ok` covers both 200 (accepted) and 202 (received, validation pending). **A zero series is ambiguous, which is what AECI-801 was filed over.** The hook is skipped entirely without `INDEXNOW_KEY` + `PUBLIC_SITE_URL`, and skipped again when the promote touched no public URL, and neither skip emits anything. So "no data" means "no key", "no crawl-worthy write", or "the transport is down", and only the first is a defect. **Do not use this metric to answer "is the key set?"** — run `wrangler secret list --env production` against **both** Workers instead (`docs/launch-cutover-runbook.md` §2). `outcome:failed` carries its reason on the paired `aeci.api.promote.indexnow_failed` warn log |
