...
- **The no-Prisma rule is lint-enforced, tests included.** `no-restricted-imports` bans `@prisma/*`, `pg`, `postgres`, `@neondatabase/serverless` and the Postgres Drizzle drivers. `no-restricted-syntax` bans the `getPrisma` and `PrismaClient` identifiers and the `DATABASE_URL` / `DIRECT_URL` vars. A violation fails `pnpm lint` (AECI-549).
- There is no `getPrisma`, no Prisma in application code, and no `apps/api/prisma/schema.prisma` (AECI-278).
...
| `mailing_list` | `POST /api/subscribe` + `POST /api/unsubscribe` (`routes/landing-forms.ts`) | `GET /api/admin/subscribers` (AECI-859 — the only row-level reader); `GET /api/admin/audience` (AECI-586); `GET /api/admin/overview`; the 00:15 snapshot cron | Mailing-list signups (idempotent on `email`). |

**Columns, and which read surface exposes them (AECI-586).** `GET /api/admin/feedback` returns a `feedback` row whole — `id`, `created_at`, `features`, `tools`, `email`, `subscribed`, `country`, `city`, `region`, `timezone`, `referrer` — because the operator email fired from the handler was previously the table's only reader, so there is no prior projection to preserve. `GET /api/admin/audience` reads `mailing_list` in **aggregate only**: `created_at` and `unsubscribed_at` for the growth/churn series, `utm_source` / `utm_medium` / `utm_campaign` for attribution, and `country` / `region` / `city` / `asn` / `as_organization` for geography. It returns no row at all, and in particular no `email`.

**The row-level reader is `GET /api/admin/subscribers` (AECI-859), and its projection is an allow-list.** It returns `id`, `email`, `created_at`, `unsubscribed_at`, a derived `status`, the three `utm_*` columns, `referrer`, and `country` / `region` / `city` / `as_organization`. Two columns are deliberately absent. `asn` is omitted because the row already carries `as_organization`, which is the same fact in a form a human can read — a column of bare AS numbers is what the holder-name capture exists to replace. **`unsubscribe_token` is omitted because it is a bearer capability, not an identifier**: `POST /api/unsubscribe` suppresses whoever presents the token, with no other credential, so a response body carrying it would put a working opt-out link for every subscriber into a browser tab, a screenshot, and any log that captures a response. The handler therefore names its columns rather than selecting the row — `apps/api/src/lib/admin-audience.ts` (`listSubscribers`) — which is what stops a future column leaking one by default, and `admin-subscribers.spec.ts` asserts the token appears nowhere in the serialized body. `email` crossing in full is the opposite treatment from `page_views`' hashed visitor id, on the reasoning `API_CONTRACTS.md` §6.10 records for the feedback inbox: an address a person volunteered in order to be emailed is not an observation of someone who never identified themself.
...
- **`description` is `not null`** — the one column that differs from the sibling taxonomy tables, which stay nullable. Trade browse pages ship as SEO landing pages, so copy is part of the contract, not a later addition. The siblings no longer differ in *content*: since 2026-09-14 all 73 category / audience / phase terms ship a populated description too (ADR 0008 "Follow-ups", now closed). Only the constraint differs.
...
- **Two reads, and they differ by exactly this predicate** (AECI-303 shipped the second). `integrationPairConfig` applies `liveAttestationsWhere` and feeds the pair page's agreement + presence; `integrationTimelineConfig` (and its evidenced-pair twin `connectorEvidencedPairTimelineConfig`, AECI-1035) **omits it** and feeds `GET …/integrations/:otherSlug/timeline`, where the retracted rows *are* the history. Both live in `apps/api/src/lib/drizzle-helpers.ts`. Never call `computeAgreement` on the timeline config's output — routing history through the vote engine is how a withdrawn assertion finds its way back into a tally.
...
additive. **Rebuilt by `0049_rainy_puma.sql` (AECI-1092) onto two anchors**, below.

```sql
create table integration_field_challenges (
  id text primary key not null,
  integration_id text references integrations(id) on delete cascade,               -- nullable since 0049
  evidenced_pair_id text references connector_evidenced_pairs(id) on delete cascade, -- 0049
...
create unique index integration_field_challenges_open_evidenced_key                -- 0049
...
-- 0049: constraint integration_field_challenges_anchor_check
...
**Two anchors (AECI-1092, migration `0049_rainy_puma.sql`, a table rebuild).** A contest sits
...
- **Why the rebuild was safe.** Nothing holds a foreign key INTO this table. No migration names
  it in a `REFERENCES` clause, no `schema.ts` column references it, and the 0048 snapshot lists
...
- **What the rebuild carried.** Every row, copied with an explicit column list (the old
  ...
  `src/test/migration-0049.spec.ts` is the tripwire.
...
-- since AECI-1092's 0049, `integration_field_challenges.evidenced_pair_id`, a leaf. A
