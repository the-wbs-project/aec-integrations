...
- **`listing_tier` is on the records but ranks nothing yet (AECI-636, part A).** Adding it does not backfill it: the nightly sync is watermarked on `updated_at`, so a record only gains the attribute when its own row is next touched. Each environment needs one full sweep of `products` and `vendors` before the settings change that ranks on it can ship. Otherwise every product and vendor ties on an absent attribute and falls to arbitrary order. The §1.2 watermark sweep does that without emptying the index, though its script is hard-coded to `integrations` today and needs a `products` / `vendors` option first. The datatool reindex does it too, but it empties the index while it runs, and until AECI-1038 is fixed it also drops `verified` from every vendor record.
