...
export interface PromoteIntegrationResult {
  ...
  targetSlug?: string;
  /**
   * Slug of the connector product whose page this promote invalidated
   * (`integrations.powered_by_product_id`, or the connector of the
   * `connector_evidenced_pairs` row). Present so the cache-tag deriver can purge
   * the connector's own product detail page — its "Integrations it powers" hub
   * view (Stage 1.5 Addendum B) renders this edge, and without the tag it would
   * sit stale until the TTL. Absent when the integration has no powered-by
   * product, and older responses omit it entirely, so consumers must tolerate
   * its absence.
   *
   * **It is a purge target, not a read-back of what was stored (AECI-888).** On a
   * DE-ROUTE — an explicit `poweredByProduct: null` that moves an edge out of
   * `connector_evidenced_pairs` — the stored column ends up NULL and this field
   * carries the connector the edge moved AWAY from, because that is the page that
