...
- **Both themes, always.** Light and dark must both render correctly for every component before it ships. Theme switcher is implemented in `apps/web/src/app/theme.service.ts` (AECI-25); class-based `.theme-dark` toggle on `<html>`.
